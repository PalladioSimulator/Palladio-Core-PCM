...
/*
 * $Id: bbox.c,v 1.10 2000/08/04 08:46:47 bless Exp $
 *
 * $Log: bbox.c,v $
...
*  global variables   *
**********************/
const char *const version= "$Revision: 1.10 $ $Date: 2000/08/04 08:46:47 $";
                        ...
                        const unsigned char tight)
{
	...
	fprintf(stderr,"\nreading picture: %s   X: %u   Y: %u\n",name,xmax,ymax);
        ...
        if ( image_row )
        {
#ifdef _WIN32   /* this is really braindead */
...
{ 
  ...
  for (i= 1; i<argc; i++)
  {
    ...
    {
      fprintf(stderr,"%s: bbox Version %s\n",prgname,version);
      return 0;
    }
    else
    if ( strcmp(argv[i],"-h")==0 || strcmp(argv[i],"--help")==0 )
    {
      ...
      fprintf(stderr," usage: %s [-l] [-r resolution] [-V] [filename]\n",prgname);
      fprintf(stderr," -l: loose bounding box (bbox is expanded by 1 point)\n",prgname);
      fprintf(stderr," -r: resolution of picture in dpi\n");
      fprintf(stderr," -V: version information\n");
      fprintf(stderr," -h: this help\n");
      fprintf(stderr," bbox reads a rawppm or rawpbm file and prints out the\n");
      fprintf(stderr," bounding box of the image. If no filename is specified\n");
