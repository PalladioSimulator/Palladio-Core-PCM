...
import org.opt4j.gui.Viewer.CloseEvent;
...
@Info("A GUI to observe the optimization process.")
public class ViewerModule extends VisualizationModule {

	@Info("The title of the GUI frame.")
	@Constant(value = "title", namespace = Viewer.class)
	protected String title = "Viewer";

	@Info("Event on the optimization process if the GUI is closed")
	@Constant(value = "closeEvent", namespace = Viewer.class)
	protected CloseEvent closeEvent = CloseEvent.STOP;

	@Info("Close GUI automatically when the optimization process stops")
	...
	@Override
	public void config() {
		bind(Viewer.class).asEagerSingleton();
