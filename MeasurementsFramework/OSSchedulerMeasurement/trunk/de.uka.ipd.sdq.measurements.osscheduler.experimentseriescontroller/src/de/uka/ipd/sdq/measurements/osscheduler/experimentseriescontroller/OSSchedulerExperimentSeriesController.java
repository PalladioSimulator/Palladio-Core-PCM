...
public class OSSchedulerExperimentSeriesController extends AbstractSeriesController {
	...
	@Override
	public void startMeasurements(MeasurementScript script, int dataSourceId) {
		...
		for (TaskSet taskSet : script.getTaskSets()) {
			osSchedulerExperimentController.performExperiment(script.getMachineDescriptions(), taskSet, dataSourceId);
		}
	...
	public void startMeasurements(String machineIP, int machinePort, int dataSourceId) {
		...
		startMeasurements(script, dataSourceId);
	}
