...
/**
 * <!-- begin-user-doc --> A representation of the model object ' <em><b>Implementation Component
 ...
 * </p>
 * <ul>
 * <li>{@link org.palladiosimulator.pcm.repository.ImplementationComponentType#getParentCompleteComponentTypes
 * <em>Parent Complete Component Types</em>}</li>
 * <li>{@link org.palladiosimulator.pcm.repository.ImplementationComponentType#getComponentParameterUsage_ImplementationComponentType
...
public interface ImplementationComponentType extends RepositoryComponent {
    ...
    /**
     * Returns the value of the ' <em><b>Component Parameter Usage Implementation Component
     * Type</b></em>' containment reference list. The list contents are of type
     ...
     * @model annotation= "http://www.eclipse.org/uml2/1.1.0/GenModel body='-- ImplementationTypes
     *        required Interfaces have to be a subset\r\n-- of CompleteComponentType required
     *        Interfaces #\r\n--\r\n-- ACCx are used to accumulate Sets/Bags; usually only the very
     *        inner ACCx is used at all.\r\n--\r\n-- Recursive Query for parent Interface IDs\r\n--
     *        see \'lpar2005.pdf\' (Second-order principles in specification languages for
     *        Object-Oriented Programs; Beckert, Tretelman) pp. 11 #\r\n--let parentInterfaces :
     *        Bag(Interface) =\r\n--\tself.parentCompleteComponentTypes->iterate(pt :
     *        CompleteComponentType; acc1 : Bag(Interface) = Bag{}
     *        |\r\n--\t\tacc1->union(pt.requiredRoles->iterate(r : RequiredRole; acc2 :
     *        Bag(Interface) = Bag{}
     *        |\r\n--\t\t\tacc2->union(r.requiredInterface.parentInterface->asBag()) -- asBag
     *        required to allow Set operations #\r\n--\t\t))\r\n--\t) in\r\n--let
     *        anchestorInterfaces : Bag(Interface)
     *        =\r\n--\tself.parentCompleteComponentTypes->iterate(pt : CompleteComponentType; acc3 :
     *        Bag(Interface) = Bag{} |\r\n--\t\tacc3->union(pt.requiredRoles->iterate(r :
     *        RequiredRole; acc4 : Bag(Interface) = Bag{}
     *        |\r\n--\t\t\tacc4->union(r.requiredInterface.parentInterface->asBag()) -- asBag
     *        required to allow Set operations #\r\n--\t\t))\r\n--\t)->union( -- union with
     *        anchestors found in former recursion
     *        #\r\n--\t\tself.parentCompleteComponentTypes->iterate(pt : CompleteComponentType; acc5
     *        : Bag(Interface) = Bag{} |\r\n--\t\t\tacc5->union(pt.requiredRoles->iterate(r :
     *        RequiredRole; acc6 : Bag(Interface) = Bag{}
     *        |\r\n--\t\t\t\tacc6->union(r.requiredInterface.parentInterface.anchestorInterfaces)
     *        --already Set/Bag\r\n--\t\t\t))\r\n--\t\t)\r\n--\t) in\r\n-- Directly required
     *        interfaces need to be a subset of required anchestorInterfaces of Supertype
     *        #\r\n--anchestorInterfaces.identifier.id->includesAll(\r\n--\tself.requiredRoles->iterate(p
     *        : RequiredRole; acc7 : Bag(String) = Bag{}
     ...
     * @param context
     *            The cache of context-specific information. <!-- end-model-doc -->
     * @model annotation= "http://www.eclipse.org/uml2/1.1.0/GenModel body='-- ### EXACT COPY FROM
     *        ABOVE ###\r\n-- ImplementationComponentTypes provided Interfaces have to be a
     *        superset\r\n-- of CompleteComponentType provided Interfaces #\r\n--\r\n-- ACCx are
     *        used to accumulate Sets/Bags; usually only the very inner ACCx is used at
     *        all.\r\n--\r\n-- Recursive Query for parent Interface IDs\r\n-- see \'lpar2005.pdf\'
     *        (Second-order principles in specification languages for Object-Oriented Programs;
     *        Beckert, Tretelman) pp. 11 #\r\n--let parentInterfaces : Bag(Interface)
     *        =\r\n--\tself.providedRoles->iterate(r : ProvidedRole; acc2 : Bag(Interface) = Bag{}
     *        |\r\n--\t\tacc2->union(r.providedInterface.parentInterface->asBag()) -- asBag required
     ...
     *        |\r\n--\t\tacc4->union(r.providedInterface.parentInterface->asBag()) -- asBag required
     *        to allow Set operations #\r\n--\t)->union( -- union with anchestors found in former
     *        recursion #\r\n--\t\tself.providedRoles->iterate(r : ProvidedRole; acc6 :
     *        Bag(Interface) = Bag{}
     *        |\r\n--\t\t\tacc6->union(r.providedInterface.parentInterface.anchestorInterfaces)
     *        --already Set/Bag\r\n--\t\t)\r\n--\t) in\r\n\t-- Directly provided anchestorInterfaces
     *        need to be a superset of provided interfaces of Supertype
     *        #\r\n--\tanchestorInterfaces.identifier.id->includesAll(\r\n--\t\tself.parentProvidesComponentTypes->iterate(pt
     *        : ProvidesComponentType; acc1 : Bag(String) = Bag{}
     *        |\r\n--\t\t\tpt.providedRoles->iterate(r : ProvidedRole; acc2 : Bag(String) = Bag{}
     *        |\r\n--\t\t\t\tacc2->union(r.providedInterface.identifier.id->asBag()) -- asBag
     *        required to allow Set operations #\r\n--\t\t\t)\r\n--\t\t)\r\n--\t)\r\ntrue'"
     * @generated
     */
     ...
     * @param context
     *            The cache of context-specific information. <!-- end-model-doc -->
     * @model annotation= "http://www.eclipse.org/uml2/1.1.0/GenModel body='-- assures that
     *        InfrastructureComponents only have InfrastructureInterfaces and that
     *        BusinessComponents only have OperationInterfaces or EventGroups\r\nif
     *        self.componentType = ComponentType::INFRASTRUCTURE_COMPONENT
     *        then\r\n\tself.providedRoles_InterfaceProvidingEntity->select(role |
     *        role.oclIsTypeOf(OperationInterface) or role.oclIsTypeOf(EventGroup))->size() =
     *        0\r\nelse if self.componentType = ComponentType::BUSINESS_COMPONENT
     *        then\r\n\tself.providedRoles_InterfaceProvidingEntity->select(role |
     *        role.oclIsTypeOf(InfrastructureInterface))->size() = 0\r\nelse\r\n\t1 =
     *        0\r\nendif\r\nendif'"
     * @generated
     */
     ...
     *        ImplementationComponent has to provide the same or more interfaces like the
     *        CompleteComponentType (if set) #\r\n\tif\r\n\t\t -- apply constraint only for
     *        non-empty CompleteComponentTypes
     *        #\r\n\t\tself.parentCompleteComponentTypes->notEmpty()\r\n\tthen\r\n\t\tlet\r\n\t\t\t--own
     *        interfaces:\r\n\t\t\townInterfaces : Set(OperationInterface)\r\n\t\t\t =
     *        self.providedRoles_InterfaceProvidingEntity->select(pr|pr.oclIsTypeOf(OperationProvidedRole))->collect(pr
     *        : ProvidedRole | pr.oclAsType
     *        (OperationProvidedRole).providedInterface__OperationProvidedRole)->asSet()\r\n in \r\n
     *        \t--complete type interfaces:\r\n \tself.parentCompleteComponentTypes->forAll\r\n ( p
     *        : CompleteComponentType |\r\n (\r\n
     *        \tp.providedRoles_InterfaceProvidingEntity->select(pr|pr.oclIsTypeOf(OperationProvidedRole))->collect(pr
     *        : ProvidedRole | pr.oclAsType
     *        (OperationProvidedRole).providedInterface__OperationProvidedRole)->asSet()\r\n \t-\r\n
     *        \townInterfaces\r\n \t)->isEmpty()\r\n \t)\r\n\telse\r\n\t\ttrue\r\n\tendif'"
     * @generated
     ...
     *            The cache of context-specific information. <!-- end-model-doc -->
     * @model annotation= "http://www.eclipse.org/uml2/1.1.0/GenModel body='--
     *        ImplementationComponent has to require the same or fewer interfaces like the
     *        CompleteComponentType (if set) #\r\n\tif\r\n\t\t-- apply constraint only for non-empty
     *        CompleteComponentTypes
     *        #\r\n\t\tself.parentCompleteComponentTypes->notEmpty()\r\n\tthen\r\n\t let\r\n\t --own
     *        interfaces:\r\n\t ownInterfaces : Set(OperationInterface) \r\n\t =
     *        self.requiredRoles_InterfaceRequiringEntity->select(rr|rr.oclIsTypeOf(OperationRequiredRole))->collect(rr
     *        : RequiredRole | rr.oclAsType
     *        (OperationRequiredRole).requiredInterface__OperationRequiredRole)->asSet()\r\n\t
     *        in\r\n\t --complete type interfaces:\r\n\t
     *        self.parentCompleteComponentTypes->forAll\r\n\t ( p : CompleteComponentType |\r\n\t
     *        \t(\r\n\t\t \townInterfaces\r\n\t\t -\r\n\t\t
     *        p.requiredRoles_InterfaceRequiringEntity->select(rr|rr.oclIsTypeOf(OperationRequiredRole))->collect(rr
     *        : RequiredRole | rr.oclAsType
     *        (OperationRequiredRole).requiredInterface__OperationRequiredRole)->asSet()\r\n\t
     *        \t)->isEmpty()\r\n\t \t)\r\n\telse\r\n\t\ttrue\r\n\tendif'"
     * @generated
