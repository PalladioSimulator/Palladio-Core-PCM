...
/**
 * The {@code Configurator} configures the modules and starts the specific
 * {@code ApplicationFrame}.
 * 
 * @author lukasiewycz
...
public class Configurator {
	...
	/**
	 * Returns the module for the {@code Configurator}.
	 * 
	 * @param taskClass
	 *            the task class
	 * @return the module
	...
	/**
	 * Starts the {@code Configurator} with the specified task class and the
	 * file to be loaded.
	 * 
	 * @param taskClass
	 *            the task clsas
	 ...
	 */
	public void main(final Class<? extends Task> taskClass,
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				ApplicationFrame frame = injector
						.getInstance(ApplicationFrame.class);
				frame.startup();
				if (filename != null) {
					File file = new File(filename);
					FileOperations fo = injector
							.getInstance(FileOperations.class);
					fo.load(file);
				}
			}
		});
