...
public class ResourceUsageImpl extends EObjectImpl implements ResourceUsage {
	...
	public ProbabilityDensityFunction basicGetUsageTime() {
		ManagedPDF pdf;
		try {
			pdf = ManagedPDF.createFromString(this.usageTimeSpecification);
			return pdf.getModelPdf();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public void setUsageTime(ProbabilityDensityFunction newUsageTime) {
		ManagedPDF pdf = new ManagedPDF(newUsageTime);
		this.usageTimeSpecification = pdf.toString();
	}
