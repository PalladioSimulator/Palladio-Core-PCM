package com.palladio_simulator.protocom.framework.experiment;
...
import com.palladio_simulator.protocom.framework.registry.IRmiRegistry;
...
public class ExperimentManager extends UnicastRemoteObject implements IExperimentManager, Serializable {
	...
	/**
	 * Stores whether this ExperimentManager is a master or slave. The master
	 * ExperimentManager is informing the slave ones when the experiment run finishes.
	 */
	...
	public static void init(String experimentName, String directory, int remoteType) {
		...
		if (remoteType == EXPERIMENT_MANAGER_SLAVE) {
			IRmiRegistry registry = (IRmiRegistry) com.palladio_simulator.protocom.framework.registry.RmiRegistry.lookup(IRmiRegistry.PCM_EXPERIMENT_MANAGER_REGISTRY);
			try {
