...
public abstract class AbstractMain {
        ...
        if (itemId == 1) {
            ...
            try {
                ExperimentManager.init(runProps.getOptionValue('n') + " (Usage Scenario)", runProps.getOptionValue('d')
                        + "-UsageScenario", IExperimentManager.EXPERIMENT_MANAGER_MASTER);
            ...
            for (String containerId : containers) {
                if (itemId == i) {
                    ...
                    ExperimentManager.init(
                            runProps.getOptionValue('n') + " ("
                                    + AbstractAllocationStorage.getContainerName(containerId) + ")",
                            runProps.getOptionValue('d') + "-"
                                    + AbstractAllocationStorage.getContainerName(containerId),
                            IExperimentManager.EXPERIMENT_MANAGER_SLAVE);
    ...
    /**
     * Starts the prototype in a local mode. This can only be used for testing, since all
     * systems/components are started on one piece of hardware.
     */
    private void startLocalMode() {
        ExperimentManager.init(runProps.getOptionValue('n'), runProps.getOptionValue('d'),
                IExperimentManager.EXPERIMENT_MANAGER_SLAVE);
        ExperimentManager.getInstance().createExperimentRun();
