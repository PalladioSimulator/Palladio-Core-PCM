...
/**
 * Represents a failure type for a software-induced failure-on-demand
 * occurrence.
 * 
 * This failure type specification corresponds to the specification of
 * SoftwareInducedFailureTypes in a PCM Repository model.
 * 
 * @author brosch
 * 
 */
public class MarkovSoftwareInducedFailureType extends MarkovFailureType {
	...
	public static MarkovSoftwareInducedFailureType createExternalFailureType(
			...
			final Signature signature, final Role role, final Interface iface) {
		return new MarkovSoftwareInducedFailureType(evaluationType, failureType
				.getId(), failureType.getEntityName(), signature.getId(),
				signature.getEntityName(), role.getId(), role.getEntityName(),
				iface.getId(), iface.getEntityName());
	}
	...
	public static MarkovSoftwareInducedFailureType createExternalFailureType(
			final MarkovEvaluationType evaluationType,
			final String softwareFailureId, final String signatureId,
			final String systemRequiredRoleId) {
		return new MarkovSoftwareInducedFailureType(evaluationType,
				softwareFailureId, "", signatureId, "", systemRequiredRoleId,
				"", "", "");
	}
	 ...
	 * @param signature
	 *            the component-provided signature that exhibits the failure
	 *            type
	 * @param role
	 *            the component-provided role that exhibits the failure type
	 * @param iface
	 *            the component-provided interface that exhibits the failure
	 *            type
	 * @param component
	...
	public static MarkovSoftwareInducedFailureType createInternalFailureType(
			final MarkovEvaluationType evaluationType,
			final SoftwareInducedFailureType failureType,
			final InternalAction internalAction, final Signature signature,
			final Role role, final Interface iface,
			final BasicComponent component) {
		return new MarkovSoftwareInducedFailureType(evaluationType, failureType
				.getId(), failureType.getEntityName(), internalAction.getId(),
				internalAction.getEntityName(), signature.getId(), signature
						.getEntityName(), role.getId(), role.getEntityName(),
				iface.getId(), iface.getEntityName(), component.getId(),
	...
	public static MarkovSoftwareInducedFailureType createInternalFailureType(
		...
		return new MarkovSoftwareInducedFailureType(evaluationType,
				softwareFailureId, "", internalActionId, "", "", "", "", "",
				"", "", "", "");
	}
	...
	/**
	 * The constructor for software-induced failures outside the system, as
	 * annotated to system-external calls.
	 * 
	 * @param evaluationType
	 *            the degree of differentiation between failure types
	 * @param softwareFailureId
	 ...
	 *            System model
	 * @param interfaceId
	 *            the id of the associated system-required service interface as
	 *            specified in the PCM Repository model
	 * @param interfaceName
	 *            the name of the associated system-required service interface
	 *            as specified in the PCM Repository model
	 */
	private MarkovSoftwareInducedFailureType(
			final MarkovEvaluationType evaluationType,
			final String softwareFailureId, final String softwareFailureName,
			final String signatureId, final String signatureName,
			final String systemRequiredRoleId,
			final String systemRequiredRoleName, final String interfaceId,
			final String interfaceName) {
		switch (evaluationType) {
		case POINTSOFFAILURE:
			this.id = systemRequiredRoleId + "/" + signatureId + "/"
					+ softwareFailureId;
			this.name = systemRequiredRoleName + "/" + signatureName + "/"
					+ softwareFailureName
	 ...
	 *            PCM Repository model
	 * @param signatureId
	 *            the id of the associated service signature as specified in the
	 *            PCM Repository model
	 * @param signatureName
	 *            the name of the associated service signature as specified in
	 *            the PCM Repository model
	 * @param providedRoleId
	 *            the id of the associated component-provided role as specified
	 *            in the PCM Repository model
	 * @param providedRoleName
	 *            the name of the associated component-provided role as
	 *            specified in the PCM Repository model
	 * @param interfaceId
	 *            the id of the associated component-provided service interface
	 *            as specified in the PCM Repository model
	 * @param interfaceName
	 *            the name of the associated component-provided service
	 *            interface as specified in the PCM Repository model
	 * @param componentId
	...
	private MarkovSoftwareInducedFailureType(
			...
			final String internalActionId, final String internalActionName,
			final String signatureId, final String signatureName,
			final String providedRoleId, final String providedRoleName,
		...
		case POINTSOFFAILURE:
			...
			this.name = componentName + "/" + interfaceName + "/"
					+ signatureName + "/" + internalActionName + "/"
					+ softwareFailureName
					+ " (system-internal software-induced failure)";
			break;
		case TYPES:
