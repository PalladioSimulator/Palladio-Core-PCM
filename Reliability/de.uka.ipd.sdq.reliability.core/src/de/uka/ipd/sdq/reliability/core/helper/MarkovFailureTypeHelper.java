...
public class MarkovFailureTypeHelper {
	...
	/**
	 * Retrieves a list of potential failure types from a given PCM model
	 * instance.
	 * 
	 ...
	 */
	public List<MarkovFailureType> getFailureTypes(
			final MarkovEvaluationType evaluationType,
			final List<Repository> repositories,
			final ResourceEnvironment environment, final System system) {
		List<MarkovFailureType> failureTypes = getNetworkFailureTypes(
				evaluationType, environment, system);
		failureTypes.addAll(getHardwareFailureTypes(evaluationType,
				environment, system));
		failureTypes.addAll(getResourceTimeoutFailureTypes(evaluationType,
				system));
		failureTypes.addAll(getSoftwareFailureTypes(evaluationType,
				repositories, system));
	...
	/**
	 * Retrieves the list of processing resource failure types from the given
	 * PCM model.
	 * 
	 * @param evaluationType
	 *            the degree of differentiation between failure types
	 ...
	 */
	private List<MarkovFailureType> getHardwareFailureTypes(
			final MarkovEvaluationType evaluationType,
			final ResourceEnvironment environment, final System system) {
		...
		for (ResourceContainer container : environment
				.getResourceContainer_ResourceEnvironment()) {
			for (ProcessingResourceSpecification specification : container
					.getActiveResourceSpecifications_ResourceContainer()) {
				ProcessingResourceType resourceType = specification
						.getActiveResourceType_ActiveResourceSpecification();
				resultList.add(MarkovHardwareInducedFailureType
						.createInternalFailureType(evaluationType, container,
								resourceType));
			}
		}
		...
		for (QoSAnnotations qos : system.getQosAnnotations_System()) {
			for (SpecifiedQoSAnnotation annotation : qos
				...
				if (annotation instanceof SpecifiedReliabilityAnnotation) {
					...
					for (ExternalFailureOccurrenceDescription description : relAnnotation
						...
						if (failureType instanceof HardwareInducedFailureType) {
							...
							resultList
									.add(MarkovHardwareInducedFailureType
											.createExternalFailureType(
													...
													((OperationRequiredRole) annotation
															.getRole_SpecifiedQoSAnnotation())
	...
	/**
	 * Retrieves the list of communication failure types from the given PCM
	 * model.
	 * 
	 * @param evaluationType
	 *            the degree of differentiation between failure types
	 ...
	 */
	private List<MarkovFailureType> getNetworkFailureTypes(
			final MarkovEvaluationType evaluationType,
			final ResourceEnvironment environment, final System system) {
		...
		// Handle the PCM resource environment model:
		for (LinkingResource commLink : environment
			...
			resultList.add(MarkovNetworkInducedFailureType
					.createInternalFailureType(evaluationType, commLink,
							resourceType));
		}
				...
				if (annotation instanceof SpecifiedReliabilityAnnotation) {
					...
					for (ExternalFailureOccurrenceDescription description : relAnnotation
						...
						if (failureType instanceof NetworkInducedFailureType) {
							...
							resultList
									.add(MarkovNetworkInducedFailureType
											.createExternalFailureType(
													evaluationType,
													resourceType,
													annotation
	...
	/**
	 * Retrieves the list of resource timeout failure types from the given PCM
	 * model.
	 * 
	 * @param evaluationType
	 *            the degree of differentiation between failure types
	 ...
	 */
	private List<MarkovFailureType> getResourceTimeoutFailureTypes(
			final MarkovEvaluationType evaluationType, final System system) {

		// Initialize the list of results:
		ArrayList<MarkovFailureType> resultList = new ArrayList<MarkovFailureType>();

		// Handle the PCM system model:
		for (AssemblyContext context : system
				.getAssemblyContexts__ComposedStructure()) {
			RepositoryComponent component = context
					.getEncapsulatedComponent__AssemblyContext();
			if (!(component instanceof BasicComponent)) {
				continue;
			}
			for (PassiveResource resource : ((BasicComponent) component)
					.getPassiveResource_BasicComponent()) {
				resultList
						.add(MarkovResourceTimeoutFailureType
								.createResourceTimeoutFailureType(
										evaluationType, context,
										((BasicComponent) component), resource));
			}
		}
	 ...
	 */
	private List<MarkovFailureType> getSoftwareFailureTypes(
			final MarkovEvaluationType evaluationType,
		...
		for (Repository repository : repositories) {
			...
			for (Object c_object : components) {
				...
				for (ProvidedRole role : component
						.getProvidedRoles_InterfaceProvidingEntity()) {
					if (role instanceof OperationProvidedRole) {
						OperationProvidedRole opRole = (OperationProvidedRole) role;
						OperationInterface iface = opRole
								.getProvidedInterface__OperationProvidedRole();
						for (OperationSignature signature : iface
								.getSignatures__OperationInterface()) {
							ServiceEffectSpecification rdseff = getRDSEFF(
									component, signature);
							EList<EObject> internalActions = helper
									.getElements(rdseff, SeffFactory.eINSTANCE
							...
							for (Object a_object : internalActions) {
								...
								for (InternalFailureOccurrenceDescription description : internalAction
									...
									resultList
											.add(MarkovSoftwareInducedFailureType
	 ...
	 * @return the RDSEFF
	 */
	private ServiceEffectSpecification getRDSEFF(
			final BasicComponent component, final OperationSignature signature) {
		for (ServiceEffectSpecification rdseff : component
				.getServiceEffectSpecifications__BasicComponent()) {
			if (rdseff.getDescribedService__SEFF().getId().equals(
					signature.getId())) {
				return rdseff;
			}
		}
	 ...
	 * 
	 * Double failure types can occur if the distinction of failure types is not
	 * at the most fine-granular level.
	 * 
	 * @param failureTypes
	 *            the list of failure types
	 * @return the result list
	 */
	private List<MarkovFailureType> removeDoubleFailureTypes(
