...
/**
 * This class has the functionality to perform a complete PCM2Markov
 * transformation.
 * 
 * @author brosch
 * 
...
public class MarkovTransformation {
	 ...
	 * transformation.
	 */
	private static Logger logger = Logger.getLogger(MarkovTransformation.class
			.getName());
	 ...
	 */
	private void runDSolver(final UsageScenario scenario,
	 ...
	 * @param markovResult
	 *            results of the PCM2MarkovTransformation
	 * @return true if the transformation has stopped before completion, such
	 *         that the result has to be approximated
	 */
	private boolean runPcm2Markov(
			final PCMSolverWorkflowRunConfiguration configuration,
		...
		if (!configuration.isIterationOverPhysicalSystemStatesEnabled()) {
			...
			// "always ask" strategy:
			runPcm2MarkovSingle(configuration, scenario, markovSource,
					markovResult);
		} else {
			// Repeat the transformation for all physical system states:
			approximate = runPcm2MarkovIteratively(configuration, scenario,
					markovSource, markovResult);
		}
	 ...
	 *         that the result has to be approximated
	 */
	private boolean runPcm2MarkovIteratively(
			final PCMSolverWorkflowRunConfiguration configuration,
			final UsageScenario scenario,
			final MarkovTransformationSource markovSource,
			final MarkovTransformationResult markovResult) {
		...
		for (int permutationClass = 0; permutationClass <= size; permutationClass++) {
			...
			for (;;) {
				...
				// Check if any of the stop conditions is reached:
				if (isStopConditionReached(configuration, markovResult,
						startTimeMs)) {
					return true;
				}
	...
	private void runPcm2MarkovSingle(
		...
		MarkovUsageModelVisitor visitor = new MarkovUsageModelVisitor(
				markovSource, getEvaluationMode(configuration), !configuration
	 ...
	 * @param sensitivity
	 *            sensitivity analysis configuration
	 * @return the transformation results (only the results of the last step in
	 *         the sensitivity analysis are returned)
	 */
	private List<MarkovTransformationResult> runTransformIteratively(
			final PCMInstance model,
			final PCMSolverWorkflowRunConfiguration configuration,
			final MarkovSensitivity sensitivity) {
		...
		while (step != null) {
			...
			logger.info("Sensitivity analysis step " + sensitivityStepCount
					+ " completed");
	 ...
	 * @return the transformation results
	 */
	private List<MarkovTransformationResult> runTransformSingle(
			final PCMInstance model,
			final PCMSolverWorkflowRunConfiguration configuration) {
		...
		for (UsageScenario scenario : model.getUsageModel()
			...
			resultList
					.add(runScenarioTransform(model, configuration, scenario));
		}
