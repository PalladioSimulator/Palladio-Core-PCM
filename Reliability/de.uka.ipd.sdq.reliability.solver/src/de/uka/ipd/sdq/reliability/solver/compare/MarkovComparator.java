 ...
 * 
 * The comparison is based on the tracing information which uniquely identifies
 * each Markov state.
 * 
 * @author brosch
 * 
...
public class MarkovComparator {
	 ...
	 */
	private static Logger logger = Logger.getLogger(MarkovComparator.class
			.getName());
	...
	/**
	 * Counts the number of transitions with changed properties in chains #1 and
	 * #2.
	 */
	 ...
	 *            the second chain
	 */
	public void compare(final MarkovChain firstChain,
			final MarkovChain secondChain) {
		...
		compareTransitions(firstChain.getTransitions(), secondChain
				.getTransitions());
	...
	/**
	 * Loads and compares the Markov models identified by the two given file
	 * names.
	 * 
	 ...
	 */
	public void compare(final String firstMarkovFileName,
			final String secondMarkovFileName) {
		...
		if ((ch1 == null) || !(ch1 instanceof MarkovChain) || (ch2 == null)
				|| !(ch2 instanceof MarkovChain)) {
			return;
		}
	...
	private void compareProperties(final MarkovChain ch1, final MarkovChain ch2) {
		if (!ch1.getName().equals(ch2.getName())) {
			logger.debug("Chain name changed from \"" + ch1.getName()
					+ "\" to \"" + ch2.getName() + "\"");
		}
	}
	...
	private void compareProperties(final State st1, final State st2) {
		...
		if (!typeEquals || !nameEquals || !labelsEqual) {
			...
			if (!nameEquals) {
				logger.debug("- mame changed from \"" + st1.getName()
						+ "\" to \"" + st2.getName() + "\"");
			}
			if (!typeEquals) {
				logger.debug("- type changed from \""
						+ st1.getType().toString() + "\" to \""
	...
	private void compareProperties(final Transition tr1, final Transition tr2) {
		...
		boolean probabilityEquals = (tr1.getProbability() == tr2
				.getProbability());
		...
		if (!nameEquals || !probabilityEquals) {
			...
			if (!nameEquals) {
				logger.debug("- mame changed from \"" + tr1.getName()
						+ "\" to \"" + tr2.getName() + "\"");
			}
			if (!probabilityEquals) {
				logger.debug("- probability changed from "
						+ tr1.getProbability() + " to " + tr2.getProbability());
			}
		}
	}
	 ...
	 */
	private void compareStates(final EList<State> states1,
			final EList<State> states2) {
	 ...
	 *            the transitions of the second chain
	 */
	private void compareTransitions(final EList<Transition> transitions1,
			final EList<Transition> transitions2) {
	 ...
	 */
	private void printCompareStatistics(final MarkovChain firstChain,
			final MarkovChain secondChain) {
		...
		int numberOfUnchangedStates1 = numberOfStates1 - numberOfChangedStates1
				- numberOfDeletedStates1;
		int numberOfChangedStates2 = statePropertyChangeCount;
		int numberOfAddedStates2 = newStateCount;
		int numberOfUnchangedStates2 = numberOfStates2 - numberOfChangedStates2
				- numberOfAddedStates2;
		...
		int numberOfUnchangedTransitions1 = numberOfTransitions1
				- numberOfChangedTransitions1 - numberOfDeletedTransitions1;
		int numberOfChangedTransitions2 = transitionPropertyChangeCount;
		int numberOfAddedTransitions2 = newTransitionCount;
		int numberOfUnchangedTransitions2 = numberOfTransitions2
				- numberOfChangedTransitions2 - numberOfAddedTransitions2;
		...
		logger.info("- added:     " + df1.format(numberOfAddedStates2) + " ("
				+ df2.format(percentageOfAddedStates2) + "%)");
	 ...
	 * @return
	 */
	private boolean testEquality(final EList<Label> labels1,
			final EList<Label> labels2) {
		...
		for (int i = 0; i < labels1.size(); i++) {
			...
			if (!label1.getValue().equals(label1_2.getValue())
					|| !label2.getValue().equals(label2_1.getValue())) {
				return false;
			}
		}
		return true;
	 ...
	 * 
	 * The transitions are considered to have the same identity if they
	 * connected the same states.
	 * 
	 * @param t1
	...
	private boolean testIdentity(final Transition t1, final Transition t2) {
		return testIdentity(t1.getFromState(), t2.getFromState())
				&& testIdentity(t1.getToState(), t2.getToState());
	}
}
