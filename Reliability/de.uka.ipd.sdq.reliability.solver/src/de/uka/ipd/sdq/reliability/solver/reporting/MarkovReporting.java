...
/**
 * Class used for aggregation and output of success/failure probabilities that
 * were calculated during a reliability analysis.
 * 
...
public class MarkovReporting {
	 ...
	 * aggregation according to the given Markov transformation results.
	 * 
	 * @param markovResults
		...
		for (MarkovFailureType failureType : cumulatedFailureTypeProbabilities
			...
			if (!failureType.isSystemExternal()) { // only consider (internal)
				...
				if (failureType instanceof MarkovSoftwareInducedFailureType) {
					...
					nameParts
							.add(softwareInducedFailureType.getComponentName());
					...
					if (!foundEntry) {
						...
						failureProbabilityAggregations
								.add(new ImpactAnalysisFailureProbabilityAggregation(
										ImpactAnalysisFailureType.COMPONENTS_INTERNAL_ACTIONS,
										identifiers, nameParts,
			...
			if (!failureType.isSystemExternal()) { // only consider (internal)
				...
				if (failureType instanceof MarkovSoftwareInducedFailureType) {
					...
					nameParts
							.add(softwareInducedFailureType.getComponentName());
					nameParts
							.add(softwareInducedFailureType.getInterfaceName());
					for (ImpactAnalysisFailureProbabilityAggregation aggregation : failureProbabilityAggregations) {
			...
			if (!failureType.isSystemExternal()) { // only consider (internal)
				...
				if (failureType instanceof MarkovSoftwareInducedFailureType) {
					...
					nameParts
							.add(softwareInducedFailureType.getInterfaceName());
					nameParts
							.add(softwareInducedFailureType.getSignatureName());
					for (ImpactAnalysisFailureProbabilityAggregation aggregation : failureProbabilityAggregations) {
						if (aggregation
								.compareToIdentifier(
	...
	private void createClassesFailureAnalysisTable(
		...
		MarkovReportingTable failureCategoryTable = new MarkovReportingTable(
		...
		List<String> row = new ArrayList<String>(failureCategoryTableHeaderRow
				.size());
		row.add("Software-induced failure");
		failureProbability = softwareInducedFailureAggregation
				.getFailureProbability();
		row.add(String.valueOf(doApproximate ? getFormattedProbabilityAsString(
				failureProbability, failureProbability + 1
		...
		failureProbability = hardwareInducedFailureAggregation
				.getFailureProbability();
		row.add(String.valueOf(doApproximate ? getFormattedProbabilityAsString(
				failureProbability, failureProbability + 1
						- cumulatedPhysicalStateProbability)
		...
		failureProbability = networkInducedFailureAggregation
				.getFailureProbability();
		row.add(String.valueOf(doApproximate ? getFormattedProbabilityAsString(
				failureProbability, failureProbability + 1
						- cumulatedPhysicalStateProbability)
	...
	private void createImpactAnalysisTables(
		...
		List<String> componentsInternalActionFailureProbabilitiesTableHeaderRow = new ArrayList<String>(
				3);
		componentsInternalActionFailureProbabilitiesTableHeaderRow
				.add("Component");
		componentsInternalActionFailureProbabilitiesTableHeaderRow
				.add("Aggregated Failure Mode Probability");
		...
		List<String> componentsServiceFailureProbabilitiesTableHeaderRow = new ArrayList<String>(
				3);
		...
		componentsServiceFailureProbabilitiesTableHeaderRow
				.add("Aggregated Failure Mode Probability");
		componentsServiceFailureProbabilitiesTable
				.setHeaderRow(componentsServiceFailureProbabilitiesTableHeaderRow);
		...
		List<String> componentsServiceOperationFailureProbabilitiesTableHeaderRow = new ArrayList<String>(
				4);
		componentsServiceOperationFailureProbabilitiesTableHeaderRow
				.add("Component");
		componentsServiceOperationFailureProbabilitiesTableHeaderRow
				.add("Interface");
		componentsServiceOperationFailureProbabilitiesTableHeaderRow
		...
		List<String> externalServiceFailureProbabilitiesTableHeaderRow = new ArrayList<String>(
				4);
		externalServiceFailureProbabilitiesTableHeaderRow
				.add("System-required Role");
		externalServiceFailureProbabilitiesTableHeaderRow.add("Interface");
		externalServiceFailureProbabilitiesTableHeaderRow
				.add("Aggregated Failure Mode Probability");
		externalServiceFailureProbabilitiesTable
				.setHeaderRow(externalServiceFailureProbabilitiesTableHeaderRow);
		...
		List<String> externalServiceOperationFailureProbabilitiesTableHeaderRow = new ArrayList<String>(
				4);
		externalServiceOperationFailureProbabilitiesTableHeaderRow
				.add("System-required Role");
		externalServiceOperationFailureProbabilitiesTableHeaderRow
				.add("Interface");
		externalServiceOperationFailureProbabilitiesTableHeaderRow
		...
		/*
		 * Create a table row for each failure probability aggregation, and add
		...
		for (ImpactAnalysisFailureProbabilityAggregation aggregation : failureProbabilityAggregations) {
			...
			case COMPONENTS_SERVICES:
				...
				for (String entityNamePart : aggregation.getEntityNameParts()) { // add
																					...
																					// name
																					// part
					componentsServicesFailureProbabilitiesTableRow
							.add(entityNamePart);
				}
				componentsServicesFailureProbabilitiesTableRow
						.add(doApproximate ? getFormattedProbabilityAsString(
								...
								: getFormattedProbabilityAsString(failureProbability)); // also
																						...
																						// row
				componentsServiceFailureProbabilitiesTable
						.addRow(componentsServicesFailureProbabilitiesTableRow); // insert
																					// row
																					// into
																					// table
				break;
			...
			case EXTERNAL_SERVICES:
				...
				for (String entityNamePart : aggregation.getEntityNameParts()) { // add
																					...
																					// name
																					// part
					externalServiceFailureProbabilitiesTableRow
							.add(entityNamePart);
				}
				externalServiceFailureProbabilitiesTableRow
						.add(doApproximate ? getFormattedProbabilityAsString(
								...
								: getFormattedProbabilityAsString(failureProbability)); // also
																						...
																						// row
				externalServiceFailureProbabilitiesTable
						.addRow(externalServiceFailureProbabilitiesTableRow); // insert
																				// row
																				// into
																				// table
				break;
		...
		if (componentsInternalActionFailureProbabilitiesTable.getRows().size() > 0) {
			markovReportItem
					.addImpactAnalysisTable(componentsInternalActionFailureProbabilitiesTable);
		}
		if (componentsServiceFailureProbabilitiesTable.getRows().size() > 0) {
			markovReportItem
					.addImpactAnalysisTable(componentsServiceFailureProbabilitiesTable);
		}
		if (componentsServiceOperationFailureProbabilitiesTable.getRows()
				.size() > 0) {
			markovReportItem
					.addImpactAnalysisTable(componentsServiceOperationFailureProbabilitiesTable);
		}
		if (externalServiceFailureProbabilitiesTable.getRows().size() > 0) {
			markovReportItem
					.addImpactAnalysisTable(externalServiceFailureProbabilitiesTable);
		}
		if (externalServiceOperationFailureProbabilitiesTable.getRows().size() > 0) {
			markovReportItem
					.addImpactAnalysisTable(externalServiceOperationFailureProbabilitiesTable);
		}
	}

	/*
	 * Consider all Markov transformation results: For each such result we will
	 * calculate the probabilities of interest, and then enclose them in a
	 * MarkovResultItem for further processing.
	 */
	private void createMarkovReportItems() {
		...
		MarkovEvaluationType mode = MarkovEvaluationType.valueOf(configuration
		...
		for (MarkovTransformationResult markovResult : markovResults) {
			...
			/*
			 * Create failure mode tables first: Create a table row for each
			 * failure probability aggregation, and add this row to the
			...
			/*
			 * Calculate accumulated failure probabilities if our configuration
			 * is set accordingly. Otherwise, we are done at this point.
			 */
			if (mode == MarkovEvaluationType.POINTSOFFAILURE) {
				...
				/*
				 * Now, create impact analysis tables. Insert calculated failure
				 * probabilities into report.
				 */
	...
	private void createPointsOfFailureAnalysisTable(
		...
		List<String> internalSoftwareFailuresTableHeaderRow = new ArrayList<String>(
				6);
		...
		internalSoftwareFailuresTable
				.setHeaderRow(internalSoftwareFailuresTableHeaderRow);
		...
		List<String> internalHardwareFailuresTableHeaderRow = new ArrayList<String>(
				3);
		...
		internalHardwareFailuresTable
				.setHeaderRow(internalHardwareFailuresTableHeaderRow);
		...
		List<String> internalNetworkFailuresTableHeaderRow = new ArrayList<String>(
				3);
		internalNetworkFailuresTableHeaderRow.add("Communication Link");
		internalNetworkFailuresTableHeaderRow
				.add("Communication Resource Type");
		internalNetworkFailuresTableHeaderRow.add("Failure Mode Probablity");
		internalNetworkFailuresTable
				.setHeaderRow(internalNetworkFailuresTableHeaderRow);
		...
		List<String> externalSoftwareFailuresTableHeaderRow = new ArrayList<String>(
				4);
		...
		externalSoftwareFailuresTable
				.setHeaderRow(externalSoftwareFailuresTableHeaderRow);
		...
		List<String> externalHardwareFailuresTableHeaderRow = new ArrayList<String>(
				4);
		externalHardwareFailuresTableHeaderRow.add("System-required Role");
		externalHardwareFailuresTableHeaderRow.add("Signature");
		externalHardwareFailuresTableHeaderRow
				.add("Communication Resource Type");
		externalHardwareFailuresTableHeaderRow.add("Failure Mode Probablity");
		externalHardwareFailuresTable
				.setHeaderRow(externalHardwareFailuresTableHeaderRow);
		...
		List<String> externalNetworkFailuresTableHeaderRow = new ArrayList<String>(
				4);
		externalNetworkFailuresTableHeaderRow.add("System-required Role");
		externalNetworkFailuresTableHeaderRow.add("Signature");
		externalNetworkFailuresTableHeaderRow
				.add("Communication Resource Type");
		externalNetworkFailuresTableHeaderRow.add("Failure Mode Probablity");
		externalNetworkFailuresTable
				.setHeaderRow(externalNetworkFailuresTableHeaderRow);
		...
		for (MarkovFailureType failureType : failureTypesSorted) {
			double failureProbability = getFailureTypeProbability(failureType,
					cumulatedFailureTypeProbabilities);
			if (!(failureProbability > 0.0)) {
				continue;
			}
			if (failureType.isSystemExternal()) { // external
				if (failureType instanceof MarkovSoftwareInducedFailureType) {
					...
					externalSoftwareFailuresTableRow
							.add(softwareInducedFailureType.getRoleName());
					externalSoftwareFailuresTableRow
							.add(softwareInducedFailureType.getSignatureName());
					externalSoftwareFailuresTableRow
							.add(softwareInducedFailureType
					...
					externalSoftwareFailuresTableRow
							.add(doApproximate ? getFormattedProbabilityAsString(
									...
									failureProbability));
					externalSoftwareFailuresTable
							.addRow(externalSoftwareFailuresTableRow);
					...
					externalHardwareFailuresTableRow
							.add(hardwareInducedFailureType.getRoleName());
					externalHardwareFailuresTableRow
							.add(hardwareInducedFailureType.getSignatureName());
					externalHardwareFailuresTableRow
							.add(hardwareInducedFailureType
									.getResourceTypeName());
					externalHardwareFailuresTableRow
							.add(doApproximate ? getFormattedProbabilityAsString(
							/*
							...
							failureProbability + 1
									...
									failureProbability));
					externalHardwareFailuresTable
							.addRow(externalHardwareFailuresTableRow);
					...
					externalNetworkFailuresTableRow
							.add(networkInducedFailureType.getRoleName());
					externalNetworkFailuresTableRow
							.add(networkInducedFailureType.getSignatureName());
					externalNetworkFailuresTableRow
							.add(networkInducedFailureType
					...
					externalNetworkFailuresTableRow
							...
							failureProbability + 1
									...
									failureProbability));
					externalNetworkFailuresTable
							.addRow(externalNetworkFailuresTableRow);
				}
			} else { // internal
				if (failureType instanceof MarkovSoftwareInducedFailureType) {
					MarkovSoftwareInducedFailureType softwareInducedFailureType = (MarkovSoftwareInducedFailureType) failureType;
					...
					internalSoftwareFailuresTableRow
							...
							failureProbability + 1
									...
									failureProbability));
					internalSoftwareFailuresTable
							.addRow(internalSoftwareFailuresTableRow);
				} else if (failureType instanceof MarkovHardwareInducedFailureType) {
					MarkovHardwareInducedFailureType hardwareInducedFailureType = (MarkovHardwareInducedFailureType) failureType;
					...
					internalHardwareFailuresTableRow
							...
							failureProbability + 1
									...
									failureProbability));
					internalHardwareFailuresTable
							.addRow(internalHardwareFailuresTableRow);
				} else if (failureType instanceof MarkovNetworkInducedFailureType) {
					MarkovNetworkInducedFailureType networkInducedFailureType = (MarkovNetworkInducedFailureType) failureType;
					...
					internalNetworkFailuresTableRow
							...
							failureProbability + 1
									...
									failureProbability));
					internalNetworkFailuresTable
							.addRow(internalNetworkFailuresTableRow);
				}
			}
		}

		/*
		 * Finally, add the generated failure mode tables to our report item if
		 * they contain at least one row (per table).
		 */
	...
	private void createTypesFailureAnalysisTable(
		...
		double failureProbability;
		// create tables
		MarkovReportingTable softwareInducedFailuresTable = new MarkovReportingTable(
				"Software-induced failure modes");
		List<String> softwareInducedFailuresTableHeaderRow = new ArrayList<String>(
				2);
		softwareInducedFailuresTableHeaderRow.add("Failure Type");
		softwareInducedFailuresTableHeaderRow.add("Failure Mode Probability");
		softwareInducedFailuresTable
				.setHeaderRow(softwareInducedFailuresTableHeaderRow);

		MarkovReportingTable hardwareInducedFailuresTable = new MarkovReportingTable(
				"Hardware-induced failure modes");
		List<String> hardwareInducedFailuresTableHeaderRow = new ArrayList<String>(
				2);
		...
		hardwareInducedFailuresTable
				.setHeaderRow(hardwareInducedFailuresTableHeaderRow);

		MarkovReportingTable networkInducedFailuresTable = new MarkovReportingTable(
				"Network-induced failure modes");
		List<String> networkInducedFailuresTableHeaderRow = new ArrayList<String>(
				2);
		...
		networkInducedFailuresTable
				.setHeaderRow(networkInducedFailuresTableHeaderRow);
			...
			if (!type.isSystemExternal()) { // only consider internal failures
				if (type instanceof MarkovSoftwareInducedFailureType) { // only
					...
					for (TypesFailureProbabilityAggregation aggregation : failureProbabilityAggregations) {
						if (aggregation.compareTo(
								FailureAnalysisFailureType.SOFTWARE_INDUCED,
								softwareInducedFailureType
										.getSoftwareFailureName())) { // see if
																		// there
																		// exists
																		// already
																		// a
							...
							// probabilities
							aggregation
									.addToFailureProbability(cumulatedFailureTypeProbabilities
					...
					if (!foundEntry) {
						failureProbabilityAggregations
								.add(new TypesFailureProbabilityAggregation(
										FailureAnalysisFailureType.SOFTWARE_INDUCED,
										softwareInducedFailureType
												.getSoftwareFailureName(),
										cumulatedFailureTypeProbabilities
					...
					for (TypesFailureProbabilityAggregation aggregation : failureProbabilityAggregations) {
						if (aggregation.compareTo(
								FailureAnalysisFailureType.NETWORK_INDUCED,
								networkInducedFailureType
										.getCommLinkResourceTypeName())) { // see
																			// if
																			// there
																			// exists
																			// already
		...
		for (TypesFailureProbabilityAggregation aggregation : failureProbabilityAggregations) {
			...
			case SOFTWARE_INDUCED:
				...
				softwareInducedFailuresTableRow.add(aggregation
						.getTypeIdentifier());
				softwareInducedFailuresTableRow
						.add(String
								.valueOf(doApproximate ? getFormattedProbabilityAsString(
										failureProbability,
										failureProbability
												+ 1
												- cumulatedPhysicalStateProbability)
										: getFormattedProbabilityAsString(failureProbability)));
				softwareInducedFailuresTable
						.addRow(softwareInducedFailuresTableRow);
				break;
			case HARDWARE_INDUCED:
				hardwareInducedFailuresTableRow = new ArrayList<String>(2);
				hardwareInducedFailuresTableRow.add(aggregation
						.getTypeIdentifier());
				hardwareInducedFailuresTableRow
						.add(String
								.valueOf(doApproximate ? getFormattedProbabilityAsString(
										failureProbability,
										failureProbability
										...
										: getFormattedProbabilityAsString(failureProbability)));
				hardwareInducedFailuresTable
						.addRow(hardwareInducedFailuresTableRow);
				break;
			case NETWORK_INDUCED:
				networkInducedFailuresTableRow = new ArrayList<String>(2);
				networkInducedFailuresTableRow.add(aggregation
						.getTypeIdentifier());
				networkInducedFailuresTableRow
						.add(String
								.valueOf(doApproximate ? getFormattedProbabilityAsString(
										failureProbability,
										failureProbability
										...
										: getFormattedProbabilityAsString(failureProbability)));
				networkInducedFailuresTable
						.addRow(networkInducedFailuresTableRow);
				break;
			default:
				break;
			}
	 ...
	 */
	private double getFailureTypeProbability(
