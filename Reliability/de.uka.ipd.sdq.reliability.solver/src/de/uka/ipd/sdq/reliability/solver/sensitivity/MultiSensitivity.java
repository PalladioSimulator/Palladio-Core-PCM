...
/**
 * Provides a possibility to define multiple sensitivity parameters to be
 * defined within one sensitivity analysis.
 * 
 * @author brosch
 * 
 */
		...
		for (int i = 0; i < existingHeadings.size(); i++) {
			...
			for (int j = 0; j < existingHeadings.get(i).size(); j++) {
				...
				int resultIndex = resultList.size()
						- existingHeadings.get(i).size() + j;
				resultList.get(resultIndex).addAll(
						existingHeadings.get(i).get(j));
			}
