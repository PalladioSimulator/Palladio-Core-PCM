	...
	public MarkovChain caseExternalCallAction(
		...
		if (seff == null) {
			...
			for (ExternalFailureOccurrenceDescription description : rawFailureDescriptions) {
				StopFailureType failureType = description
						.getStopFailureType__ExternalFailureOccurrenceDescription();
				if (failureType instanceof SoftwareInducedFailureType) {
					resultFailureDescriptions.add(FailureDescription
							.createSoftwareInducedFailureDescription(
									failureType.getEntityName(), failureType
											.getId(), description
		...
		for (ProcessingResourceDescriptor descriptor : descriptors) {
			...
			if (state.equals(MarkovResourceState.NA)) {
				failureDescriptions.add(FailureDescription
						.createResourceFailureDescription(descriptor.getType()
								.getName(), descriptor.getType().getId(), 1.0));
				continue;
			}
		}
