...
/**
 * This creates a probability density function, not a PDF document.
 * 
 * @author Steffen or Jens or Henning or Ihssane, followed by Michael Kuperberg
 * 
 */
public class ExportDoublePDF extends Action {
	...
	private String exportCSV(XYSeries series) {
		...
		for (Object item : series.getItems()) {
			...
			result += "("
					+ (data.getX().doubleValue() - viewer.getHistogramWidth() / 2)
					+ ";" + lastProb + ")";
			lastProb = data.getY().doubleValue();
			lastX = data.getX().doubleValue();
		}
		return result + "(" + (lastX + viewer.getHistogramWidth() / 2) + ";"
				+ lastProb + ")]";
	}
}
