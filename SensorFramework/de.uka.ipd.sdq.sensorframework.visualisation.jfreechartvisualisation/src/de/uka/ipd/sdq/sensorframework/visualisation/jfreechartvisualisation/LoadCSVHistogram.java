...
public class LoadCSVHistogram extends Action {
	...
	public void run() {
		...
		if (dialog.getFileName() != null) {
			this.histogram = importHistogram(dialog.getFilterPath()
	...
	private Histogram importHistogram(String fileName) {
		Histogram h = new Histogram("Measurements");//TODO here, the x axis label cannot be identified
		...
		try {
			...
			while ((row = bufferedReader.readLine()) != null) {
				...
				h.addEntity(new HistogramBucketInformation(y, x));// TODO: why
																	// in this
																	// order?
			}
		} catch (Exception e) {
		...
		for (HistogramBucketInformation e : h.getBucketInformation()) {
			System.out.print("runif(" + Math.round(e.getProbability() * 100000)
					+ "," + (e.getValue() - 5) + "," + (e.getValue() + 5)
					+ "),");
		}
		return h;
	}
