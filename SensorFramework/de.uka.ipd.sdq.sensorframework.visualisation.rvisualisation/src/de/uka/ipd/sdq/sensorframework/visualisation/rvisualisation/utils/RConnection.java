...
//		System.setProperties(prop);
		...
		if (!Rengine.versionCheck()) {
			...
			.log(
					IStatus.ERROR,
					"Creating R engine ** Version mismatch - Java files (version "+javaVersion+") do not "
					+ "match library version (version "+rniVersion+").");
			new MessageDialog(
					PlatformUI.getWorkbench().getActiveWorkbenchWindow()
					.getShell(),
					"Error loading R",
					null,
					"Could not load R. The version of the java files (version "+javaVersion+") and the "
					+ "library versions (version "+rniVersion+") do not match.",
			...
			new MessageDialog(
					...
					"Error loading R",
					null,
					"Could not load R 2.12. You need to install the correct "
		...
		if (!rengine.waitForR()) {
			...
			new MessageDialog(
					...
					+"Please check the output on the console for more details, as they are written to System.out and System.err",
					MessageDialog.ERROR, new String[] { "OK" }, 0).open();
			rengine = null;
			return;
		}
