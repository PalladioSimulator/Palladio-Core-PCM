...
/**Provides access to the single dataset of the SensorFramework.
 * Used to display all datasources in the SimuBench Perspective.
...
public class SensorFrameworkDataset {
	...
	public synchronized void addDataSource(IDAOFactory factory) {
		for(IDAOFactory f : datasources) {
			...
			if (f.getID() == nextID && factory.getID() == IDAOFactory.ID_NOT_SET)
	 ...
	 * @param factory the datasource itself to remove.
	 */
	...
	/** Reloads all known datasources.
	 *  Throws runtime exeception if some datasources are faulty.
	 */
