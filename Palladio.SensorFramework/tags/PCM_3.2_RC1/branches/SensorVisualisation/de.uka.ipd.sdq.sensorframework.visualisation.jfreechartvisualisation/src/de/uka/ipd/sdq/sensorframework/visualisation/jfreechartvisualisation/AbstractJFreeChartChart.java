...
class SaveImageAsAction extends Action {
	...
	public void run() {
		...
		if (dialog.getFileName() != null) {
			...
			try {
				ChartUtilities.saveChartAsPNG(f, chart,
						chartViewer.getBounds().width,
						chartViewer.getBounds().height);
			} catch (IOException e) {
	...
	public void run() {
		...
		if (dialog.getFileName() != null) {
			...
			try {
				...
				SVGGraphics2D svgRenderer = new SVGGraphics2D(doc);// Apache
																	// Batik is
																	// used here
...
public abstract class AbstractJFreeChartChart<T> extends ChartComposite {// SWT
																			// things...
