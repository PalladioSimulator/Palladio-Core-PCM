...
/**
 * Report for Histograms. Provides the basic possibilities to generate
 * histograms and change their classes width.
 * 
 * @author groenda
 */
public abstract class AbstractJFreeChartWidthReport extends AbstractReportView
	...
	protected abstract AbstractJFreeChartWidthViewer createViewer(
	...
	protected void generateVisualization(List<DataAdapter> newList) {
		if (newList == null || newList.isEmpty())
			throw new RuntimeException(
					"Histogram reports must have at least one data input.");
		...
		if (adapterList != null && adapterList.isEmpty() == false) {
			...
			Double test = (Double) adapterProperties
					.get(TimeSpanToHistogramAdapter.HISTOGRAM_WIDTH);
			newWidth = test;
		} else {
			// this is the first data added
			DataAdapter adapter = newList.get(0);
			adapterProperties = adapter.getProperties();
			newWidth = (Double) adapterProperties
					.get(TimeSpanToHistogramAdapter.HISTOGRAM_WIDTH);
		}
		...
		/*
		 * Set all histograms to the same histogram width and add histograms to
		 * the current view.
		 */
		...
		for (DataAdapter adapter : newList) {
			if (!allSameWidth) {
				...
				adapterProperties.put(
						TimeSpanToHistogramAdapter.HISTOGRAM_WIDTH, newWidth);
				// Note: The property change will notify all listeners, so be
				// careful
				adapter.setProperties(adapterProperties);
			}
	...
	private boolean isSameWidth(List<DataAdapter> adapterList) {
		...
		for (DataAdapter adapter : adapterList) {
			adapterProperties = adapter.getProperties();
			currentWidth = (Double) adapterProperties
					.get(TimeSpanToHistogramAdapter.HISTOGRAM_WIDTH);
