...
import de.uka.ipd.sdq.pcm.seff.RecoveryBlockAtion;
...
public class MarkovSeffVisitor extends SeffSwitch<MarkovChain> {
	 ...
	 *            transformation
	 */
	public MarkovSeffVisitor(final PCMInformationProvider pcmInfoProvider,
							final ContextWrapper wrapper, 
							final boolean optimize) {
		this.pcmInfoProvider = pcmInfoProvider;
	...
	public MarkovChain caseInternalAction(final InternalAction internalAction) {
		...
		List<FailureOccurrenceDescription> failureDescriptions=new ArrayList<FailureOccurrenceDescription>();
		...
		for (ParametricResourceDemand demand : internalAction
			...
			ProcessingResourceDescriptor descriptor = pcmInfoProvider.getDescriptor(demand, contextWrapper);
			if (descriptor == null) {
			...
			if (state.equals(ProcessingResourceState.NA)) {
				failureDescriptions.add(createOccurrenceDescriptionForResource(
						demand.getRequiredResource_ParametricResourceDemand()));
				break;
			}
		}

		//if a resource failure was added, the failure probability is set to 100%, so the other failures do not matter.
		if(failureDescriptions.isEmpty()) {
			//add all remaining failure probabilities
			failureDescriptions.addAll(internalAction.getFailureOccurrenceDescriptions());
		}
		// Build the corresponding Markov Chain:
		return markovBuilder.initInternalMarkovChain(internalAction
				.getEntityName(), failureDescriptions);
	}

	/**
	 * Creates a FalilureOccurrenceDescription for an environment failure. 
	 * That is, the FailureType is EnvironmentFailureType referencing the specified resource type
	 * and a failure probability of 1.0.
	 * 
	 * @param resourceType
	 *            the resource type to create the failure occurrence for
	 * @return a corresponding FailureOccurrenceDescription. Note that this FailureOccurrenceDescription
	 * is "virtual", i.e not part of the model.
	 */
	private FailureOccurrenceDescription createOccurrenceDescriptionForResource(ProcessingResourceType resourceType){
		FailureOccurrenceDescription failureOccurrence=SeffFactory.eINSTANCE.createFailureOccurrenceDescription();
		failureOccurrence.setFailureProbability(1.0);
		
		EnvironmentFailureType failureType= RepositoryFactory.eINSTANCE.createEnvironmentFailureType();
		//set the name to the resource name
		failureType.setEntityName(resourceType.getEntityName());
		failureType.setProcessingresourcetype(resourceType);
		failureOccurrence.setFailuretype(failureType);
	...
	/**
	 * Handles an ExternalCallAction. Generates a Markov Chain of the executing behavior.  
	 * The Markov Chain is generated by {@link MarkovSeffVisitor.createExternalCallActionChain}.
	 * If an retry count is set in the internal action, the chain is repeatedly 
	 * appended to the main chains failure state.
	 * 
	...
	public MarkovChain caseExternalCallAction(
		...
		if(externalCallAction==null)
			throw new IllegalArgumentException("Passed ExternalCallAction is null.");
		...
		if (seff == null) {
			...
			if(retryCount<0)
			{
				logger.info("Retry count is negative. Assuming 0.");
				retryCount=0;
			}
			
			//create a Markov Chain for one call
			MarkovChain chain = createExternalCallActionChain(externalCallAction);
			
			List<String> failureTypes=getFailureTypeNames(externalCallAction);
			
			MarkovChain handlingChain=markovBuilder.copyMarkovChain(chain);
			//if a retry count is specified, append the chain to itself as a failure handling chain at the failure state
			//this models the re-execution
			for (int i = 0; i < retryCount; ++i) {
				markovBuilder.appendFailureHandlingChain(chain, handlingChain, failureTypes, optimize);
			}
	...
	private List<String> getFailureTypeNames(FailureHandlingEntity entity) {
		List<String> failureTypes=new ArrayList<String>();
		for(FailureType failureType : entity.getFailuretype()) {
			if(failureType instanceof ApplicationFailureType) {
				failureTypes.add(failureType.getEntityName());
			}
			else if(failureType instanceof EnvironmentFailureType) {
				failureTypes.add(((EnvironmentFailureType)failureType).getProcessingresourcetype().getEntityName());
			}
			else
	 ...
	 * @return the resulting Markov Chain.
	 */
	public MarkovChain createExternalCallActionChain(
			final ExternalCallAction externalCallAction) {
		...
		MarkovChain innerMarkovChain = new MarkovSeffVisitor(this.pcmInfoProvider, newContextWrapper, 
				optimize).doSwitch(seff);
		...
		if (commLink != null) {
			...
			List<FailureOccurrenceDescription> occurrences=new ArrayList<FailureOccurrenceDescription>();
			FailureOccurrenceDescription description =createOccurrenceDescriptionForResource(commLink.getCommunicationLinkResourceType_CommunicationLinkResourceSpecification());
			description.setFailureProbability(commLink.getFailureProbability());
			occurrences.add(description);
			...
			MarkovChain messagingMarkovChain = markovBuilder
					.initInternalMarkovChain("Messaging", occurrences);
			MarkovChain returnMarkovChain = markovBuilder
					.initInternalMarkovChain("Returning", occurrences);
	...
	@Override
	public MarkovChain caseRecoveryBlockAction(final RecoveryBlockAction action) {
		// Do the logging:
		logger.debug("Visit RecoveryBlockAction [" + action.getEntityName() + "]");

		List<RecoveryBlockAlternativeBehaviour> behaviours=action.getRecoveryBlockalternativeBehaviours();

		
		if(behaviours.size()==0)
			throw new MarkovException("RecoveryBlockAction '"+action.getEntityName()+"' does not specify any behaviours.");
		
		RecoveryBlockAlternativeBehaviour previousAlternative=behaviours.get(0);
		MarkovChain theChain = doSwitch(previousAlternative);

		while(previousAlternative.getNextAlternative() !=null) {

			previousAlternative=previousAlternative.getNextAlternative();
			List<String> failureTypes=getFailureTypeNames(previousAlternative);
			
			MarkovChain handlingChain=doSwitch(previousAlternative);
			markovBuilder.appendFailureHandlingChain(theChain, handlingChain, failureTypes, optimize);
