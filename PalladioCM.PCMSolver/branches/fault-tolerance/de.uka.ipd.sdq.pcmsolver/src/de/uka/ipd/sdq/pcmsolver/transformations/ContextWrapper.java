...
//		String interfaceId = requiredInterface.getId();
	...
	private void readComputedContextsToHashMaps() {
		...
		for (ExternalCallInput eci : eciList) {
			...
			for (LinkingResource lr : lrList) {
				...
				boolean rc1Contained = exists(lr.getConnectedResourceContainers(), rc1);
				boolean rc2Contained = exists(lr.getConnectedResourceContainers(), rc2);
				if (rc1!=rc2 && rc1Contained && rc2Contained) {
					CommunicationLinkResourceSpecification clrs = lr
							.getCommunicationLinkResourceSpecifications();
	...
	private Collection<VariableUsage> copyComponentParameters(AssemblyContext context)
	{
		...
		List<VariableUsage> originalVariableUsages = component.getComponentParameterUsage_ImplementationComponentType();

		Collection<VariableUsage> newVariableUsages = VariableUsageHelper.copyVariableUsageList(originalVariableUsages);
	...
	private void addComponentParametersToNewContext(
		...
		//We need to make a copy because the VariableUsages are changed if they are overwritten by configuration parameters from the assembly context. 
		...
		//Remove variable characterization that are overwritten in the assembly context. Compare 
		//the variable usage pairwise by both name and then variable characterization.
		//Finally, add the config parameters to the component parameter list (compParams).
		for (VariableUsage confParVariableUsage : confParList) {
			...
			for (VariableUsage repoVariableUsage : compParams) {
				if (confParVariableUsage.getNamedReference_VariableUsage().getReferenceName().equals(
						repoVariableUsage.getNamedReference_VariableUsage().getReferenceName())){
					repoVariableUsageWithSameNameFound = true;
					//
					List<VariableCharacterisation> confParCharacterisations = confParVariableUsage.getVariableCharacterisation_VariableUsage();
					List<VariableCharacterisation> repoCharacterisations = repoVariableUsage.getVariableCharacterisation_VariableUsage();
					List<VariableCharacterisation> vcsToBeDeleted = new ArrayList<VariableCharacterisation>(repoCharacterisations.size());
					for (VariableCharacterisation repoVariableCharacterisation : repoCharacterisations) {
						for (VariableCharacterisation confParVariableCharacterisation : confParCharacterisations) {
							if (repoVariableCharacterisation.getType().equals(confParVariableCharacterisation.getType())){
					...
					//Add all variable characterizations of this confParVariableUsage instead					
					repoVariableUsage.getVariableCharacterisation_VariableUsage().addAll(VariableUsageHelper.copyVariableCharacterisations(confParCharacterisations));
				}
			}
			//If there is no variable with the name specified in the assembly context, add the whole VariableUsage from the AssemblyContext.
			if (!repoVariableUsageWithSameNameFound){
				compParams.add((VariableUsage)EcoreUtil.copy(confParVariableUsage));
			}
		}
