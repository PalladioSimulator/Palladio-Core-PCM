...
public class MarkovSeffVisitor extends SeffSwitch<MarkovChain> {
	...
	public MarkovChain caseInternalAction(final InternalAction internalAction) {
		...
		// OK:
		String failureProbability = ((Double) internalAction
				.getFailureProbability()).toString();
	...
	public MarkovChain caseExternalCallAction(
		...
		if (seff == null) {
			...
			for (int i = 0; i < retryCount; ++i) {
				markovBuilder
						.appendFailureHandlingChain(chain, handlingChain, optimize);
			}
	...
	public MarkovChain createExternalCallActionChain(
		...
		MarkovChain innerMarkovChain = new MarkovSeffVisitor(newContextWrapper,
				resourceDescriptors, optimize).doSwitch(seff);
		...
		if (commLink != null) {
			...
			MarkovChain messagingMarkovChain = markovBuilder
					.initInternalMarkovChain("Messaging", ((Double) commLink
							.getFailureProbability()).toString());
			MarkovChain returnMarkovChain = markovBuilder
					.initInternalMarkovChain("Returning", ((Double) commLink
							.getFailureProbability()).toString());
