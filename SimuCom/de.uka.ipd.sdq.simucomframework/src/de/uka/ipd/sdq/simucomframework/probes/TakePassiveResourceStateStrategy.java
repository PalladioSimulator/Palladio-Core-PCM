 ...
 * {@link SimulatedPassiveResource}. The state is calculated as follows:
...
public class TakePassiveResourceStateStrategy implements IProbeStrategy {
	...
	/**
	 * @param o
	 *            expects a {@link SimulatedPassiveResource}
	 */
	@Override
	public ProbeSample<Integer, Dimensionless> takeSample(String probeId,
			Object... o) {
		SimulatedPassiveResource r = null;
		if (o.length >= 1 && o[0] instanceof SimulatedPassiveResource) {
			r = (SimulatedPassiveResource) o[0];
		} else {
			throw new IllegalArgumentException("Expected an argument of type "
					+ SimulatedPassiveResource.class.getSimpleName() + ".");
		}
