...
public class ConfidenceStopCondition extends Condition implements Observer {
	...
	private IConditionStatus myConfidenceStopConditionStatus = new ConfidenceStopConditionStatus();
	...
	private void initialize() {
		...
		logger.info("Using Sensor: " + sensor.getSensorID() + "(ID) "
				+ sensor.getSensorName()+ " for confidence stop criterion.");
	...
	public IConditionStatus getStatus() {
		return myConfidenceStopConditionStatus ;
	}

}

class ConfidenceStopConditionStatus implements IConditionStatus{
	
	private String status = INDEPENDENCE_NOT_REACHED;
	
	private static final String LABEL = "Confidence Stop Criterion Status: ";
	private static final String INDEPENDENCE_NOT_REACHED = "Independence Test not passed: Value: ";
	

	public void independenceReached(){
		this.status = "indep passed"; 
	}
