...
public class SSJSimEvent extends Event implements ISimEventDelegate {
	...
	public SSJSimEvent(SimEvent myEvent, String name) {
		...
		super(((SSJExperiment) myEvent.getModel().getSimulationControl())
				.getSimulator());
	...
	public void actions() {
		...
		((SSJExperiment) myAbstractEvent.getModel().getSimulationControl())
				.checkStopConditions();

		if (who != null) {
			((SSJEntity) who).isScheduled = false;
			((SSJEntity) who).nextEventForThisEntity = null;
			myAbstractEvent.eventRoutine((SSJEntity) who);
		} else {
			myAbstractEvent.eventRoutine(null);
		}
	}
