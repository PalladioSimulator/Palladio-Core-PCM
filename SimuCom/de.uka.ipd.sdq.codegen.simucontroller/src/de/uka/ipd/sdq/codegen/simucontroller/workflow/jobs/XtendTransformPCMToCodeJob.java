...
implements IJob, IBlackboardInteractingJob<MDSDBlackboard> {
	...
	public void execute(IProgressMonitor monitor) throws JobFailedException,
		...
		final AllocationXpt allocationXpt = guiceInjector.getInstance(AllocationXpt.class);
...
//		private static final String REPOSITORY_ROOT_EXPAND_EXPRESSION = "m2t_transforms::repository::Root FOR pcmmodel";
		for (int repositoryIndex = 0; repositoryIndex < getRepositoryCount(); repositoryIndex++) {
			java.lang.System.out.println(getRepositoryTransformationSlots(repositoryIndex));
			...
			if (configuration.isAccuracyInfluenceAnalysisEnabled()) {
				workflow.addComponent(new IWorkflowComponent() {
					...
					@Override public void postInvoke() {}
					@Override
					public void invoke(IWorkflowContext ctx) {
						accuracyInfluenceExt.setQualityAnnotationRepository((QualityRepository) currentRepositorySlot.get(SLOT_NAME_QUALITY_ANNOTATION_MODEL));
...
//		private static final String SYSTEM_ROOT_EXPAND_EXPRESSION     = "m2t_transforms::system::Root FOR system";
		workflow.addComponent(new IWorkflowComponent() {
			@Override public void preInvoke() {}
			@Override public void postInvoke() {}
			@Override public void invoke(IWorkflowContext ctx) {
...
//		private static final String USAGE_ROOT_EXPAND_EXPRESSION 	  = "m2t_transforms::usage::UsageModel(allocation) FOR usage";
		workflow.addComponent(new IWorkflowComponent() {
			...
			@Override public void invoke(IWorkflowContext ctx) {
				usageXpt.usageModel(
						(UsageModel) systemTransformationSlots.get("usage"), 
...
//		// Now let them run
	...
	private XpandGeneratorJob getGeneratorJob(
		...
		java.lang.System.arraycopy(AbstractPCMWorkflowRunConfiguration.PCM_EPACKAGES, 0, pcmAndAccuracyPackages, 0, AbstractPCMWorkflowRunConfiguration.PCM_EPACKAGES.length);
