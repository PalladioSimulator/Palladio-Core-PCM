...
public class MarshallerComponentSeffBuilder extends
	...
	private void prepareClient() {
		...
		this.appendPreAction(new BytesizeAddingSetVariableActionDescriptor(
				createVariableUsage("stream", VariableCharacterisationType.BYTESIZE, "stream.BYTESIZE"),Modifier.IN));
	...
	private void prepareServer() {
		...
		this.appendPostAction(new BytesizeAddingSetVariableActionDescriptor(
				createVariableUsage("stream", VariableCharacterisationType.BYTESIZE, "stream.BYTESIZE"),Modifier.OUT));
	}
	...
	/**
	 * This method is responsible to determine the size of the stream that has to be sent over the network
	 * after marshalling (i.e. the marshall out data). 
	 ...
	 * This data is input to the middleware component configured in the models, and can be used by 
	 * the middleware component to calculate the size of the passed stream. 
	 * For example, in the default Glassfish component, ints.NUMBER_OF_ELEMENTS is then transformed to a BYTESIZE characterisation 
	 * with 2 bytes for each int (see default glassfish model).  
	 * 
	 * 
	 * @param currentSignature
	 * @param type
	 ...
	 * @return
	 */
	protected VariableUsage createInputParameterFor(OperationSignature currentSignature,
