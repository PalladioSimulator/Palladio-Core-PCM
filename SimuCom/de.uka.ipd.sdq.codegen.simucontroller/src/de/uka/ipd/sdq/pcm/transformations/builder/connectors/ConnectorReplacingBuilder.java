...
public class ConnectorReplacingBuilder implements IBuilder {
    ...
    public ConnectorReplacingBuilder(final PCMAndCompletionModelHolder models, final AssemblyConnector con,
    ...
    @Override
    public void build() {
        // Teste hier: Entweder sind es 2 Prozesse auf dem selben Rechner _oder_ zwei Prozesse auf
        // verschiedenen Rechner.
        // Im letzten Fall ist die Konfigurationsoption "SameAddressSpace" illegal und wird somit
        // uebergangen...
        if (FeatureUtils.hasFeature(featureConfig, "DifferentAddressSpace") || linkingRes != null) {
            ...
            final IClientServerConnectorCompletionComponentBuilder componentBuilder = configureCompletionComponentBuilder();
    ...
    private void connectConnectorCompletionWithMiddleware(
        ...
        assert (clientMWContext.getAssemblyContext_AllocationContext().getEncapsulatedComponent__AssemblyContext()
                .getProvidedRoles_InterfaceProvidingEntity().get(0) instanceof OperationProvidedRole);
    ...
    private void embeddConnectorCompletionInApplication(
            final IClientServerConnectorCompletionComponentBuilder componentBuilder) {
        addAssemblyConnector(connector.getRequiredRole_AssemblyConnector(),
                connector.getRequiringAssemblyContext_AssemblyConnector(), componentBuilder.getOperationProvidedRole(),
                componentBuilder.getAssemblyContext());
        addAssemblyConnector(componentBuilder.getOperationRequiredRole(), componentBuilder.getAssemblyContext(),
                connector.getProvidedRole_AssemblyConnector(),
                connector.getProvidingAssemblyContext_AssemblyConnector());
    }

    private void addAssemblyConnector(final OperationRequiredRole from, final AssemblyContext fromContext,
            final OperationProvidedRole to, final AssemblyContext toContext) {
    ...
    private AllocationContext findClientSideMiddlewareAllocationContext() {
        ...
        if (linkingRes != null) {
            if ((linkingRes.getConnectedResourceContainers_LinkingResource() != null)
                    && (linkingRes.getConnectedResourceContainers_LinkingResource().size() > 0)) {
    ...
    private AllocationContext findServerSideMiddlewareAllocationContext() {
        final ResourceContainer container = linkingRes == null ? findContainer(connector
                .getRequiringAssemblyContext_AssemblyConnector()) : linkingRes
                .getConnectedResourceContainers_LinkingResource().get(0);
        // private AllocationContext findServerSideMiddlewareAllocationContext() {
        // //Hauck: Changed code here because of changed metamodel. Please check.
        // ResourceContainer container = null;
        // if (linkingRes != null) {
     ...
     *            the interface to be searched for
     * @return the allocation context which has been found
     */
    private AllocationContext findAllocationContext(final ResourceContainer resourceContainer,
            final Interface interfaceToSearch) {
        ...
        throw new RuntimeException("Model invalid, unable to find middleware component for resource container "
    ...
    private LinkingResource findLinkingResource(final AssemblyConnector con) {
        ...
        for (final LinkingResource lr : models.getAllocation().getTargetResourceEnvironment_Allocation()
    ...
    private ResourceContainer findContainer(final AssemblyContext requiringAssemblyContext_AssemblyConnector) {
        for (final AllocationContext ac : this.models.getAllocation().getAllocationContexts_Allocation()) {
            if (ac.getAssemblyContext_AllocationContext().getId()
                    .equals(requiringAssemblyContext_AssemblyConnector.getId())) {
