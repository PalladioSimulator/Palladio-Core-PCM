...
public class SimuService implements ISimuService {
	...
	private SimuComWorkflowConfiguration createWorkflowConfig(
		...
		allocationFiles.add(params.getAllocationFile());
		...
		// Configure the setting for completions (no completions will be used)
			...
			completionProperties.put("de.uka.ipd.sdq.cip.completion", new HashMap<String,String>());
			completionProperties.put("de.uka.ipd.sdq.cip.revalidate", false);
			// Create configuration (using the properties) and set it for the workflow
			CompletionConfiguration completionConfig = new CompletionConfiguration(completionProperties);
