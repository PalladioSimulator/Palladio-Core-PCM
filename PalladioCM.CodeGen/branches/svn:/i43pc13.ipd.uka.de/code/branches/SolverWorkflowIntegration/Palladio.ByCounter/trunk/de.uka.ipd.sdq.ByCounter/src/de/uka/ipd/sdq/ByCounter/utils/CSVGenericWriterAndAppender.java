...
public class CSVGenericWriterAndAppender {
	...
	public CSVGenericWriterAndAppender() {
		super();
//		PropertyConfigurator.configure("log4j.properties");//TODO make systematic
...
//				);
	...
	public long writeColumns(//TODO make a non-typed version for simpler creating of columns by callers
		...
		if(usePrevTimestamp){
			timestamp = prevTimestampToUse; //TODO add plausibility checks for past times, etc. ...
		}else{
			//maintaining the prev. value --> debug
		}
		
		if(writeBooleansAsIntegers){
			highestNumberColumnType = 4;
		}else{
			highestNumberColumnType = 3;
		}
				...
				for(int columnIndex=0; columnIndex<columns.size(); columnIndex++){
					...
					if(      columnType==DOUBLE_COLUMN_TYPE){
						...
						if(something!=null){
							...
							log.error("TODO CSVWriterAndAppender cannot write null " +
...
//				sb.append("\n");
	...
	public long writeColumns_arr(//TODO make a non-typed version for simpler creating of columns by callers
		...
		if(writeBooleansAsIntegers){
			highestNumberColumnType = 4;//TODO rework this
		}else{
			highestNumberColumnType = 3;
		}
		
		if(columns==null || columns.length==0){
			log.error("No data (i.e., no columns)");
			return -1;
		}
