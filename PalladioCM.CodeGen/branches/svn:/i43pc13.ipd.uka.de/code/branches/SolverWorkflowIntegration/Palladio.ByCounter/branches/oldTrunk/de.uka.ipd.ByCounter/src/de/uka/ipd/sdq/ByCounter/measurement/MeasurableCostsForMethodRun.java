...
public class MeasurableCostsForMethodRun implements IMeasurable {
	 ...
	 * @param className Name of the class in which the method that is to be tested resides.
	 ...
	 */
	public MeasurableCostsForMethodRun(String className, MethodDescriptor method, int instrumentationMode) {
...
*/
		...
		try {
			if((instrumentationMode & CostMeasurement.INSTRUMENTATION_NONE) > 0) {
				classObj = ClassLoader.getSystemClassLoader().loadClass(className);
