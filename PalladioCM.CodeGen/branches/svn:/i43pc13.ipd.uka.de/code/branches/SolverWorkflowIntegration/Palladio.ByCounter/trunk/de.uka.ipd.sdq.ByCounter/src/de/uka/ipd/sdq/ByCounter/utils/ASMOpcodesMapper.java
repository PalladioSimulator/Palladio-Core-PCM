...
public class ASMOpcodesMapper implements Opcodes {
	...
	private ASMOpcodesMapper(boolean displayListAtStartup){
		...
		opcodesByName = new TreeMap<String,Integer>();
		...
		for (Field field: fields) {
			try {
				...
				codes.put(opcode_curr, opcodeName_curr);
				opcodesByName.put(opcodeName_curr, opcode_curr);
				if(this.displayList){
...
//					daoInstance.getOpcodeString(i));
		...
		Iterator<String> iterStr = opcodesByName.keySet().iterator();
		String key;
		System.out.println("Opcodes by name, sorted alphabetically"); 
		while(iterStr.hasNext()){
			key = iterStr.next();
			System.out.println(key+" : "+opcodesByName.get(key));
		}
	}
}
