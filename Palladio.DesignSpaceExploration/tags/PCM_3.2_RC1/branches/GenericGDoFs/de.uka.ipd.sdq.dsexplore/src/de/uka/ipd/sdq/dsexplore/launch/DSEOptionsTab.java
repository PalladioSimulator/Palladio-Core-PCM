...
public class DSEOptionsTab extends FileNamesInputTab {
	...
	private FileNamesInputTab modelsTab;
...
//		maxCost.addModifyListener(modifyListener);
		...
		this.createFileInputSection(container, modifyListener, "Start population or predefined candidates (optional), either .csv or designdecision containing Candidates", DSEConstantsContainer.CANDIDATE_EXTENSION, textGivenInstances);
		
		this.textCacheInstances = new Text(container, SWT.SINGLE | SWT.BORDER);
		this.createFileInputSection(container, modifyListener, "Candidates for evaluation cache (optional, .csv)", DSEConstantsContainer.CSV_EXTENSION, textCacheInstances);

		this.textAllInstances = new Text(container, SWT.SINGLE | SWT.BORDER);
		this.createFileInputSection(container, modifyListener, "All previous candidates (.csv), will be included in allcandidates*.csv, and cache (optional). Not considered by the evolutionary algorithm.", DSEConstantsContainer.CSV_EXTENSION, textAllInstances);

		this.textArchiveInstances = new Text(container, SWT.SINGLE | SWT.BORDER);
		this.createFileInputSection(container, modifyListener, "Archive candidates, will be included in the internal archive and archiveIndividuals.csv and cache (optional, .csv). Must be specified if an evolutionary search should be properly continued.", DSEConstantsContainer.CSV_EXTENSION, textArchiveInstances);
