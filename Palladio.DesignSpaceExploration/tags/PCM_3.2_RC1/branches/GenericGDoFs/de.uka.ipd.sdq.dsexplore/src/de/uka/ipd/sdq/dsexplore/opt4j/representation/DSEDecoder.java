...
import de.uka.ipd.sdq.dsexplore.designdecisions.GenomeToCandidateModelTransformation;
...
//			
//			applyChange(Opt4JStarter.getProblem().getDesignDecision(index), doubleGene);
//
//			index++;
//		}
//		
		...
		return new PCMPhenotype(pcm,genotypeString);
	}
	...
	private void applyChangeAllocationDecision(
		...
		logger.debug("Handling a "+designDecision.getClass()+", setting resource container to "+rc.getEntityName()+ " for allocation context "+designDecision.getPrimaryChanged().getEntityName());
	...
	public static String getDecisionString(Choice choice){
		...
		if (choice instanceof ContinousRangeChoice){
			...
			result = ((ClassChoice)choice).getEntity().getEntityName();
	...
	private static Entity getEntityFor(ClassDegree designDecision,
		...
		entities.addAll(designDecision.getDesignOptions());
