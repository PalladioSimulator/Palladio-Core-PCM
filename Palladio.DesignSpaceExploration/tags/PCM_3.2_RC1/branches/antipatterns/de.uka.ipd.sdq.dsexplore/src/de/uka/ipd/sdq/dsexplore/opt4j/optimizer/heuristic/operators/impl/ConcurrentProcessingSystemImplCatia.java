...
public class ConcurrentProcessingSystemImplCatia extends AbstractTactic {
	...
	private ExtensiveProcessing extensiveProcessing;

	private OneLaneBridge oneLaneBridge;

	private ConcurrentProcessingSystem concurrentProcessing;
	...
	public ConcurrentProcessingSystemImplCatia(Copy<Genotype> copy,
		...
		//FIXME: Quick adjustment to easily get Info which tactic has been applied. refactor and make the different antipatterns different classes.  
		this.extensiveProcessing = new ExtensiveProcessing(copy, individualBuilder, configuration);
		this.oneLaneBridge = new OneLaneBridge(copy, individualBuilder, configuration);
		this.concurrentProcessing = new ConcurrentProcessingSystem(copy, individualBuilder, configuration);

	}
	
	protected ConcurrentProcessingSystemImplCatia(Copy<Genotype> copy,
			DSEIndividualBuilder individualBuilder,
			DSEWorkflowConfiguration configuration, boolean pseudoArgument){
		super(copy, individualBuilder, configuration);
		//dummy constructor. 
	}
	...
	public List<TacticsResultCandidate> getSolution(DSEIndividual i) {
		...
		if (performance != null
			...
			List<PassiveResInfo> passiveResInfoList = new ArrayList<PassiveResInfo>(5);
			...
			for (UtilisationResult anyUtilResult : allUtilResults) {
				if (anyUtilResult instanceof ProcessingResourceSpecificationResult) {
					...
					double avgQueueLength = ((PassiveResourceResult)anyUtilResult).getAverageQueueLength();
					double avgWaitTime = ((PassiveResourceResult)anyUtilResult).getAverageWaitTime();
					double avgHoldingTime = ((PassiveResourceResult)anyUtilResult).getAverageHoldingTime();
					
					//@author catia: (1) queue length, (2) waiting and (3) holding time of passive resources are currently set to pre-defined values
					PassiveResource passiveResource = ((PassiveResourceResult)anyUtilResult).getPassiveResource_PassiveResourceResult();
					passiveResInfoList.add(new PassiveResInfo(passiveResource,
									passiveResource.getBasicComponent_PassiveResource().getEntityName(), 
									Integer.parseInt(passiveResource.getCapacity_PassiveResource().getSpecification()), 
									avgQueueLength, avgWaitTime, avgHoldingTime));
			...
			for (ServiceExecutionContext serviceExecutionContext : serviceContexts) {
				...
				List<AggregatedCommunication> communicationInfoList = serviceExecutionContext
						.getAggregatedCommunications_ServiceExecutionContext();
		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof EnumerationChoice) {
				...
				if (enumerationChoice.getDegreeOfFreedom() instanceof AllocationDegree) {
					...
					if (enumerationChoice.getDegreeOfFreedom()
							.getChangeableEntity() == result
							.getFirst().ac) {

						// if (enumerationChoice.getDegreeOfFreedom().g)
						
						Entity oldContainer = enumerationChoice.getEntity();

						enumerationChoice.setEntity(result
								.getSecond());

						// set weight to one for now, maybe later find a better value.
						candidate.setCandidateWeight(1);
						candidate.setHeuristic(this.concurrentProcessing);
						increaseCounterOfGeneratedCandidates();
						
						String info = "Applied CPS solution. Moved "+result.getFirst().ac.getEntityName()+" from "+oldContainer.getEntityName()+" to "+enumerationChoice.getEntity().getEntityName(); 
						logger.info(info+" New candiate: "+candidate.hashCode());
	...
	private TacticsResultCandidate createIncreasedCapacityCandidate(
		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof DiscreteRangeChoice) {
				...
				if (schedChoice.getDegreeOfFreedom() instanceof CapacityDegree) {

					if (schedChoice.getDegreeOfFreedom()
							.getChangeableEntity() == pr) {
						...
						// set weight to one for now, maybe later find a better value.
						candidate.setCandidateWeight(1);
						candidate.setHeuristic(this.oneLaneBridge);
						increaseCounterOfGeneratedCandidates();
						
						String info = "Increased capacity of "+pr.getEntityName()+" to "+newCapacity+".";
						logger.info(info+" Candidate: "+candidate.hashCode());
						candidate.setInfo(info);
						return candidate;
	...
	private TacticsResultCandidate createUpdatedSchedulingCandidate(
		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof SchedulingPolicyChoice) {
				SchedulingPolicyChoice enumerationChoice = (SchedulingPolicyChoice) choice;
				if (enumerationChoice.getDegreeOfFreedom() instanceof SchedulingPolicyDegree) {

					SchedulingPolicyDegree schedDegree = (SchedulingPolicyDegree)enumerationChoice.getDegreeOfFreedom();
					if (EMFHelper.checkIdentity(schedDegree.getChangeableEntity(),resourceToChange.rc)
							&& EMFHelper.checkIdentity(schedDegree.getProcessingresourcetype(),resourceToChange.type)) {

						// if (enumerationChoice.getDegreeOfFreedom().g)

						enumerationChoice.setChosenValue(schedulingPolicy);

						// set weight to one for now, maybe later find a better
						// value.
						candidate.setCandidateWeight(1);
						candidate.setHeuristic(this.extensiveProcessing);
						increaseCounterOfGeneratedCandidates();
						
						String info = "Changed scheduling policy of "+resourceToChange.type.getEntityName()+" of "
