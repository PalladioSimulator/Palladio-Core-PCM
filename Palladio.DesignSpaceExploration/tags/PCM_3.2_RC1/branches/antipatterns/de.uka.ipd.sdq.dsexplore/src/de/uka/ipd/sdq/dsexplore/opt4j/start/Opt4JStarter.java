...
public class Opt4JStarter {
	...
	public static void init(List<IAnalysis> evaluators, DSEWorkflowConfiguration dseConfig, PCMInstance pcmInstance, IProgressMonitor monitor, MDSDBlackboard blackboard) throws CoreException{
		...
		Nsga2Module nsgaModule = new Nsga2Module();
		...
		ev.init(evaluators, monitor,blackboard);
	 ...
	 * @throws CoreException
	 */
	public static void startOpt4J(List<IAnalysis> evaluators, DSEWorkflowConfiguration dseConfig, IProgressMonitor monitor, PCMInstance pcmInstance)
			throws CoreException {
		
		List<DesignDecisionGenotype> genotypes = Collections.emptyList();
		runOpt4JWithPopulation(dseConfig, monitor, genotypes, genotypes, genotypes);
...
//			ResultsWriter.printOutIndividuals(allIndividuals.getParetoOptimalIndividuals(), "Own Optimal Candidates");
			...
			ResultsWriter.writeStringToFile("config", config, listener.getIteration(), exceptions);
	...
	public static Choice createRandomGene(int index, DesignDecisionGenotype genotype) {
		if (Opt4JStarter.creator != null){
			return Opt4JStarter.creator.createRandomChoice(genotype.get(index).getDegreeOfFreedom());
		} else {
