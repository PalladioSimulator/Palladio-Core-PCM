...
public class TacticsTab extends FileNamesInputTab {
	 ...
	 * QML bounds Settings
	 */
	final static String QML_BOUND_GROUP_NAME = "QML Bounds";
	...
	public void createControl(Composite parent) {
		 ...
		 */
		{
			final Group qmlGroup = new Group(container, SWT.NONE);
			...
			qmlGroup.setLayout(qmlGroupLayout);
			qmlGroup.setText(QML_BOUND_GROUP_NAME);
			qmlGroup.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
					true, false));

			considerQMLBoundsButton = new Button(qmlGroup, SWT.CHECK);
...
//			considerQMLBounds.addSelectionListener(new SelectionAdapter() {
//				@Override
//				public void widgetSelected(SelectionEvent e) {
//					//...
//				}
...
//				false);
	...
	public boolean isValid(ILaunchConfiguration launchConfig) {
		...
		if (useProcessingRateButton.getSelection()) {
			...
			valuesToCheck.put(processingRateThresholdLowUtilisation, "Processing rate: Threshold for low utilisation");
			valuesToCheck
					.put(processingRateThresholdHighUtilisation, "Processing rate: Threshold for high utilisation");
		}
		if (useReallocationButton.getSelection()) {
			valuesToCheck.put(reallocationWeight, "Reallocation: Weight");
			valuesToCheck.put(reallocationThresholdUtilisationDifference, "Reallocation: Threshold for utlisation difference");
		}
		if (useServerConsolidationButton.getSelection()) {
			valuesToCheck.put(serverConsolidationWeight, "Server consolidation: Weight");
			valuesToCheck.put(serverConsolidationThresholdLowUtilisation,
					"Server consolidation: Threshold for low utilisation");
		}
		if (useServerExpansionButton.getSelection()) {
			valuesToCheck.put(serverExpansionWeight, "Server expansion: Weight");
			valuesToCheck.put(serverExpansionThresholdHighUtilisation,
					"Server expansion: Threshold for high utilisation");
		}
