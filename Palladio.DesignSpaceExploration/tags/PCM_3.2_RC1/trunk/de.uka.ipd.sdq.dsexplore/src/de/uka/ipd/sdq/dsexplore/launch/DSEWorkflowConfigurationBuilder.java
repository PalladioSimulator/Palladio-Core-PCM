...
//		addEvaluatorsIfSelected(evaluators, DSEConstantsContainer.COST, config);
		for(de.uka.ipd.sdq.dsexplore.qml.contracttype.QMLContractType.Dimension d : new AnalysisQualityAttributes().getAllDimensions()) {
	...
	private void addEvaluatorsIfSelected(ArrayList<IAnalysis> evaluators,
			de.uka.ipd.sdq.dsexplore.qml.contracttype.QMLContractType.Dimension dimension, DSEWorkflowConfiguration config) throws CoreException {
	    if (isAnalysisEnabledFor(dimension.getId())){
	    	String methodName = configuration.getAttribute(DSEConstantsContainer.getAnalysisMethod(dimension.getId()), "");
	    	AnalysisProxy analysis = new AnalysisProxy(this.configuration, dimension.getEntityName(), dimension.getId(), this.dseLaunch, methodName);
	    	evaluators.add(analysis);
	    }
		
	}

	private boolean isAnalysisEnabledFor(String qualityId) throws CoreException {
		return !this.configuration.getAttribute(DSEConstantsContainer.getAnalysisMethod(qualityId), "").equals(DSEConstantsContainer.NONE);
	}
