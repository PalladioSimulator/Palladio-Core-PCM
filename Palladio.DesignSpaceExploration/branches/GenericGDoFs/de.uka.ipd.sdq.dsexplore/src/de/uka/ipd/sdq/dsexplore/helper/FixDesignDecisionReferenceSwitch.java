 ...
 * Solution: Just use the same EMF resource set when loading more models and then resolve proxies using ECoreUtil#resolveAll.
 * @author martens
 *
 * @deprecated
 */
public class FixDesignDecisionReferenceSwitch extends designdecisionSwitch<EObject> {
	...
	public void fixEntitiesForDomain(ClassDegree degree, List<Entity> memoryEntityList) {
		...
		for (Entity entity : degree.getDesignOptions()) {
			Entity rightOne = EMFHelper.retrieveEntityByID(memoryEntityList, entity.getId());
			newList.add(rightOne);
		}

		degree.getDesignOptions().clear();
		degree.getDesignOptions().addAll(newList);
	}
	...
	public EObject caseAllocationDegree(AllocationDegree object) {
		String id = object.getPrimaryChanged().getId();
	...
	public EObject caseClassChoice(ClassChoice object) {
		...
		if (degree instanceof ClassDegree){
			...
			Entity inMemoryEntity = EMFHelper.retrieveEntityByID(enumDegree.getDesignOptions(), object.getEntity().getId());
