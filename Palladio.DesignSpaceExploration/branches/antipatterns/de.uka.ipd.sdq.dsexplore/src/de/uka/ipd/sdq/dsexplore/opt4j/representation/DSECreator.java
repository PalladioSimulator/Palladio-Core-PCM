...
public class DSECreator implements Creator<DesignDecisionGenotype> {
	...
	public DesignDecisionGenotype create() {
		...
		List<DegreeOfFreedom> degrees = problem.getDesignDecisions();
		for (DegreeOfFreedom degreeOfFreedom : degrees) {
			genotype.add(createRandomChoice(degreeOfFreedom));
		}
	...
	public Choice createRandomChoice(DegreeOfFreedom degree) {
		...
		if (degree instanceof DiscreteRangeDegree){
			...
			choice = contChoice;
		} else if (degree instanceof EnumerationDegree){
			EnumerationChoice enumChoice = factory.createEnumerationChoice();
			enumChoice.setEntity(createRandomEntity((EnumerationDegree)degree));
			choice = enumChoice;
	...
	private Entity createRandomEntity(EnumerationDegree enumDegree) {
		List<Entity> domain = enumDegree.getDomainOfEntities();
		int index = this.random.nextInt(domain.size()); 
		return domain.get(index);
