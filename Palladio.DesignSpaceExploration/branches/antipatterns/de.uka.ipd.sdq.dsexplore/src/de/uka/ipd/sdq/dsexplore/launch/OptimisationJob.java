...
public class OptimisationJob implements IJob, IBlackboardInteractingJob<MDSDBlackboard> {
	...
	public void execute(IProgressMonitor monitor) throws JobFailedException,
		...
		pcmInstance = new PCMInstance((PCMResourceSetPartition)this.blackboard.getPartition(LoadPCMModelsIntoBlackboardJob.PCM_MODELS_PARTITION_ID));
	    ...
	    if (this.dseConfig.isOptimise()){
	    	if (!dseConfig.hasPredefinedInstances()){
	    		...
	    		List<DesignDecisionGenotype> genotypes = GenotypeReader.getGenotypes(populationFilename);
	    		...
	    		// TODO Atanas: Create the configuration for the stop criteria here
	    		// The Opt4J objectives are currently only initialised after calling Opt4J.init(..)
	    		Collection<Objective> usedObjectives = Opt4JStarter.getDSEEvaluator().getObjectives();
	    		// You need to iterate over all Opt4J usedObjectives here and choose 
	    		// the ones whose name equals the dimension id.
	    		// Ignore all dimensions that are not in the list of obectives. 
	    		// Later, the objectives might be inititialised earlier and could be already set 
