...
public abstract class AbstractStartingPopulationHeuristic implements IStartingPoulationHeuristic {
	...
	protected ArrayList<ResourceContainer> getResourceContainers() {
		if (resourceContainers != null) {
			...
			Collection<DegreeOfFreedom> degreesOfFreedom = Opt4JStarter.getProblem().getDesignDecisions();
			for (DegreeOfFreedom degreeOfFreedom : degreesOfFreedom) {
				if (degreeOfFreedom instanceof AllocationDegree) {
					AllocationDegree allocationDegree = (AllocationDegree) degreeOfFreedom;
	...
	protected ArrayList<AllocationContext> getAllocationContexts() {
		if (allocationContexts != null) {
			...
			for (DegreeOfFreedom degreeOfFreedom : degreesOfFreedom) {
				if (degreeOfFreedom instanceof AllocationDegree) {
					AllocationDegree allocationDegree = (AllocationDegree) degreeOfFreedom;
					Entity entity = allocationDegree.getChangeableEntity();
