...
public class ReliabilityAnalysis implements IAnalysis {
	...
	private IAnalysisResult retrieveReliabilitySolverResults(PCMPhenotype pheno)  {
		...
		ReliabilityAnalysisResult result = new ReliabilityAnalysisResult(this.previousPofodResults.get(pheno.getNumericID()));
	...
	private void launchReliabilitySolver(PCMPhenotype pheno, IProgressMonitor monitor)
		...
		builder = new PCMSolverReliabilityConfigurationBasedConfigBuilder(this.config, ILaunchManager.RUN_MODE);
		builder.fillConfiguration(solverConfiguration);
	...
	private void initialiseCriteria(ILaunchConfiguration configuration) throws CoreException{
		...
		List<EvaluationAspectWithContext> reliabilityAspects = reader.getDimensionConstraintContextsForUsageModel(usageModel, reliabilityQualityAttribute.getDimension().getId());
		reliabilityAspects.addAll(reader.getDimensionObjectiveContextsForUsageModel(usageModel, reliabilityQualityAttribute.getDimension().getId()));
	...
	private boolean canEvaluateAspect(EvaluationAspect aspect){
		return reliabilityQualityAttribute.canEvaluateAspect(aspect);
	}
...
//	}
	...
	public String getQualityAttribute() throws CoreException {
		...
		return reliabilityQualityAttribute.getDimension().getEntityName();
	}
