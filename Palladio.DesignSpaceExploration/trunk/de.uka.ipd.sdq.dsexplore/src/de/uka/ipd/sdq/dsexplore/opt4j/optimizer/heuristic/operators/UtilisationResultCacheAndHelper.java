...
public class UtilisationResultCacheAndHelper {
		...
		for (Choice choice : genotype) {
			if (choice instanceof EnumerationChoice) {
				...
				if (enumerationChoice.getDegreeOfFreedom() instanceof AllocationDegree) {
					AllocationDegree allocationDegree = (AllocationDegree) enumerationChoice.getDegreeOfFreedom();
					for (Entity entity : allocationDegree.getDomainOfEntities()) {
	...
	private Collection<ResourceContainer> determineUnusedAvailableResourceContainers(DSEIndividual individual) {
		...
		for (Choice choice : genotype) {
			if (choice instanceof EnumerationChoice) {
				EnumerationChoice enumerationChoice = (EnumerationChoice) choice;
				unusedResourceContainers.remove(enumerationChoice.getEntity());
			}
		}
	...
	public LinkingResourceResults getMaxLinkUtilisationResult(DSEIndividual individual){
		if (maxLinkResult == null){
			EList<UtilisationResult> utilisationResults = getUtilisationResult(individual);
			LinkingResourceResults minUtilisationResult = null;
			if (utilisationResults != null) {
				for (UtilisationResult currentUtilisationResult : utilisationResults) {
					if (currentUtilisationResult instanceof LinkingResourceResults){
						...
						if (linkUtilisationResult.getResourceUtilisation() > 0){
							if (minUtilisationResult == null 
									|| minUtilisationResult.getResourceUtilisation() > linkUtilisationResult.getResourceUtilisation()) {
								minUtilisationResult = linkUtilisationResult;
							}
						}
					}
				}
				maxLinkResult = minUtilisationResult;
			}
		}
	...
	public Set<ResourceType> getResourceTypes(DSEIndividual individual) {
		...
		if (this.resourceTypes == null){
			...
			Problem problem = individual.getProblem();
			for (DegreeOfFreedom dof : problem.getDesigndecision()) {
				if (dof instanceof AllocationDegree){
					List<Entity> entities = ((AllocationDegree) dof).getDomainOfEntities();
