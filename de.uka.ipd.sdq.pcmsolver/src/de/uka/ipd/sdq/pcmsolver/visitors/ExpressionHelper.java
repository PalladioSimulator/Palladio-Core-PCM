...
public class ExpressionHelper {
	...
	public static Expression getSolvedExpressionWithoutPMFs(String specification, ContextWrapper contextWrapper) {
		specification = ExpressionHelper.getSolvedExpressionAsString(specification, contextWrapper);
		
		//TODO: support PMFs (also in InternalActionHandler itself)
		// quick fix: (convert pmfs to pdfs)
		specification = specification.replaceAll("IntPMF", "DoublePDF");
		specification = specification.replaceAll("DoublePMF", "DoublePDF");
		
		Expression solvedExpr = (Expression) ExpressionHelper
			.getSolvedExpression(specification, contextWrapper);
		return solvedExpr;
	}
