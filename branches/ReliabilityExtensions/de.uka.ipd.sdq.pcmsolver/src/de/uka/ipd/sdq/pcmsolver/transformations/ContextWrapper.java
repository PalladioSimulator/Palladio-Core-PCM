...
public class ContextWrapper implements Cloneable{
	...
	private ComputedUsageContext getFirstComputedUsageContext(
		...
		if (cuc!=null){
			logger.debug("Reusing existing computed usage context for "+assCtx.getEntityName());
			return cuc;
		} else {
			logger.debug("Creating new computed usage context for "+assCtx.getEntityName());
...
//			}
	...
	private ComputedAllocationContext getNextComputedAllocationContext() {
		...
		if (cac!=null){
			logger.debug("Reusing existing computed allocation context for "+assCtx.getEntityName());
			return cac;
		} else {
			logger.debug("Creating new computed allocation context for "+assCtx.getEntityName());
