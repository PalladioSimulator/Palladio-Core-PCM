...
public class AllocationCanonicalEditPolicy extends
	...
	protected List getSemanticChildrenList() {
		...
		Allocation allocation = (Allocation) ((View) getHost().getModel())
				.getElement();
		ResourceEnvironment env = allocation
				.getTargetResourceEnvironment_Allocation();
		View viewObject = (View) getHost().getModel();
		EObject nextValue;
		int nodeVID;
		for (Iterator values = env.getResourceContainer_ResourceEnvironment()
				.iterator(); values.hasNext();) {
			nextValue = (EObject) values.next();
			nodeVID = PcmVisualIDRegistry
					.getNodeVisualID(viewObject, nextValue);
			if (ResourceContainerEditPart.VISUAL_ID == nodeVID) {
	...
	private Collection refreshPhantoms() {
		...
		for (Iterator it = resource.getContents().iterator(); it.hasNext();) {
			...
			int nodeVID = PcmVisualIDRegistry.getNodeVisualID(diagram,
					nextResourceObject);
			switch (nodeVID) {
