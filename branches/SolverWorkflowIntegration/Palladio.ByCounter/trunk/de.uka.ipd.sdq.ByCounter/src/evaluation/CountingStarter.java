...
import evaluation.SciMark.MonteCarlo;
import evaluation.SciMark.Random;
/*import evaluation.spec.benchmarks.scimark.lu.LU;
import evaluation.spec.harness.Launch;
import evaluation.spec.benchmarks.scimark.lu.LU;
import evaluation.spec.benchmarks.scimark.monte_carlo.MonteCarlo;
import evaluation.spec.benchmarks.scimark.utils.Random;
import evaluation.spec.harness.Launch;
...
//		log.debug("Method descriptor: "+methDesc);
		...
		CountingResult[] finalResults = resultColl.getAllCountingResultsAsArray(false);
		log.info(finalResults.length+" counting results found, logging them: ");
		for(CountingResult r : finalResults) {
			resultColl.logResult(r, true, true); //from Martin
		}
