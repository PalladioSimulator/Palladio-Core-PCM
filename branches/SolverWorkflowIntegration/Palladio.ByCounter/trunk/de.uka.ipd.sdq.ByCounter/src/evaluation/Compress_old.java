 ...
 *
 *   @(#)Compress_old.java	1.7 06/17/98
...
//package evaluation.spec.benchmarks._201_compress;
...
final class Code_Table {
	...
	public Code_Table() {
		tab = new short[Compress_old.HSIZE];
	}
...
class Comp_Base {
	...
	public Comp_Base(Input_Buffer in, Output_Buffer out) {
		Input = in;
		Output = out;
		maxbits = Compress_old.BITS;
		block_compress = Compress_old.BLOCK_MASK;
		buf = new byte[Compress_old.BITS];
	}
...
final class Compressor extends Comp_Base {
	...
	public Compressor(Input_Buffer in, Output_Buffer out) {
		super(in, out);
		if (maxbits < Compress_old.INIT_BITS)
			maxbits = Compress_old.INIT_BITS;
		if (maxbits > Compress_old.BITS)
			maxbits = Compress_old.BITS;
		maxmaxcode = 1 << maxbits;
		n_bits = Compress_old.INIT_BITS;
		maxcode = MAXCODE();
		...
		free_ent = ((block_compress != 0) ? Compress_old.FIRST : 256);

		htab = new Hash_Table(); // dm/kmd 4/10/98
		codetab = new Code_Table();

		Output.putbyte(Compress_old.magic_header[0]);
		Output.putbyte(Compress_old.magic_header[1]);
	...
	private void cl_block() {
		...
		if (rat > ratio) {
			...
			ratio = 0;
			htab.clear();
			free_ent = Compress_old.FIRST;
			clear_flg = 1;
			output((int) Compress_old.CLEAR);
		}
	}
	...
	private void output(int code) {
		...
		if (code >= 0) {
			 ...
			 * on the left.
			 */
			buf[bp] = (byte) ((buf[bp] & Compress_old.rmask[r_off]) | (code << r_off)
					& Compress_old.lmask[r_off]);
			bp++;
			bits -= (8 - r_off);
			code >>= 8 - r_off;
...
final class Decompressor extends Comp_Base {
	 ...
	 */
		...
		public De_Stack() {
			tab = new byte[Compress_old.STACK_SZ];
			index = 0;
		}
	 ...
	 */
		...
		public Suffix_Table() {
			tab = new byte[Compress_old.SUFFIX_TAB_SZ];
		}
	...
	public Decompressor(Input_Buffer in, Output_Buffer out) {
		...
		if (((Input.getbyte() & 0xFF) != (Compress_old.magic_header[0] & 0xFF))
				|| ((Input.getbyte() & 0xFF) != (Compress_old.magic_header[1] & 0xFF))) {
			System.err.println("stdin: not in compressed format");
			// System.exit(1);
		}

		maxbits = Input.getbyte(); /* set -b from file */
		block_compress = maxbits & Compress_old.BLOCK_MASK;
		maxbits &= Compress_old.BIT_MASK;
		maxmaxcode = 1 << maxbits;
		if (maxbits > Compress_old.BITS) {
			System.err.println("stdin: compressed with " + maxbits
					+ " bits, can only handle " + Compress_old.BITS + " bits");
			// System.exit(1);
		}
		n_bits = Compress_old.INIT_BITS;
		maxcode = MAXCODE();
	...
	public void decompress() {
		...
		while ((code = getcode()) > -1) {
			...
			if ((code == Compress_old.CLEAR) && (block_compress != 0)) {
				tab_prefix.clear(256);
				clear_flg = 1;
				free_ent = Compress_old.FIRST - 1;
	...
	private int getcode() {
		...
		code = ((buf[bp++] >> r_off) & Compress_old.rmask[8 - r_off]) & 0xff;
		bits -= (8 - r_off);
		...
		/* high order bits. */
		// code |= (buf[bp] & Compress_old.rmask[bits]) << r_off; // kmd
		// Don McCauley/kmd - IBM 02/26/98
		if (bits > 0)
			code |= (buf[bp] & Compress_old.rmask[bits]) << r_off;
		offset += n_bits;
