...
//	 * TODO Merge into central interface (interface means "entry point to ByCounter").
//	 * @param args
//	 */
...
//			}
//			if(ins!=null){
//				System.out.println("Testing success: "+ins.testPathConversion());
//			}else{
...
//	}
	 ...
	 */
	private Instrumenter(InstrumentationParameters parameters) {
		...
		if(instrumentationParameters.getMethodsToInstrument()==null){
			...
			if(nr==0){
				...
				log.info("Instrumenter has "+nr+" methods to instrument");
			}
		}
	}
	...
	/**
	 * @return measurements TODO todoc
	 */
	...
	public synchronized void instrument() {
		log.debug("Instrumentation starting...");
		this.timestampBeforeReaderAccept_2 = System.nanoTime();
		this.classReader.accept(this.classAdapter, ClassReader.EXPAND_FRAMES);//TODO why take expand frames despite performance degradation?
		log.debug(">>>Verifying");
		...
		log.debug("... instrumentation finished.");
	...
	private void setupASM() {
		...
		for(MethodDescriptor methodDesc 
				: instrumentationParameters.getMethodsToInstrument()) {
			log.info("Instrumenting the method '" + methodDesc.getMethodName()
		...
		// Remember: the order of the visitor construction is inverse to their 
		// execution order.
		
		// Debug output (for bytecode output remove comments: //TODO introduce a "verbose" flag...
	    ...
	    if(instrumentationParameters.getCountStatically()){//TODO what has static to do with "section"? what if a section has a loop?
	 ...
	 * @return
	 */
	private boolean testPathConversion() {
