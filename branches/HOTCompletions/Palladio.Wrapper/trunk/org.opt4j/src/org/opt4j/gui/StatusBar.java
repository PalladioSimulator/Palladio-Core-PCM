...
/**
 * The {@code StatusBar} contains informations about the optimization progress
 * and time per iteration.
 * 
 * @author lukasiewycz
 * 
 */
public class StatusBar implements OptimizerIterationListener,
	...
	public void init() {
		...
		Border border = BorderFactory.createMatteBorder(1, 0, 0, 0, label
	...
	public void iterationComplete(Optimizer optimizer, int iteration) {
		final long diff = Math
				.round(((double) (System.nanoTime() - time)) / 1000000.0);
		time = System.nanoTime();

		DecimalFormat format = new DecimalFormat();

		update(format.format(diff), "" + progress.getCurrentIteration()
