...
package deprecated.spec.benchmarks.crypto.signverify;
...
import deprecated.spec.benchmarks.crypto.Util;
...
public class Main extends SpecJVMBenchmarkBase {
    ...
    public void runSignVerify(byte[] indata, String algorithm, PrivateKey privKey, PublicKey pubKey) {
        ...
        if (verification) {
            deprecated.spec.harness.Context.getOut().println(algorithm + " Verification PASS");
        } else {
            deprecated.spec.harness.Context.getOut().println(algorithm + " Verification FAILED.");
        }
