...
public class CSVResultWriter implements IResultWriter {
	...
	public void writeResult(CountingResult cr) {
		...
		Integer 	currentOpcodeCount = 0;	// opcode count
		Integer 	currentMethodCount = 0;	// method count
...
//				
		...
		for (int i=0; i<numberOfValueColumns; i++){
			columnTypes.add(new Integer(CSVWriterAndAppender.INTEGER_COLUMN_TYPE));
		}
		for (int i=0; i<numberOfValueColumns; i++){
			columns.add(new ArrayList<Object>());
		}
		...
		Integer currentOpcodeCountIntegerObject;
		for(int i=0; i<listOpcodes.size(); i++) {
			...
			currentOpcodeCountIntegerObject = opcodeCounts.get(currentOpcode);
			if(currentOpcodeCountIntegerObject!=null){
				currentOpcodeCount = currentOpcodeCountIntegerObject.intValue();
			}else{
				currentOpcodeCount = 0;
			}
		...
		if(performIntegrityCheckOnInvokeOpcodes){
			Integer counts182 = cr.getOpcodeCounts().get(new Integer(182));
			Integer counts183 = cr.getOpcodeCounts().get(new Integer(183));
			Integer counts184 = cr.getOpcodeCounts().get(new Integer(184));
			Integer counts185 = cr.getOpcodeCounts().get(new Integer(185));
