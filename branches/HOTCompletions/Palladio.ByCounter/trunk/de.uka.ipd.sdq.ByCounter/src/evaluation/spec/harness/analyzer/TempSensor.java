...
package evaluation.spec.harness.analyzer;
...
import evaluation.spec.harness.StopBenchmarkException;
import evaluation.spec.harness.Util;

public class TempSensor {

    // private final static boolean verbose =
    // Boolean.parseBoolean(System.getProperty("spec.harness.analyzer.PowerMeter.verbose",
    // "false"));
    private final static boolean verbose = Util.getBoolProperty(Constants.ANALYZER_SENSOR_VERBOSE, null);
    static String[] AnalyzerCommands = { "Temp" };
    ...
    public TempSensor(String host, int port) {

        if (verbose) {
            Context.getOut().println("\nChecking for temperature ptd connection...\n");
        }

        try {
            meterSocket = new Socket(host, port);
        } catch (UnknownHostException unknown) {
            throw new StopBenchmarkException("Could not connect to temperature ptd.", unknown);
        } catch (IOException er) {
            throw new StopBenchmarkException("Error connecting to temperature ptd.", er);
        }
        ...
        try {
            ...
            throw new StopBenchmarkException("Error connecting to ptd.", er);
        }
        ...
        try {
            ...
            if (meterResponseParts.length == 12) {
                ...
                if (verbose) {
                    Context.getOut().println("Temperature sensor type: " + meterResponseParts[0]);
                    Context.getOut().println("Temperature sensor SPEC compliant: " + meterResponseParts[8]);
            ...
            } else { // error in meter response
                Context.getOut().println("Warning! Unexpected input from temperature sensor: " + meterResponse);
    ...
    public void endSocConnection() {
        try {
            ...
            throw new StopBenchmarkException("Error closing the temperature sensor connection", e);
        }
    }
    ...
    public HashMap<String, String> reportRun(String propPrefix) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            if (msg_parts.length == 7) {
                ...
                Context.getOut().println("Warning! Unexpected input from temperature sensor: " + msg);
                ...
                // put 1 sample and 1 error (rather than 0 and 0) so we don't
                // get divide by 0 exception
    ...
    public Boolean sensorCompliant() {
        return (identifyParts[8].equals("1"));
    }

    public String sensorPtdVersion() {
