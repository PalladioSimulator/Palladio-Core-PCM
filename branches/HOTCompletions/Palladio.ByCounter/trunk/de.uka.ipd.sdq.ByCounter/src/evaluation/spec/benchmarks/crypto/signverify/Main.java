...
package evaluation.spec.benchmarks.crypto.signverify;
...
import evaluation.spec.benchmarks.crypto.Util;
...
public class Main extends SpecJVMBenchmarkBase {
    ...
    public void runSignVerify(byte[] indata, String algorithm, PrivateKey privKey, PublicKey pubKey) {
        ...
        if (verification) {
            evaluation.spec.harness.Context.getOut().println(algorithm + " Verification PASS");
        } else {
            evaluation.spec.harness.Context.getOut().println(algorithm + " Verification FAILED.");
        }
