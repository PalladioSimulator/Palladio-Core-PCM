		...
		if(instrumentationParameters.getMethodsToInstrument()==null){
			...
			for(int i = 0; i < instrumentationParameters.getMethodsToInstrument().size(); i++) {
				if(instrumentationParameters.getMethodsToInstrument().get(i).
						getCanonicalClassName().equals(
								this.getClassCanonicalName().replace('/', '.'))) {
					nr++;
				}
			}
			if(nr==0){
	...
	public synchronized boolean instrument() {
		...
		for(int i = 0; i < mcca.methodInstrumentationStatus.length; i++) {
			...
			if(!canonicalClassName.equals(thisCanonicalClassName)) {
				...
				log.debug("Skipping method "+methDesc.getMethodName()+": "+
						"class name "+canonicalClassName+
						" not identical with "+thisCanonicalClassName+
						" (latter is \"this\" class to instrument)");
				continue;
			}
			if(mcca.methodInstrumentationStatus[i] == false) {
				log.error("The method with the following MethodDiscriptor "
						+ "could not be instrumented.");
				log.error(instrumentationParameters.getMethodsToInstrument().get(i));
				retValue = false;
			}
		}
