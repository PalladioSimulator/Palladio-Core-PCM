...
public class MethodInvocationHelper {
	...
	public static void callMethods(
		...
		try {
			...
			if(params == null) {
				...
				log.warn("Warning: callMethods called with less " +
			...
			for(int i = 0; i < methodsToCall.size(); i++) {
				...
				if(methodsToCall.get(i).getIsConstructor()) {
					Constructor<?>[] constructors = 
						parentObject.getClass().getDeclaredConstructors();
	...
	//						log.debug(">>> Invoking method '" + buildMethodSignature(m) + "'...");
							...
							try {
								...
								log.error("Invocation error while trying to execute method.");
								ei.printStackTrace();
								if(ei.getCause().getClass() == StackOverflowError.class) {
									log.error("Stack overflow on method execution. " 
