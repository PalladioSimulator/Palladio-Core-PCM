...
public class MethodHasMinNumberOfFormalParameters extends MethodConstraint implements IntegerConstraint {

    public MethodHasMinNumberOfFormalParameters(String check_object_identifier, int number) {
    ...
    public boolean evaluateInteger(Object check_object, int number) {
        if (check_object instanceof de.fzi.sissy.metamod.Method)
            return MetamodRetrievalEngineImplementation.getSingleton().methodHasMinNumberOfFormalParameters((de.fzi.sissy.metamod.Method)check_object, number);
        else
            return false;
    }
}
