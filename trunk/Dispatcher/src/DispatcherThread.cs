...
namespace Dispatcher
{
	...
	public class DispatcherThread
	{
		...
		public DispatcherThread(TcpClient aSocket, DateTime now, ref RequestParserComponent parser, ref RequestProcessorComponent prozzesor)
		{
		...
		public void StartListen()
		{
			try
			{
				this.parsesRequest.HandleRequest(this.request, this.clientStream, this.arrivalTime, ref this.sendsResponse, this.clientSocket);
			}
			catch (Exception e)
			{
...
//					Console.Write(String.Format("{0}", data));
			...
			msg = Encoding.ASCII.GetBytes(completeMessage);
