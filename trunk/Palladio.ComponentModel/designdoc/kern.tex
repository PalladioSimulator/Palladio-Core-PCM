...
Zusammengesetzten Komponenten (\emph{composite components})können andere Komponenten enthalten. Diese können jedoch möglicherweise ebenfalls Bestandteil anderer Komponenten sein können. Die so entstandene \emph{n zu m}-Beziehung der Komponenten untereinander wurde wie in \cite{lit:db} vorgeschlagen unter Verwendung einer zusätzlichen Tabelle realisiert. Diese Tabelle enthält auschließlich Fremdschlüsselbeziehungen zu den beiden Teilnehmern der Beziehung. Im Falle des Komponentenmodells handelt es sich um die Tabelle \verb+CompRelations+ mit den Fremdschlüsselbeziehungen \verb+fk_parent+ und \verb+fk_child+ zu der Tabelle \verb+Components+. Diese Tabelle erhält abweichend von \cite{lit:db} einen zusätzlichen Primärschlüssel vom Typ \emph{long}. Dieser dient der Modellierung von Beziehungen, welche bei der Beschreibung der Konnektor-Tabellen erläutert werden. Die Kombination der beiden Fremdschlüssel bildet einen Sekundärschlüssel der Tabelle, die Beziehung zwischen zwei Komponenten nur einmalig möglich sein darf. 
...
Protokolle sind ebenso wie Signaturen Teil einer Schnittstelle (Interface). Die Schnittstelle kann auch hier mehrere Protokolle besitzen \footnote{Das theoretische Modell sieht prinzipiell nur ein Protokoll für jede Schnittstelle vor. Diese muss jedoch in unterschiedlicher Form (z.B. als Finite State Machines und Petri-Netz) möglicherweise reduntant abspeicherbar sein.}, jedoch gehört auch hier wieder ein Protokoll zu genau einer Schnittstelle. Realisiert wurde diese Beziehung auch hier unter Verwendung des Fremdschlüssels \verb+fk_iface+.
	
\item \textbf{Roles}\\
Diese Tabelle modelliert die \emph{n-zu-m}-Beziehung zwischen Komponenten und Interfaces. Ein Interface kann Bestandteil mehreren Komponenten sein. Eine Komponente kann ebenfalls mehre Interfaces besitzen. Es existieren zwei Arten von Beziehungen zwischen Komponenten und Interfaces. Im ersten Fall werden die im Interfaces definierten Dienste von der Komponente angeboten (\emph{ProvidesInterface}), im anderen Fall werden sie benötigt(\emph{RequiresInterface}). Die Spezialisierung wird ebenso wie bei den Komponenten durch das Typ-Attribut \verb+type+ aufgelöst. Die Kombination der beiden Fremdschlüssel \verb+fk_iface+ und \verb+fk_comp+ mit dem Typ der Beziehung bildet einen Sekundärschlüssel der Tabelle. Primärschlüssel bildet aus Gründen der besseren Modellierbarkeit der Beziehungen zu den Konnektoren eine eigene ID vom Typ \emph{long}.

\item \textbf{AssemblyConnections}\\
Assembly-Konnektoren verbinden die Bedarfsschnittstelle (requires interface) einer Komponente mit der Angebotsschnittstelle (provides interface) einer anderen. Hierbei ist zu beachten, dass nicht die Komponenten selber sondern die Verwendung der Komponenten (modelliert durch die Vaterschaftsbeziehung zwischen Komponenten) zu verbinden sind, da ein und dieselbe Komponente je nach Verwendung mit verschiedenen anderen Komponenten verbunden sein kann. Somit erhalten Assembly-Konnektoren Fremdschlüsselbeziehungen zur \verb+CompRelations+ und nicht zu \verb+Components+ selber. Die beiden Fremdschlüssel \verb+fk_prov_role+ und \verb+fk_req_role+ modellieren die Beziehungen zu den Schnittstellen, welche an der Verbindung beteiligt sind. Die Fremdschlüsselbeziehung in die Tabelle \verb+Roles+ enthält hierbei jedoch Redundanz, da die Komponenten sowohl in dieser Beziehung als auch in der Fremdschlüsselbeziehung zur Tabelle \verb+CompRelations+ definiert sind. Möglicherweise ist an dieser Stelle die Referenzierung auf die Schnittstellentabelle selber besser geeignet. 

\item \textbf{DelegationConnections}\\
Im Gegensatz zu Assembly-Konnektoren können Delegation-Konnektoren ausschließlich innerhalb von zusammengesetzten Komponenten (\emph{composite components}) vorkommen. Sie verbinden die Schnittstelle einer äußeren mit der Schnittstelle einer inneren Komponente entweder auf der Angebots- oder auf der Bedarfsseite. 
