...
namespace Palladio.RegularExpressions.GNFA
{
	...
	public class FSMToRegexConverter
	{
		...
		{
			IEditableFiniteStateMachine gnfa = CreateGNFA(fsm); //adds the new start and new destination state
			MeltTransitions(ref gnfa); 
			while (gnfa.States.Length > 2) // GNFA-Algorithm
			{
		...
		{
			IEditableFiniteStateMachine gnfa = FSMFactory.CreateEmptyFSM(); // new FSM
			...
			for (int i = 0; i < fsm.Transitions.Length; i++)
			{
				...
				// conversion of inputs to symbols
				IInput input = RegexFactory.Symbol(fsm.Transitions[i].Attributes as IAttributeHash, fsm.Transitions[i].InputSymbol);
		...
		private static void ConvertGNFA(ref IEditableFiniteStateMachine gnfa, IState state)
		{
			...
			foreach (IState si in gnfa.States)
			{
				...
				foreach (IState sj in gnfa.States)
				{
					...
					// create the new merged transition
		...
		{
			// regex = r1 r2* r3
			// r1 and r3 must exist to construct the left side of the new regex. 
			// They can be epsilon
		...
		{
			IRegEx reg = transition.InputSymbol as IRegEx;
			return reg;
		}
