...
Da die Berechnung von SEFFs für \textit{Composite Components} damit problemlos und effizient möglich ist, unterstützt das Komponentenmodell für zusammengesetzte Komponente nur berechnete SEFFs. Diese haben den Vorteil automatisch konsistent mit den inneren SEFFs einer Komponente gehalten werden zu können.
...
Der \textit{Provided Type} definiert nur seine angebotenen Schnittstellen. Er enthält keine Informationen über die interne Realisierung, darf diese Informationen also nicht spezifizieren. Zudem darf er keine benötigten Schnittstellen angeben. Dies bedeutet, dass alle Sub"=Typen eines konkreten \textit{Provided Type} in der internen Realisierung frei sind und ebenfalls keine Vorgaben über die benötigten Schnittstellen einhalten muss. Damit ist der \textit{Provided Type} der Typ, der für Sub"=Typen am wenigstens Anforderungen definiert. Auf diese Weise werden Komponenten definiert, deren Verwendung im Vordergrund steht.

TODO: --> Instanzbegriff! von Sub-Typ unterscheiden!



\paragraph{Complete Type}
\label{sec:CompleteType}
In Erweiterung des \textit{Provided Type} ermöglicht der \textit{Complete Type} zusätzlich die Definition der benötigten Schnittstellen. Ein \textit{Complete Type} muss \textit{alle} erlaubten benötigten Schnittstellen definieren. Ein Sub"=Typ 



\paragraph{Implementation Type}
\label{sec:ImplementationType}
Neben den Anforderungen, die ein \textit{Complete Type} erfüllen muss, muss der \textit{Implementation Type} seine interne Realisierung festlegen. Das bedeutet, dass alle internen Komponenten, die direkt in einer derart typisierten Komponente enthalten sind, definiert werden müssen.

- SEFFs sind verpflichtend. Bei CCs ist der SEFF berechnet. berechnetes requires protocol muss konsistent mit protokoll auf höheren ebenen sein, wo von hand spezifiziert, siehe \ref{sec:BerechnungvonSEFFsundProtokollen}.
