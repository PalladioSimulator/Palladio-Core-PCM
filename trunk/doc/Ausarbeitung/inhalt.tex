...
Gegenüber seinem \textit{Complete Type} Vater darf ein \textit{Implementation Type} weder \textit{Provides} noch \textit{Requires} Schnittstellen hinzufügen oder entfernen. TODO: richtig?
...
Die \texttt{m:1}"=Beziehung zwischen \textit{Provides Type} und \textit{Complete Type} ist möglich, da, wie oben beschrieben, ein \textit{Complete Type} ebenfalls Schnittstellen anbieten kann, die vom \textit{Provides Type} nicht spezifiziert wurden. Dafür können "`zuviel"' angebotene Schnittstellen von anderen \textit{Provides Types} angeboten werden, wodurch der gleiche \textit{Complete Type} automatisch zu einem möglichen gültigen Sub"=Typ werden kann. Um die gewünschten Sub"=Typ"=Beziehungen von zufällig vorkommenden zu unterscheiden, spezifiziert ein jeder Sub"=Typ seine "`Eltern"'"=Typen in einer \textit{Parent}"=Relation, realisiert als eine Liste von Eltern"=Komponenten.
...
Die Assembly"=Definition wird kann unabhängig von der Allokation erfolgen. Beide Aktionen zusammen definieren den Kontext, der in Kapitel \ref{sec:Kontext} beschrieben wird.
TODO: Muss Assembly-Def der Allokation voraus gehen?
...
Alle Entitäten des Komponentenmodells tragen eine eindeutige Identifizierer (ID). Werden in Relationen andere Entitäten referenziert, erfolgt diese Referenzierung stets über die IDs der Entitäten, damit an keiner Stelle die Relationen des Komponentenmodells mehrdeutig sein können.
...
TODO: Komp. mit gleichen prov und req interf. kann != anderer sein, die die gleichen schnittstellen bietet! --> ID.

TODO: Was hat ID, wie ist equals und == definiert?

TODO: Typisierte IDs --> Möglichkeiten mit EMF bedenken

TODO: In Relationen werden üblicherweise IDs verwendet
...
\begin{itemize}
	...
	\item Schnittstellen
	\item Kontexte; hier tragen die Kontexte Zusatzinformationen, die sich nicht nur auf eine Komponente beziehen.
...
- QoS-Fähigkeiten
- Ressourcen-Typen definieren: Berechnend und nicht-berechnend in Kapitel Allokation.
