...
\begin{itemize}
	\item den \textbf{\textit{Implementation Type}}, dann werden nur die Kompositionsstrukturen (Komponentenarchitektur) berücksichtigt
	\item \textbf{Assembly}, hier wird zusätzlich die Assembly berücksichtigt, in der der Kontext liegt
	\item \textbf{Allocation}, hier werden zusätzlich die Ressourcen (\textit{computational}, \textit{non"=computational}) erfasst, auf denen die Komponenten und Konnektoren allokiert wurden.
\end{itemize}
...
Containment
Component	Deployed-on relation
Execution environment
- Concurrency, security…
 Container properties
 Component configuration
Computed	Functional
Results of parametric contracts	Non-functional
QoS-attributes



Zusätzlich erfasst Abbildung \ref{fig:cm-component-hierarchy-context-01} die bereits in vorangehenden Kapiteln definierten Komponenten"=Typ"=Ebenen, die Assembly, die Allokation (\textit{Allocation}) und die Laufzeit (\textit{Runtime}) in einer Graphik. Letztere Ebene ist im Komponentenmodell bisher lediglich angedacht, jedoch noch nicht umgesetzt. Weitere Informationen hierzu finden sich in Kapitel \ref{sec:AusblickaufdieEntwicklung}, Seite \pageref{sec:AusblickaufdieEntwicklung}.
