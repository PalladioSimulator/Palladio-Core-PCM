...
\subsection{Rollen}
\label{sec:Rollen}
Rollen sind kein expliziter Bestandteil des Komponentenmodells, gleichwohl wird über Rollen unterschieden, welche Sichten auf das Komponentenmodell möglich sind. In Kapitel \ref{sec:Assembly} wird auf Rollen Bezug genommen, die wie folgt definiert sind:

\begin{itemize}
	\item \textbf{Entwickler} entwerfen und definieren Komponenten und Schnittstellen sowie Konnektoren zwischen diesen Entitäten.
	\item \textbf{\textit{System"=Deployer}} definieren Beziehungen zwischen Komponenten, Konnektoren und Ressourcen (siehe Kapitel \ref{sec:Allokation}).
\end{itemize}
...
\begin{itemize}
	\item \textbf{Rechnende Ressourcen} (computational resources); etwa Server"=Hardware. In Abbildung \ref{fig:cm-assembly-allocation-01} werden diese Ressourcen UML2"=konform als "`Boxen"' dargestellt.
