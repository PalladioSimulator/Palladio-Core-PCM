...
Im Palladio Kontext wird häufig der Begriff Allokation verwendet. Die Verwendung erfolgt im Sinne von \textit{Deployment} und meint die Verteilung von Komponenten auf Ressourcen. Es wird also festgelegt, welche Komponente welche Ressourcen nutzt. Neben den Komponenten erfolgt eine Abbildung der Konnektoren auf "`Verbindungsressourcen"', also etwa LAN- und WAN-Verbindungen. Als Stereotyp der Relation zwischen Komponenten, Konnektoren und Ressourcen wird \texttt{$\flqq$deployed-on$\frqq$} verwendet.

Als Ressourcen"=Typen kennt das Komponentenmodell:

\begin{itemize}
	\item Rechnende Ressourcen (computational resources); etwa Server"=Hardware. In Abbildung \ref{fig:cm-assembly-allocation-01} werden diese Ressourcen UML2"=konform als "`Boxen"' dargestellt.
	\item Nicht"=Rechnende Ressourcen (non"=computational resources); hierzu zählen Netzwerkverbindungen, also jene Ressourcen, die zur Datenübertragung zwischen rechnenden Ressourcen vermitteln. In Abbildung \ref{fig:cm-assembly-allocation-01} wird diese Form der Ressourcen UML2"=konform als Linie zwischen den berechnenden Ressourcen dargestellt.
\end{itemize}
...
Die Assembly"=Definition wird kann unabhängig von der Allokation erfolgen. Beide Aktionen zusammen definieren den Kontext, der in Kapitel \ref{sec:Kontext} beschrieben wird. Komponenten können jedoch nur allokiert werden, wenn Sie zuvor in einer Assembly definiert wurden. Das heißt, dass ein Allokation immer nur von Assemblies erfolgt, nie von Komponenten direkt. Im einfachsten Fall wird dabei eine Komponente dabei von genau einer Assembly erfasst und allokiert.



\subsubsection{Assembly}
\label{sec:Assembly}
Im vorigen Kapitel wurde die Definition einer Assembly bereits angedeutet. An dieser Stelle sollen die Eigenschaften zusammengefasst dargestellt werden.

\begin{itemize}
	\item Eine Assembly beinhaltet eine Menge von Komponenten.
	...
	\item Assemblies werden nur von System"=Deployern verwendet. Die für diese Zielgruppe definierte Sicht
\end{itemize}

Unsichtbarkeit des Komponenten-Innenlebens (für Deployer) --> Verwendung ab Assembly nur Deployer vorgesehen




\subsection{Assembly und Composite Component, Allokation}
...
\paragraph{Assembly}
Auch \textit{Assemblies} (im Komponentenmodell auch \textit{Component Assemblies} genannt) werden aus Komponenten aufgebaut. Im Gegensatz zur \textit{Composite Component} ist das Innere einer Assembly jedoch zum Zeitpunkt des \textit{Deployments} für den \textit{System Deployer} sichtbar. Dies ermöglicht, dass innere Komponenten einer Assembly auf unterschiedlichen Ausführungsumgebung allokiert werden können.
Das bedeutet, dass einzelne Komponenten (ohne Kopien) durch ihre Erfassung in einer Assembly zur gleichen Zeit nicht auf unterschiedlichen Ausführungsumgebungen ausgeführt werden können. Sehr wohl kann jedoch komponentenweise verteilt werden. 
...
Abbildung \ref{fig:cm-assembly-to-cc-01} beschreibt die Umwandlung einer Assembly in eine \textit{Composite Component}, so wie der Vorgang von einem \textit{System Deployer} wahrgenommen würde. Da das Innere einer \textit{Composite Component} nicht mehr sichtbar ist, würden innere Komponenten und Konnektoren bei einer Transformation unsichtbar. Dafür muss für eine solche Transformation definiert werden, welche Schnittstellen angeboten und benötigt werden, damit die neue \textit{Composite Componente} vollständig spezifiziert ist. Auch die Delegation"=Konnektoren müssen definiert werden, auch wenn sie für den \textit{System Deployer} nicht sichtbar sind, damit alle Aufrufe definierten Aufrufpfaden folgen können.
...
\begin{itemize}
	\item den \textbf{\textit{Implementation Type}}, dann werden die Kompositionsstrukturen (Komponentenarchitektur) mit Komponenten, den Konnektoren und der "`Enthalten"'"=Beziehung berücksichtigt.
	\item \textbf{Assembly}, hier wird zusätzlich die Assembly berücksichtigt, in der der Kontext liegt.
	\item \textbf{Allokation}, hier werden zusätzlich die Ressourcen (\textit{computational}, \textit{non"=computational}) über die \texttt{$\flqq$deployed-on$\frqq$}"=Relation und die Ausführungsumgebung erfasst, auf denen die Komponenten und Konnektoren allokiert wurden. Zusätzlich können die folgenden Eigenschaften erfasst werden: \textit{Concurrency}, \textit{Security}, Container"=Eigenschaften, Komponentenkonfiguration.
\end{itemize}

Abbildung \ref{fig:cm-component-hierarchy-context-01} erfasst die bereits in vorangehenden Kapiteln definierten Komponenten"=Typ"=Ebenen, die Assembly, die Allokation (\textit{Allocation}) und die Laufzeit (\textit{Runtime}) in einer Graphik. Letztere Ebene ist im Komponentenmodell bisher lediglich angedacht, jedoch noch nicht umgesetzt. Weitere Informationen hierzu finden sich in Kapitel \ref{sec:AusblickaufdieEntwicklung}, Seite \pageref{sec:AusblickaufdieEntwicklung}.

Je tiefer eine Ebene in der Graphik dargestellt ist, desto detailreicher müssen die Komponenten"=Typen definiert werden, entsprechend sind mehr Informationen über die Komponenten definiert. In umgekehrter Richtung nach oben werden die Komponenteninformationen abstrakter und kommen damit einer Grob"=Modellierung Komponentenarchitekturen entgegen.

TODO: 1:n-Beziehung zwischen Implementation Type und Assembly.
...
Um IDs auch bei Zusammenführung unterschiedlicher Komponentenmodell"=Instanzen eindeutig zu erhalten, wird empfohlen ein der \textit{Globally Unique Identifier} (GUID, siehe auch \cite{guid-wiki}) ähnliches Konzept zu verwenden. Dieses Konzept macht es auch über Systemgrenzen hinweg und für einander vollkommen unbekannte Systeme wahrscheinlich, dass die generierte GUID eindeutig ist.
...
Das Komponentenmodell bietet die Möglichkeit alle Entitäten des Modells mit beliebigen Zusatzinformationen "`Anotationen"' (Attribute) zu versehen. Da die genauen Anwendungsszenarien des Komponentenmodells nicht exakt absehbar sind und Vorgaben das Modell nicht unnötig limitieren sollen, bleibt das Komponentenmodell, ebenso wie bei Protokollen und SEFFs, offen und gibt keine feste Menge von Typen vor. Damit lassen sich beliebige Anotations"=Typen ergänzen, sofern sie benötigt werden. Diese können Messwerte ebenso sein wie \textit{Quality of Service} Merkmale (siehe Kapitel \ref{sec:WeitereFaehigkeiten}) von Entitäten oder beliebige andere Anotations"=Typen. Das heißt also, dass die Datentypen und Datenstrukturen nicht durch das Komponentenmodell festgelegt werden.

Gleichwohl definiert das Komponentenmodell einen Standardsatz von Anotations"=Typen, die eine schnelle Arbeit mit dem Komponentenmodell vereinfachen sollen. Hierzu zählen die Datentypen Integer (\texttt{int}, \texttt{long}), Fließkommazahl (\texttt{float}, \texttt{double}) und Zeichenkette (\texttt{string}) analog ihrer Definition in .NET C\# 1.1 (siehe \cite{net-csharp-gleitkommatypen,net-csharp-ganzzahltypen,net-csharp-string}).

Um die Unterscheidung zu erleichtern, welcher Datentyp für eine Anotation in einem konkreten Fall vorliegt, muss jede Anotation den Datentyp für das Datenfeld deklarieren. Zusätzlich trägt eine Anotation einen Anotationsnamen, der auch über mehrere Anotationen hinweg benutzt werden kann, um beispielsweise eine speziellen Laufzeitmesswert bei allen Komponenten einer Modell"=Instanz zu kennzeichnen. Anotation können überdies aus anderen Anotationswerten berechnet werden. Um eine Unterscheidung zu erlauben, welche Werte berechnet wurden, gibt es je Anotation ein \textit{Flag}, dass diese Werte kennzeichnet. Im Allgemeinen sind berechnete Anotationen \textit{read"=only}. Gleichwohl können Berechnungen, die auf eine Komponentenmodellinstanz durchgeführt wurden, berechnete Werte setzen. Daher ist eine Anotation ein Vier"=Tupel, bestehend aus $(Anotationsnamen, Datentyp, Datenfeld, Calculated-Flag)$. 

Anotationen können zu den folgenden Entitäten des Komponentenmodells vergeben werden:
\begin{itemize}
	\item Komponenten, wobei die Anotation eindeutig einer Komponente zugeordnet werden kann. Die Anotation höherer Komponenten Typ"=Ebenen vererbt sich, kann also auch von Kinder"=Entitäten einer Komponente zugegriffen werden.
	\item Schnittstellen
	...
	\item Delegations"=Konnektoren TODO: Sinn unterhalb Assembly-Ebene?
	\item Ressourcen (berechnend und nicht-berechnend)
	\item Assemblies; komplette Assemblies können ebenfalls eigene Anotationen tragen.
\end{itemize}

Zu einer Entität sind \texttt{0..*} Anotationen möglich. Damit kann die gleiche Anotation jedoch nicht für mehrere Entitäten zugleich verwendet werden. Die Anotationen zu einer Entität sind ungeordnet. Anotationen können für bestimmte Entitäten nicht vorgeschrieben werden. Es ist also nicht vorhersehbar, ob eine Entität eine bestimmte Anotation trägt.

Zusätzlich können auch SEFFs und Protokolle Anotationen verwenden. Da die Typen für SEFFs und Protokolle nicht festgelegt sind, kann an dieser Stelle jedoch nicht beschrieben werden, zu welchen dann erweiterten Modellkonstrukten Anotationen vergeben werden können. Anotationen folgen dennoch dem gleichen, oben beschriebenen, Aufbau.

TODO: oder doch nur Tupel? Gibt es für Anotationsnamen ein ID-Konzept; für Datentypen?



\subsection{Weitere Fähigkeiten}
\label{sec:WeitereFaehigkeiten}
Parametrisierte Verträge, QoS, ...



\subsection{Validierung von Modellinstanzen}
\label{sec:ValidierungvonModellinstanzen}
TODO: Erweiterungen zu Kapitel \ref{sec:Typ-Spezialisierung}. Insbesondere auf QoS-Attributen, parametrisieren Verträgen und SEFFs (prüfen ob eine substitution zulässig: erhaltung der seff-eigenschaften denkbar).
TODO: Unterscheidung: Validierung vs. Konsistenz.

zwei möglichkeiten zur validierung
- dauerhaft valide (on the fly)
- auf wunsch
...
Component State
Implementation of the concepts > Steffen
Improvement of QoS prediction models
...
- Komponentenbeschreibung getrennt von verwendung
- auf gleicher hierarchiestufe darf eine komponente nicht mehrfach vorkommen
- komponente liegt IM kontext
- "`äußere"' assembly connectors nur unterhalb von impl. möglich --> vorher TYP!
- Typ-Hierarchie (Typ vs. inter-comp-conn) erklären
- Sichtbarkeit des Inneren von CCs erklären: Entwickler hat IMMER Einblick, Deployer nicht (nur in Assemblies)
