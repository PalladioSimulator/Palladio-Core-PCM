% $Log$
% Revision 1.103  2006/02/14 10:34:02  kelsaka
% - BC und CC als Realisierung eines Implementation Type herausgestellt
%
% Revision 1.102  2006/02/14 10:09:17  kelsaka
% - assembly: kontext-komponente darf in 0..1 assemblies erfasst werden.
%
% Revision 1.101  2006/02/12 15:29:24  kelsaka
% - weitere TODOs entfernt
%
% Revision 1.100  2006/02/12 15:17:40  kelsaka
% - inhalt: modellierung in inhalt2 ausgegliedert
%
% Revision 1.99  2006/02/12 15:04:28  kelsaka
% - Annotationen um Alternativen ergänzt
%
% Revision 1.98  2006/02/12 14:49:26  kelsaka
% - IDs im Bezug auf Kontext überarbeitet
%
% Revision 1.97  2006/02/12 14:21:00  kelsaka
% - Kapitel Kontext überarbeitet - vollständig
%
% Revision 1.96  2006/02/12 13:50:10  kelsaka
% - Kapitel Kontext überarbeitet
%
% Revision 1.95  2006/02/12 09:55:38  kelsaka
% - weitere überarbeitung
%
% Revision 1.94  2006/02/11 18:47:31  kelsaka
% - kontext-bezüge in weiteren kapiteln überarbeitet
%
% Revision 1.92  2006/02/11 14:35:02  kelsaka
% - cc im hinblick auf kontext überarbeitet
%
% Revision 1.91  2006/02/11 13:00:05  kelsaka
% - anhang ausgegliedert
%
% Revision 1.90  2006/02/11 11:34:49  kelsaka
% - kontext bei assembly konnektoren ergänzt
%
% Revision 1.89  2006/02/11 10:56:16  kelsaka
% - kontext kurzeinführung ergänzt
%
% Revision 1.88  2006/02/11 10:02:08  kelsaka
% - appendix angefügt
%
% Revision 1.87  2006/02/10 19:15:26  kelsaka
% - pagestyle geändert
%
% Revision 1.86  2006/02/10 19:07:23  kelsaka
...
\chapter{Das Palladio Komponentenmodell}
...
\section{Schnittstellen und Rollen}
...
\section{Kurzeinführung: Kontext}
\label{sec:Kurzeinfuehrung-Kontext}
An dieser Stelle soll der Begriff des \textit{Kontext}s kurz skizziert werden. Um die weiteren Konzepte des Komponentenmodells korrekt darstellen zu können, wird eine kurze Definition dieses Begriffs vorgezogen. In den nächsten Kapiteln reicht ein Basis"=Verständnis des Konzepts aus. Die ausführliche Beschreibung des Kontexts findet in Kapitel \ref{sec:Kontext} auf Seite \pageref{sec:Kontext}, wo unter Berücksichtigung der bis dahin eingeführten Hierarchie des Komponentenmodells, eine genauere Definition erfolgen kann.

Bisher wurden Komponenten und Rollen vorgestellt. Tatsächlich wurden noch keine Möglichkeiten zur Interaktion zwischen Rollen vorgestellt. Komponenten, die Rollen anbieten, werden bis jetzt also von ihrer Verwendung losgelöst beschrieben. In diesem Zusammenhang eignet sich der Begriff des \textit{Komponenten"=Typs} zur Einordnung des aktuellen Verständnisses: Welche Rollen eine Komponente einnimmt, wird über ihren Typ definiert.

Komponenten und Rollen können, wie zu erwarten, verwendet werden. Werden Komponenten und Rollen verwendet, stehen sie, vereinfacht betrachtet, in einem Verwendungs"=\textit{Kontext}. Es ist also strikt zwischen \textit{Komponenten"=Typen} und der Verwendung von Komponenten"=Typen in einem \textit{Kontext} zu unterscheiden. Ein Komponenten"=Typ kann also in einer beliebigen Anzahl (\texttt{0..*}) von Kontexten verwendet werden. Der Kontext ermöglicht eine eindeutige Identifikation des Verwendungsortes. Die Verwendung einer Komponente bzw. Rolle impliziert einen Kontext, in dem die Komponente bzw. Rolle liegt. Der Kontext sollte als immanente Eigenschaft verstanden werden, die an die Verwendung von Komponenten geknüpft ist.

Um eine einfachere Unterscheidung zwischen Komponenten (als Typ) bzw. Rollen und Komponent bzw. Rollen im Kontext zu ermöglichen, werden im Kontext verwendete Komponenten und Rollen im Folgenden kurz "`Kontext"=Komponenten"' und "`Kontext"=Rollen"' genannt.



\section{Assembly Konnektoren}
...
Assembly Konnektoren sind implizit immer vorhanden, wenn eine \textit{Requires}"=Schnittstelle mit einer \textit{Provides}"=Schnittstelle verbunden ist. Auch wenn Assembly Konnektoren in der graphischen Notation, wie in Abbildung \ref{fig:cm-calculated-cc-seff-01}, nicht gezeichnet werden, ist ein Assembly Konnektor vorhanden.



\section{Zusammengesetzte Komponenten}
...
\paragraph{Delegations"=Konnektor}
In Abbildung \ref{fig:cm-composite-component-01} wird eine exemplarische \textit{Composite Component} dargestellt. Wie zu sehen ist, kann eine \textit{Composite Component} intern eine beliebige Menge von Komponente aufnehmen. Damit diese Komponenten verwendet werden können, bedient man sich zwischen den Komponenten auf der gleichen Hierarchiestufe der bereits eingeführten Assembly Konnektoren. Zusätzlich reglementieren Delegationskonnektoren (\textit{Delegation Connector}), wie auf der Provides-Seite externe Aufrufe auf interne Aufrufe geleitet werden und umgekehrt wie auf der Requires Seite Aufrufe interner Komponente nach außen aus die \textit{Composite Component} heraus geleitet werden.

Auch im Bezug auf die \textit{Composite Components} und Delegations"=Konnektoren ist der Kontext bei genauer Betrachtung zu berücksichtigen. Während eine Composite Component \textit{nach außen} über die von ihr angebotenen und benötigten Rollen als Komponenten"=Typ aufgefasst werden können, findet intern eine Verwendung von Komponenten statt. Entsprechend muss der Verwendungskontext von Rollen und Komponenten erfasst werden.

Damit die Abbildung zwischen der äußeren und inneren Rolle einer \textit{Composite Component} eindeutig bleibt, muss stets eine \texttt{1:1} Beziehung zwischen äußerer und innerer Rolle, bestehen. Ein Delegationskonnektor verbindet als immer genau zwei Kontext"=Rollen (eine äußere angebotene Kontext"=Rolle mit einer inneren angebotenen Kontext"=Rolle \textit{oder} eine äußere geforderte Kontext"=Rolle mit einer inneren geforderten Kontext"=Rolle). 

Manchmal werden Delegationskonnektoren zusätzlich über ihren Verwendungskontext unterschieden. Findet die Verwendung zum Zwecke einer Abbildung zwischen angebotenen Rollen statt, spricht man auch von \textit{Provides Delegation Connectors}. Hier werden somit genau eine interne Kontext"=\textit{Provides Role} und genau eine externe Kontext"=\textit{Provides Role} einer \textit{Composite Component} miteinander verbunden. Im umgekehrten Fall der Verwendung zwischen benötigten Schnittstellen wird der entsprechende Delegationskonnektor auch \textit{Requires Delegation Connector} genannt. Dies entspricht der Verbindung genau einer internen Kontext"=\textit{Requires Role} mit genau einer externen Kontext"=\textit{Requires Role} einer \textit{Composite Component}.
...
In der graphischen Visualisierung mit UML erscheinen Delegations"=Konnektoren als gestrichelte Verbindungslinie unter Angabe der Delegationsrichtung über einen Pfeil. Die Linie verbindet eine äußere mit einer inneren Schnittstelle. Wird im Sprachgebrauch davon gesprochen, dass ein Delegations"=Konnektor zwei Schnittstellen miteinander verbindet, sind, präzise betrachtet, die in der UML"=Darstellung mit den Schnittstellen assoziierten Rollen gemeint.



\paragraph{Definition: Composite Component}
Eine \textit{Composite Component} ist zunächst einmal eine einfache Komponente mit angebotenen und benötigten Rollen. Zusätzlich gehören zu einer \textit{Composite Component} die inneren Kontext"=Komponenten (\texttt{0..*}) und die inneren Konnektoren (\texttt{0..*}) (Assembly Konnektor, \textit{Provided Delegation Connector}, \textit{Required Delegation Connector}). Da Konnektoren (sowohl Delegation als auch Assembly) auf Kontext"=Rollen definiert sind, sind auch Konnektoren vom Kontext abhängig. Interne Schnittstellen werden indirekt über die internen Konnektoren $\mapsto$ Rollen erfasst. Nicht zu einer \textit{Composite Component} gehören externe Assembly Konnektoren.



\paragraph{Auflösen von Composite Components}
...
\section{Schnittstellen, Signaturlisten und Protokolle}
...
\subsection{Signaturlisten und Signaturen}
...
\begin{itemize}
	\item einen Rückgabe"=Datentyp oder \texttt{void} (kein Rückgabewert),
...
\section{Service Effekt Spezifikation}
...
\section{Berechnung von SEFFs und Protokollen}
...
\subsection{Berechnung von benötigten Protokollen}
...
\subsection{Berechnung der SEFFs von Composite Components}
...
\section{Komponenten: Typ-Ebenen}
...
Die farbliche Hinterlegung des "`Typ"=Balkens"' auf der linken Seite deutet an, dass der \textit{Provided Type} und \textit{Complete Type} dem "`klassischen"' Typ"=Verständnis (vgl. \cite{CZ-SOFA-CM-Hierarchy2,CZ-SOFA-CM-Hierarchy}) entsprichen, indem die angebotenen Schnittstellen definiert werden, wohingegen der \textit{Implementation Type} auch Vorgaben für die interne Realisierung eines Typs macht. Letzteres wird klassischerweise weniger als Typ angesehen.
...
Sowohl \textit{Composite Components} als auch \textit{Basic Components} sind \textit{Implementation Types}. Sie erben direkt von der Meta-Klasse "`Implementation Type"', wie in Abbildung \ref{fig:cm-component-types-inheritance-conforms-01} dargestellt wird. Ein \textit{Implementation Type} kann entweder von einer \textit{Composite Component} oder einer \textit{Basic Component} realisiert werden (\texttt{$\flqq$realizes$\frqq$}"=Stereotyp).



\subsection{Darstellung und Ebenenbeziehungen}
...
\subsection{Quantittäten der Sub-Typ-Beziehungen}
...
Auch die \texttt{m:1}"=Beziehung zwischen \textit{Complete Type} und \textit{Implementation Type} gründet auf einem der gleichen Möglichkeit, dass ein \textit{Implementation Type} zugleich Sub"=Typ zweier \textit{Complete Types} ist. Bietet ein \textit{Implementation Type} die Schnittstellen "`I1"' und "`I2"' an, so kann es zwei \textit{Complete Types} geben, von denen einer ebenfalls "`I1"' anbietet, der andere jedoch "`I2"'. Damit gibt es zu einem \textit{Implementation Type} zwei, und im Allgemeinen m, "`Eltern"'"=Typen.
...
\section{Substitution von Typen}
...
Die genaue Definition gültiger Substitutionen ist variabel und nicht an fest durch das Komponentenmodell festgeschrieben. Kapitel \ref{sec:ValidierungvonModellinstanzen} beschreibt Möglichkeiten zur dynamischen Definition von Validität. Daher kann an dieser Stelle nur das allgemeine, dem Komponentenmodell zu Grunde liegende, Verständnis dargestellt werden.



\subsection{Co- und Contra-Varianz}
...
\subsection{Delegations-Konnektoren}
...
\begin{itemize}
	\item Komponenten (gleich welcher Ebene)
	\item Schnittstellen
	\item Ressourcen
\end{itemize}
...
\section{Allokation und Assembly}
\label{sec:AllokationundAssembly}



\subsection{Allokation und Ressourcen}
\label{sec:Allokation}



\paragraph{Definition}
In der Palladio"=Gruppe wird der Begriff der \textit{Allokation} verwendet. Die Verwendung erfolgt im Sinne von \textit{Deployment} und meint die Verteilung von Komponenten auf Ressourcen. Es wird also festgelegt, welche Komponente welche Ressourcen nutzt. Neben den Komponenten erfolgt eine Abbildung der Konnektoren auf "`Verbindungsressourcen"', also etwa LAN- und WAN-Verbindungen. Als Stereotyp der Relation zwischen Komponenten, Konnektoren und Ressourcen wird \texttt{$\flqq$deployed-on$\frqq$} verwendet.

\textit{Allokation} findet immer für Komponenten und Assembly Konnektoren im Kontext statt. Assembly"=Konnektoren liegen immer im Kontext vor, da sie nur zwischen Kontext"=Rollen definiert sind. Dass die \textit{Allokation} nur für Kontext"=Komponenten sinnvoll ist, wird schnell ersichtlich, wenn man bedenkt, dass der gleiche Komponenten"=Typ sehr oft verwendet werden kann und dabei mit unterschiedlichen anderen Kontext"=Komponenten interagiert, die nicht auf den gleichen Ressourcen liegen.
...
\subsection{Vergleich von Allokation und Assembly}
...
\begin{itemize}
	\item Eine Assembly fasst eine Menge von Kontext"=Komponenten (\texttt{1..*}) und die dazugehörigen "`äußeren"' Assembly Konnektoren zusammen. Als "`äußere"' Assembly"=Konnektoren gelten dabei jene, die innerhalb keiner Komponente liegen. Die Assembly darf keine Komponenten außerhalb von Kontexten ("`Komponenten"=Typ"') enthalten. 
	\item Die Kontext"=Komponenten im Inneren einer Assembly sind nicht auf Kontext"=Instanzen einer bestimmten Komponenten"=Typ"=Ebene beschränkt. Zur gleichen Zeit dürfen auch Kontext"=Komponenten unterschiedlicher Typ"=Ebenen auftreten.
	\item Kontext"=Komponenten dürfen in \texttt{0..1} Assemblies referenziert werden. Im ersten Fall werden sie nicht von der Assembly erfasst -- im zweiten Fall sind sie auf eine Assembly festgelegt.
	...
	\item Nicht alle Komponenten einer Modell"=Instanz müssen Teil in einer Assembly erfasst werden. Die Assembly stellt somit eine Auswahl dar.
\end{itemize}
...
\section{Assembly und Composite Component}
...
\section{Komponenten-Typ vs. Komponenten-Verwendung}
...
\section{Kontext}
\label{sec:Kontext}
Im vorigen Kapitel wurde die \textit{Verwendung} von Komponenten"=Typen gegenüber der \textit{Definition} von Komponenten"=Typen abgegrenzt. Als Kurzdefinition wurde der Begriff des \textit{Kontext}s bereits eingeführt, der in diesem Zusammenhang wichtig ist. An dieser Stelle erfolgt eine vollständige Darstellung des Kontexts unter Rückgriff auf die bis jetzt eingeführten Konzepte des Komponentenmodells.



\begin{figure}[htbp]
	\centering
		\includegraphics[width=1.00\textwidth]{./image/cm-context-repository-usage-01.pdf}
	\caption{Unterscheidung von Komponenten"=Typ und Komponenten"=Kontext}
	\label{fig:cm-context-repository-usage-01}
\end{figure}



\paragraph{Idee}
Einer der neuesten Begriffe des Palladio Komponentenmodells, im Vergleich zu den Vorversionen, ist der Begriff des Kontexts (\textit{Context}). Abbildung \ref{fig:cm-context-repository-usage-01} zeigt, wie sich ein Kontext definiert. Im Beispiel kommt die Komponente "`C1"' als Kopie bzw. Instanz in zwei unterschiedlichen Kontexten vor. Zum einen (in "`Context 1"') ist sie mit "`C0"' auf der \textit{Provides}"=Seite und "`C2"' auf der \textit{Requires}"=Seite verbunden, zum anderen (in "`Context 2"') ist sie auf der \textit{Provides}"=Seite mit "`Master"' und auf der \textit{Requires}"=Seite mit "`C3"' verbunden. Um exakt definieren zu können, welche Verwendung von "`C1"' bzw. einer Komponente im Allgemeinen gemeint ist, wird der Kontext eingeführt, der beschreibt, wie eine Komponente verwendet wird (auch "`Kontext"=Komponente"'), also mit welchen anderen Komponenten eine Komponente verbunden ist. Der Kontext identifiziert eindeutig den Ort der Verwendung.
...
In Abbildung \ref{fig:cm-context-A-B-01} wird "`Component A"' zweifach verwendet. Da es sich um den gleiche Komponenten"=Typ handelt, ließen sich die Verwendungen nicht unterscheiden. Über den Kontext gelingt jedoch die Unterscheidung. Die linke (Kontext"=)Komponente liegt in "`Context A"', die rechte (Kontext"=)Komponente in "`Context B"'. Da der Kontext implizit vorhanden ist, bleibt die Verwendung des gleichen Komponenten"=Typs in jedem Fall eindeutig.



\paragraph{Definition}
Komponenten können nie ohne Kontext verwendet werden. Wichtig ist zu beachten, dass Komponenten \textit{in} einem Kontext liegen, Komponenten also einen Zeiger auf den Kontext haben, in dem sie liegen. Zu jeder Komponente, die außerhalb der Definition des Komponenten"=Typen im \textit{Repository} existiert, gibt es \textit{implizit} ein Kontext. Da jeder Kontext eindeutig ist (siehe Kapitel \ref{sec:Identitaet}), kann eine Komponente -- beispielsweise der Assembly"=Ebene -- auf der gleichen Hierarchie"=Stufe mehrfach vorkommen, ihre Verwendung bleibt automatisch eindeutig über den Kontext.

Im Kapitel über Konnektoren wurde definiert, dass Konnektoren stets \textit{Kontext}"=Rollen verbinden. Bisher wurden Kontext"=Rollen noch nicht näher betrachtet. Kontext"=Rollen unterscheiden sich von "`normalen"' Rollen lediglich durch den Ort ihrer Verwendung. Komponenten"=Typen sind mit "`normalen"' Rollen assoziiert. Kontext"=Komponenten sind mit Kontext"=Rollen assoziiert. Der Kontext, in dem eine Kontext"=Komponente und ihre Kontext"=Rollen liegen, muss der gleiche sein. Der Kontext wird durch die Kontext"=Komponente bestimmt. Alle assoziierten Kontext"=Rollen einer Kontext"=Komponente fallen automatisch in den gleichen Kontext wie die Kontext"=Komponente.

Eine Kontext"=Komponente muss die die gleichen Kontext"=Rollen anbieten und benötigen, wie der Komponenten"=Typ "`normale"' Rollen anbietet und benötigt. Zwischen "`normalen"' Rollen und Kontext"=Rollen gibt es ebenso eine "`instance-of"' Beziehung wie zwischen Komponenten"=Typen und Kontext"=Komponenten. Kontext"=Rollen sind Instanzen "`normaler"' Rollen.

Um die Unterscheidung zwischen \textit{Provided Roles} und \textit{Required Roles} zu erhalten, gibt \textit{Provided} Kontext"=Rollen und \textit{Required} Kontext"=Rollen.
...
\begin{itemize}
	\item \textbf{Implementation}. Es werden die Kompositionsstrukturen innerhalb von \textit{Composite Components} eindeutig. Da die inneren Assembly Konnektoren von Composite Components zwischen Kontext"=Rollen verbinden, erfolgt eine Verbindung zweier eindeutiger Kontexte. Eindeutig im Kontext sind somit: Kontext"=Komponenten, Kontex"=Rollen, Schnittstellenverwendung (über Rollen) und Assembly Konnektoren.
	\item \textbf{Assembly}. Auf diesem Detail"=Niveau werden die Kompositionsstrukturen der Komponentenarchitektur eindeutig, da auch außerhalb von \textit{Composite Components} liegende Assembly Konnektoren erfasst werden.
	\item \textbf{Allokation}. Hier werden zusätzlich die Ressourcen (\textit{computational}, \textit{non"=computational}) über die \texttt{$\flqq$deployed-on$\frqq$}"=Relation und die Ausführungsumgebung eindeutig erfasst, auf denen die Kontext"=Komponenten und Konnektoren allokiert wurden.
...
In den graphischen Darstellungen dieses Kapitels wurden die Kontexte hervorgehoben eingezeichnet. Der Kontext sollte jedoch als immanente Eigenschaft von Kontext"=Komponenten und auch Kontext"=Rollen verstanden werden, der nur zur einfacheren Erkennbarkeit des Kontexts gezeichnet wird. Eine "`physische Ausprägung"' im Sinne eines Datencontainers hat ein Kontext jedoch nicht. 



\section{Hierarchie-Ebenen}
...
\begin{itemize}
	\item Kontext. Jeder Kontext trägt eine eigene ID. Zu jeder Verwendung einer Komponente entsteht ein Kontext mit neuer ID. Kontext"=IDs können unter keinen Umständen mehrfach auftreten.
	\item Komponenten"=Typen (je Typ"=Ebene eine eigener ID"=Typ). Jeder Komponenten"=Typ einer Typ"=Ebene lässt sich damit von anderen unterscheiden. Wird ein Komponenten"=Typ verfeinert, so tragen auch jene Komponenten, die untereinander in einer \textit{conforms}"=Beziehung stehen, unterschiedliche IDs. Kontext"=Komponenten unterscheiden sich untereinander über die ID des Kontexts, in dem sie liegen. Da jeder Kontext nur genau eine Kontext"=Komponente enthält, ist eine Kontext"=Komponente eindeutig identifiziert.
	\item Schnittstellen. Jede Schnittstelle unterscheidet sich von jeder anderen über die ID, auch wenn die Schnittstellen ansonsten vollkommen identisch sind (bspw. exakt die gleichen Signaturen tragen). Ausnahmen sind jedoch unter bestimmten Bedingungen möglich: Werden, wie beispielsweise in einer \textit{Chain"=of"=Responsibility} (vgl. \cite{GAMMA}, S. 410ff) üblich, in einer Kette von Komponenten stets die gleichen Schnittstellen angeboten und benötigt, so kann aus Gründen der Übersichtlichkeit in der Darstellung und muss auch Gründen der Eindeutigkeit der Zuordnung direkt miteinander verknüpfter Komponenten, die \textit{gleiche} Schnittstelle mehrfach dargestellt und verwendet werden. An dieser Stelle muss einem Entwickler eindeutig klar sein, dass tatsächlich die gleiche Schnittstelle verwendet.
	\item TODO: Seffs? Protokolle? Signatur? (wird im Zusammenhang mit der Modellierbarkeit in EMF geklärt)
	\item Konnektoren. Die IDs von Konnektoren müssen sich unterscheiden.
	\item Rollen. Jede Rolle hat eine eigene ID. Jede Kontext"=Rolle hat eine von der "`normalen"' Rolle und untereinander verschiedene ID. 
...
Annotation können derzeit nur genau einer Entität zugeordnet werden. Für das Komponentenmodell sind für die Zukunft zwei Alternativen nicht ausgeschlossen:
\begin{enumerate}
	\item Annotationen sind weiterhin nur genau einer Komponente zugeordnet, können sich jedoch rekursiv auf untergeordnete Entitäten vererben. Die rekursive Vererbung ist ohnehin (siehe oben) entlang der Komponenten"=Typ"=Hierarachie vorgesehen. Die Erweiterungen würden Annotationen einer \textit{Composite Component} auf alle inneren Komponenten vererben. Eigenschaften einer Assembly würden sich auf die Entitäten in der Assembly übertragen.
	\item Annotationen könnten alternativ zu \texttt{n} Entitäten zugelassen werden. Damit könnten sich unterschiedliche Entitäten die gleichen Annotationen teilen. Zudem ermöglicht diese Variante die Definition globaler Werte, die für eine gesamte Komponentenmodell"=Instanz gelten. Dies kann von konkreten Ausfallwahrscheinlichkeiten bis hin zu globalen Zeit"=Multiplikatoren reichen.
\end{enumerate}
...
\subsection{Parametrisierte Verträge}
...
\section{Validierung von Modellinstanzen}
...
\subsection{Zustandsmodellierung}
...
\subsection{Mehrfache Schnittstellen-Verwendung}
...
\subsection{Rekursion von Composite Components}
...
\subsection{Stimulus-Response-Mechanismus}
...
\section{Ausblick auf die Entwicklung}
...
Delagationskonnektor / Delegations"=Konnektor / Delegations-Konnektor einheitlich schreiben
