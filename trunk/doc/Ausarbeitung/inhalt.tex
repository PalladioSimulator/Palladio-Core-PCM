...
Benutzer"=Rollen sind strikt von Rollen, wie sie in Kapitel \ref{sec:SchnittstellenUndRollen} definiert werden, zu unterscheiden.
...
\begin{itemize}
	\item Eine Assembly fasst eine Menge von Komponenten und die dazugehörigen "`äußeren"' Assembly"=Konnektoren, die in Kontexten (siehe Kapitel \ref{sec:Kontext}) zusammen. Als "`äußere"' Assembly"=Konnektoren gelten dabei jene, die innerhalb keiner Komponente liegen.
...
\begin{itemize}
	\item \textbf{Assembly}, dann werden die Kompositionsstrukturen (Komponentenarchitektur) mit Komponenten, den Konnektoren und der "`Enthalten"'"=Beziehung berücksichtigt. zusätzlich die Assembly berücksichtigt, in der der Kontext liegt. TODO
...
\begin{itemize}
	...
	\item TODO: Seffs? Protokolle? Signatur?
	...
	\item Kontexte. Jeder Kontext trägt eine eigene ID. Zu jeder Verwendung einer Komponente entsteht ein Kontext mit neuer ID. Kontext"=IDs können unter keinen Umständen mehrfach auftreten. TODO: richtig?
	\item TODO: Annotationen mit ID?
\end{itemize}
...
TODO: Gibt es für Annotationsnamen ein ID-Konzept; für Datentypen?
...
Auf der rechten Seite wird eine alternative Modellierung dargestellt. Hier bietet die Komponente "`Cp"' die Schnittstelle "`Ip"' lediglich ein mal an. Dies ist im Komponentenmodell ein gültiges Konstrukt. Es greifen jedoch zur gleichen Zeit zwei unterschiedliche Komponenten (hier "`Cr1"' und "`Cr2"') zwei Komponenten auf die eine Schnittstelle zu. Diese Situation ist im Komponentenmodell untersagt. Ein Assembly Konnektor kann lediglich eine \textit{Requires}"=Schnittstelle mit einer \textit{Provides}"=Schnittstelle verbinden, die ansonsten jeweils nicht über Assembly Konnektoren verbunden sind.

- die gleiche Schnittstelle darf nur 1x angeboten werden: keine Konzepte von Ports (mit eigenem Zustand)
--> Zustand
...
Abbildung \ref{fig:cm-interface-twice-forbidden-req-01}. --> Modellierbar wäre redundante Verbindung --> Ausfallsicherheit; inbesondere weil auf verschiedene Ressourcen allokierbar. nicht entscheidbar, welche tatsächlich genutzt
...
- Ja, außnahme hierzu, die wir bewußt ausgeklammer haben, wären benötigte Sequenzen. Das sind Sequenzen auf dem Requires Interface, die immer als Reaktion auf einen bestimmmten Stimulus erfolgen müssen. Beispielsweise das Speichern von Daten in einer Datenbank bei einer Datenänderung. Der Aufruf auf der DB ist Teil des Requires Interfaces, muss aber erfolgen. Den Fall kannst du in der Ausarbeitung erwähnen, er gilt aber bislang als Future Work.
...
\textbf{TODO:}
CM ein Datenspeicher und keine
Algorithmensammlung

Beides ist hier denkbar, drum ist das CM ein Datenspeicher und keine
Algorithmensammlung. Ausgefeiltere Algorithmen erkennen halt andere Klassen
von Interoperabilitätsproblemen. Es gibt aber nur eine Datensammlung jedoch
viele Strategien (Stichwort: Strategy Muster). Da ich mich ja auch nicht
mehr hauptberuflich mit Interoperabilitätsprüfungen beschäftige, wird wohl
am Ende entweder nichts oder was relativ einfaches implementiert sein (siehe
auch dein IP).
