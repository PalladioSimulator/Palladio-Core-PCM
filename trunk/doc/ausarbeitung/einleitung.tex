...
Soll ein neu zu erstellendes Softwaresystem auf Basis vorhandener Komponenten erstellt werden, so stellt sich häufig die Frage nach der Perfomance des Gesamtsystems. Deren Beantwortung vor der Implementierung ist häufig Vorraussetzung zur Einhaltung einiger nichtfunktionaler Anforderungen wie zum Beispiel der mittlere Antwortzeit. Sind die Antwortzeiten der Dienste der einzelnen Komponenten häufig bekannt, so gilt es nun die Antwortzeit des Gesamtsystems mit einer bestimmten Konfiguration dieser Komponenten und ggf. Konnektoren zu ermitteln. Weiterhin ist häufig die Aufdeckung von 'Flaschenhälsen' im System hilfreich, um durch eine gezielt andere Konfiguration der Komponenten die Performance zu erhöhen.
\par
