...
In the following, we give examples of common component based software architectures. These architectures are used later to highlight the types of analysis, which can be performed with the Palladio component meta model. In doing so, we demonstrate the advantages of our model compared to common enterprise scale or selected scientific component models.
...
Another typical scenario for component-based software development can be found in the prototypical web server developed in our group. The processing of each incoming HTTP request is realised by a Chain of Responsibility (CoR) \cite{gamma1995a}. A part of the chain is shown in figure \ref{fig:part_CoR}.


Each element of the chain is a software component as shown in figure \ref{fig:CoR_Component}. The component provides and requires an interface called \texttt{IHttpRequestProcessor}, which contains a single service \texttt{HandleRequest}. Thus, it is possible to extend the functionality of the webserver by simply adding a new component to the CoR, which is conformant to the one shown in figure \ref{fig:CoR_Component}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\columnwidth]{images/CoR_Part.pdf}
  \caption{Part of the Chain of Responsibility.}
  \label{fig:CoR_Part}
\end{figure}

Here, a single component shall not be replaced by another implementation of the same functionality, but the functionality of the system can be extended by adding components of the same type to the chain. 

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\columnwidth]{images/CoR.pdf}
  \caption{One Component of the Chain of Responsibility.}
  \label{fig:CoR_Component}
\end{figure}



%Die OMG schreibt über Komponenten (p.150, http://www.omg.org/docs/ptc/04-10-02.pdf):
A component represents a modular part of a system that encapsulates its contents and whose manifestation is replaceable within its environment.

A component defines its behaviour in terms of provided and required interfaces. As such, a component serves as a type, whose conformance is defined by these provided and required interfaces (encompassing both their static as well as dynamic semantics). One component may therefore be substituted by another only if the two are type conformant. Larger pieces of a system's functionality may be assembled by reusing components as parts in an encompassing component or assembly of components, and wiring together their required and provided interfaces.
