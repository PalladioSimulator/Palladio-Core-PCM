...
namespace Palladio.ComponentModel.UnitTests
{
	...
	public class SignatureTest
	{
		...
		{
			sigOne1 = ComponentFactory.CreateSignature("role", typeof(string), "ToString", 
			...
			sigOne2 = ComponentFactory.CreateSignature("role", typeof(string), "ToString", 
				new IParameter[] { ComponentFactory.CreateParameter(typeof(string),"tabs") },
				typeof(RoleIDMissmatchException), typeof(TypeNotFoundException));
			sigTwo1 =	ComponentFactory.CreateSignature("role", typeof(string),"ToString", 
				new IParameter[] { ComponentFactory.CreateParameter(typeof(string),"tabs"),
													 ComponentFactory.CreateParameter(typeof(string),"end") },
				typeof(TypeNotFoundException), typeof(RoleIDMissmatchException)
				);
			sigTwo2 =	ComponentFactory.CreateSignature("role", typeof(string),"ToString", 
				new IParameter[] { ComponentFactory.CreateParameter(typeof(string),"end"),
													 ComponentFactory.CreateParameter(typeof(string),"tabs") },
				typeof(TypeNotFoundException), typeof(RoleIDMissmatchException)
				);
			sigThree1 =	ComponentFactory.CreateSignature("role A","Clone");
			sigThree2 =	ComponentFactory.CreateSignature("role B","Clone");
		}
		...
		[Test] public void Equals()
		{
			...
			Assert.IsFalse(sigThree1.Equals(sigThree2));
		}
		...
		[Test] public void GetHashCode()
		{
			...
			Assert.IsFalse(sigThree1.GetHashCode() == sigThree2.GetHashCode());
		}
