	...
	public class MatrixTools
	{
		...
		public static ScalarMatrix Invert(IMatrixExpression aMatrix)
		{
			...
			for( int n = 0; n < sm.LengthY; n++ ) // (n,n) : current position in aMatrix
			{
				...
				// make pivot 1
				IVectorExpression vecSm = new VectorScalarDivision( aMatrix.Context, sm.GetRowVector(n), sm[n,n] );
				IVectorExpression vecInv = new VectorScalarDivision( aMatrix.Context, inv.GetRowVector(n), sm[n,n] );
				...
				for( int row = 0; row < sm.LengthY; row++)
				{
					if ( row != n )
					{
						...
						scaledPivotRow = new VectorScalarMultiplication(aMatrix.Context, scalar, sm.GetRowVector(n) );
						resultRow = new VectorVectorSubtraction(aMatrix.Context, sm.GetRowVector(row), scaledPivotRow );
						sm.SetRowVector(row, resultRow);
						
						scaledPivotRow = new VectorScalarMultiplication(aMatrix.Context, scalar, inv.GetRowVector(n) );
