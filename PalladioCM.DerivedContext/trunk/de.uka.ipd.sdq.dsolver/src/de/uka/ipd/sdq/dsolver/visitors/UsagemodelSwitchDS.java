 ...
 * 
 */
public class UsagemodelSwitchDS extends UsagemodelSwitch {

	private static Logger logger = Logger.getLogger(UsagemodelSwitchDS.class
			.getName());
	 ...
	 */
	public UsagemodelSwitchDS(PCMInstance inst) {
		pcmInstance = inst;
	...
	private void handleBasicComponent(EntryLevelSystemCall call,
		...
		// stackFrame.buildParameterContext(call
		// .getActualParamterUsage_EntryLevelSystemCall());

		ResourceDemandingBehaviour b = (ResourceDemandingBehaviour) seff;
		
		
		SeffSwitchDS visitor = new SeffSwitchDS(pcmInstance,
				callContext);
		try {
	...
	private Context createCallContext(EntryLevelSystemCall call,
		...
		// EList paramList = uc.getActualParameterUsage_UsageContext();
		// List blah = new ArrayList();
		// Iterator parListIter = parList.iterator();
		// while (parListIter.hasNext()){
		// ParameterUsage pu = (ParameterUsage)parListIter.next();
		//			
		// if (parListIter.hasNext()){
		// int a=0;
		// }
		// //paramList.add(pu);
		// blah.add(pu);
