...
{
	/// <summary>
	/// Zusammenfassung für PluginControlForm.
	/// </summary>
	public class PluginControlForm : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.pluginColumnAssembly.Width = 159;
			// 
			// openFileDialog
			// 
			...
			this.Text = "Plugins";
			this.Load += new System.EventHandler(this.PluginControlForm_Load);
		...
		private void PluginControlForm_Load(object sender, System.EventArgs e)
		{
			buildPluginList(-1);
		}
		...
		private void pluginList_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			if (this.pluginList.SelectedIndices.Count > 0)
			{
				...
				IPlugin plugin = AppMain.Instance.PluginManager.Plugins[this.pluginList.SelectedIndices[0]];
				this.labelDescr.Text = plugin.Description;
				if (plugin.Status == PluginStatusEnum.ACTIVE) 
				{
					this.buttonActivate.Enabled = false;
					this.buttonDeactivate.Enabled = true;
				}
				else if (plugin.Status == PluginStatusEnum.INACTIVE) 
				{
		...
		private void buttonDeactivate_Click(object sender, System.EventArgs e)
		{
			if (this.pluginList.SelectedIndices.Count > 0)
			{
				int index = this.pluginList.SelectedIndices[0];
				IPlugin plugin = AppMain.Instance.PluginManager.Plugins[index];
				plugin.Status = PluginStatusEnum.INACTIVE;
				buildPluginList(index);
			}
		}
		...
		private void buttonActivate_Click(object sender, System.EventArgs e)
		{
			if (this.pluginList.SelectedIndices.Count > 0)
			{
				int index = this.pluginList.SelectedIndices[0];
				IPlugin plugin = AppMain.Instance.PluginManager.Plugins[index];
				plugin.Status = PluginStatusEnum.ACTIVE;
				buildPluginList(index);
			}		
		}
		...
		private void buttonRemove_Click(object sender, System.EventArgs e)
		{
			if (this.pluginList.SelectedIndices.Count > 0)
			{
				//AppMain.Instance.PluginManager.RemovePlugin(type);
				buildPluginList(-1);
			}
		}

		private void buildPluginList(int selectedIndex) 
		{
			this.pluginList.Items.Clear();
			foreach (IPlugin plugin in AppMain.Instance.PluginManager.Plugins) 
			{
				string status = plugin.Status.ToString();
				string extension = "";
				if (plugin is IViewPlugin) extension = "View";
				//if (plugin is IAnalyzePlugin) extension = "Analysis";

				ListViewItem item = new ListViewItem(new string[]{status,plugin.Name,extension,""});
				item.Focused = false;
				item.Selected = false;
				this.pluginList.Items.Add(item);
			}
			if (selectedIndex >= 0 && selectedIndex < this.pluginList.Items.Count) 
			{
				this.pluginList.Items[selectedIndex].Selected = true;
				this.pluginList.Items[selectedIndex].Focused = true;
		...
		private void buttonAdd_Click(object sender, System.EventArgs e)
		{
			if (openFileDialog.ShowDialog() == DialogResult.OK)
			{
				...
				AppMain.Instance.PluginManager.loadAssembly(openFileDialog.FileName);
				buildPluginList(-1);
			}
		}
	}
