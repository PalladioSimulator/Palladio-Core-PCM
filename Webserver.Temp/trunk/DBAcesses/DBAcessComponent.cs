...
namespace DBAcesses
{
	...
	public class DBAcessComponent
	{
		...
		private ValidateDBComands sqlValidator;
		...
		public DBAcessComponent()
		{
			...
			this.sqlValidator = new ValidateDBComands(this.properties);
		}
		...
		public void InsertRow(string tablename, Hashtable fieldsToModify, string username, string passwort)
		{
			try
			{
				...
				DatabaseTable currentTable = this.properties.GetAssociatedTable(tablename);
				SqlConnection conn = new SqlConnection(@"Data Source=" + this.properties.Datasource + "; Integrated Security=SSPI;" +
					"Initial Catalog=Webservertest");
				conn.Open();
