...
public class MarkovBuilder {
	...
	/**
	 * Key to denote a Markov failure state as an application failure state.
	 */
	private static final String APPLICATIONFAILUREKEY = "ApplicationFailureType";

	/**
	 * Key to denote a Markov failure state as an environment failure state.
	 */
	...
	public void appendFailureHandlingMarkovChain(
			...
			final List<String> handledFailures, final boolean optimize) {
		...
		for (State failureState : failureStates) {
			...
			if (failureTypeLabelValue == null) {
				// TODO: Maybe we should use a default failure type.
	...
	public List<State> getFailureStates(final MarkovChain markovChain) {
		...
		for (int i = 0; i < markovChain.getStates().size(); i++) {
			if (markovChain.getStates().get(i).getType().equals(
					StateType.FAILURE)) {
	 ...
	 */
	public String getFailureTypeLabelValue(final State state) {
		for (Label label : state.getLabels()) {
			if (label.getKey().equals(APPLICATIONFAILUREKEY)
					|| label.getKey().equals(ENVIRONMENTFAILUREKEY)) {
	...
	public State getStartState(final MarkovChain markovChain) {
		...
		// Go through all states of the Markov Chain:
		for (int i = 0; i < markovChain.getStates().size(); i++) {
			if (markovChain.getStates().get(i).getType()
					.equals(StateType.START)) {
	 ...
	 *            indicates if prefixes of specific chain shall be appended to
	 *            prefixes of aggregateChain
	 */
	public void incorporateMarkovChain(final MarkovChain parentChain,
			...
			final boolean optimize, final boolean appendPrefixes) {
		...
		if (!parentChain.getStates().contains(aggregateState)) {
			throw new MarkovException("State '" + aggregateState
					+ "' is not contained in the markov chain '"
					+ parentChain.getName() + "'.");
		}
		...
		if (!aggregateState.getType().equals(StateType.DEFAULT)) {
			throw new MarkovException(
					"Only default states can be incorporated. '"
							+ aggregateState.getName() + "' is of type '"
							+ aggregateState.getType() + "'.");
		}
		...
		if (recordTraces && appendPrefixes) {
			for (State specificState : copiedSpecificMarkovChain.getStates()) {
				...
				specificState.setName(getStateName(specificState.getTraces()
						.get(specificState.getTraces().size() - 1),
						specificState.getTraces().subList(0,
								specificState.getTraces().size() - 1)));
			}
		}
		...
		for (State failureState : stateSpecificFailures) {
			...
			State existingFailureState = findMatchingFailureState(
					stateAggregateFailures, failureState);
			if (existingFailureState != null) { // duplicate found
				// delegate from duplicate to existing state
				connectStates(parentChain, failureState, existingFailureState,
						1.0);
				// remove duplicate
		...
		//
		delegateOutgoingTransitions(parentChain, aggregateState,
	...
	/**
	 * Creates a basic Markov chain with a start, success and multiple failure
	 * nodes for a given list of failure descriptions. Such a chain can be used
	 * to represent an internal action with its application failure
	 * probabilities, or the sending of a message over a communication link. All
	 * failure probabilities must sum up to at most 1.0.
	 * 
	 * @param prefixes
	 *            the prefixes of the chain name
	 ...
	 * @return the resulting Markov Chain
	 */
	public MarkovChain initBasicMarkovChainWithFailures(
		...
		for (FailureDescription description : failureDescriptions) {
			State failureState = addStateForFailureDescription(markovChain,
					prefixes, description);
			connectStates(markovChain, startState, failureState, description
		...
		if (successProbability < 0) {
			throw new MarkovException(
					"Total failure probability of basic Chain \""
							+ getName(prefixes) + "\" is greater than 1.0.");
		}
	 ...
	 * ScenarioBehaviour or ResourceDemandingBehaviour.
	 * 
	 * @param prefixes
	 ...
	 * @return the resulting Markov Chain
	 */
	public MarkovChain initBehaviourMarkovChainByAction(
			final List<String> prefixes, final List<AbstractAction> actions,
	 ...
	 * @return the resulting Markov Chain
	 */
	public MarkovChain initBranchMarkovChain(final List<String> prefixes,
		...
		for (int i = 0; i < branchProbabilities.size(); i++) {
			...
			// Create a new State:
			State stateBranch = addState(markovChain, StateType.DEFAULT,
					"Alternative(" + (i + 1) + ")", prefixes);

			// Create a transition from the Start State to the new one:
			connectStates(markovChain, startState, stateBranch,
					branchProbabilities.get(i));
	...
	/**
	 * Creates a Markov Chain for a fork action with a list of forked
	 * behaviours.
	 * 
	 * @param prefixes
	...
	public MarkovChain initForkMarkovChain(List<String> prefixes,
			ArrayList<ForkedBehaviour> behaviours, ArrayList<State> statesOut) {
	 ...
	 * @return the resulting Markov Chain
	 */
	public MarkovChain initLoopMarkovChain(final List<String> prefixes,
		...
		for (int i = 0; i < pmf.getModelPmf().getSamples().size(); i++) {
			...
			if (!(sample.getValue() instanceof Integer)) {
				throw new MarkovException("Invalid Sample value "
						+ sample.getValue().toString() + " in "
			...
			for (int j = 0; j < sampleValue; j++) {
				...
				// Create a new State:
				State stateSample = addState(markovChain, StateType.DEFAULT,
						"Alternative(" + (i + 1) + ")-Iteration(" + (j + 1)
								+ ")", prefixes);
				connectStates(markovChain, stateToSuccess, stateSample,
			...
			connectStates(markovChain, stateToSuccess, successState,
					transitionProbability);
		}
	...
	/**
	 * Creates a Markov chain for an internal action failing with probability
	 * 1.0 because of unavailable hardware resources. The failure probability is
	 * divided between all unavailable resources.
	 * 
	 * @param prefixes
	 *            the prefixes of the chain name
	 * @param failureDescriptions
	 ...
	 * @return the resulting Markov Chain
	 */
	public MarkovChain initResourceFailureMarkovChain(
			final List<String> prefixes,
			final List<FailureDescription> failureDescriptions) {
		...
		State startState = addState(markovChain, StateType.START,
				StateType.START.toString(), prefixes);
		addState(markovChain, StateType.SUCCESS, StateType.SUCCESS.toString(),
				prefixes);
		...
		for (FailureDescription description : failureDescriptions) {
			State failureState = addStateForFailureDescription(markovChain,
					prefixes, description);
			connectStates(markovChain, startState, failureState,
					failureProbability);
		}
	...
	private State addStateForFailureDescription(final MarkovChain chain,
		...
		// The state needs a corresponding label:
		Label failureLabel = markovFactory.createLabel();
		if (description.getFailureType() instanceof MarkovEnvironmentFailureType) {
			failureLabel.setKey(ENVIRONMENTFAILUREKEY);
		} else {
			failureLabel.setKey(APPLICATIONFAILUREKEY);
		}
		failureLabel.setValue(description.getFailureType().getName());
	...
	private void appendFailureHandlingChain(final MarkovChain aggregateChain,
		...
		connectStates(aggregateChain, stateHandlingSuccess,
				stateAggregateSuccess, 1.0);
		...
		for (State innerFailureState : stateHandlingFailures) {
			// check if this is a duplicate
			State existingFailureState = findMatchingFailureState(
					stateAggregateFailures, innerFailureState);
			if (existingFailureState != null) { // duplicate found
				// delegate from duplicate to existing state
				connectStates(aggregateChain, innerFailureState,
						existingFailureState, 1.0);
				// remove duplicate
	...
	private State findFailureState(final List<State> states,
		...
		for (State state : states) {
			...
			for (Label label : state.getLabels()) {
				...
				// performance
				if ((label.getKey().equals(APPLICATIONFAILUREKEY) || label
						.getKey().equals(ENVIRONMENTFAILUREKEY))
	...
	private void reduceState(final MarkovChain markovChain,
		...
		if (!markovChain.getStates().contains(stateToReduce)) {
			throw new MarkovException(
					"Cannot reduce Markov State. The Markov Chain does not contain that state.");
		}
		...
		if (!stateToReduce.getType().equals(StateType.DEFAULT)) {
			throw new MarkovException(
					"Cannot reduce Markov State. Only intermediate states can be reduced.");
		}
	...
	/**
	 * Checks if a Markov chain is valid. The chain is judged to be valid if
	 * each transition has a source state and a target state, and if exactly one
	 * start and success state exist. If a chain is not valid, a Markov
	 * exception is thrown.
	 * 
	 * @param chain
	...
	private void validateChain(final MarkovChain chain) {
		...
		for (State s : chain.getStates()) {
			if (s.getType() == StateType.START) {
				if (hasStart) {
					throw new MarkovException(
							"Multiple start states in Markov Chain.");
				} else {
					hasStart = true;
				}
				...
				if (hasSuccess) {
					throw new MarkovException(
							"Multiple success states in Markov Chain.");
				} else {
					hasSuccess = true;
				}
			}
