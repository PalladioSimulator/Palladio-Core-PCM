...
public class MarkovSeffVisitor extends SeffSwitch<MarkovChain> {
	...
	public MarkovChain caseExternalCallAction(
		...
		if (seff == null) {
			...
			List<FailureOccurrenceDescription> rawFailureDescriptions = contextWrapper
			...
			for (FailureOccurrenceDescription description : rawFailureDescriptions) {
				resultFailureDescriptions
						.add(FailureDescription
								.createApplicationFailureDescription(
										description
												.getApplicationFailureType_FailureOccurrenceDescription()
												.getEntityName(),
										description
												.getApplicationFailureType_FailureOccurrenceDescription()
												.getId(), description
		...
		// If a retry policy has been specified for this external call, reflect
		// the failure handling behaviour accordingly:
	...
	public MarkovChain createExternalCallActionChain(
		...
		MarkovChain innerMarkovChain = new MarkovSeffVisitor(
				...
				simplifiedStateHandling, optimize, recordTraces).doSwitch(seff);
		...
		CommunicationLinkResourceSpecification commLink = contextWrapper
				.getConcreteLinkingResource(call);
		...
		if (commLink != null) {
			...
			FailureDescription description = FailureDescription
					.createEnvironmentFailureDescription(
							commLink
									.getCommunicationLinkResourceType_CommunicationLinkResourceSpecification()
									.getEntityName(),
							commLink
									.getCommunicationLinkResourceType_CommunicationLinkResourceSpecification()
									.getId(), commLink.getFailureProbability());
	...
	private MarkovChain caseInternalActionForResourceState(
		...
		for (ProcessingResourceDescriptor descriptor : descriptors) {
			...
			MarkovProcessingResourceState state = descriptor.getCurrentState();
			if (state == null) {
			...
			// 100% probability:
			if (state.equals(MarkovProcessingResourceState.NA)) {
				failureDescriptions.add(FailureDescription
						.createEnvironmentFailureDescription(descriptor
								.getType().getName(), descriptor.getType()
								.getId(), 1.0));
				continue;
			}
		}
		...
		if (failureDescriptions.isEmpty()) {
			...
			for (FailureOccurrenceDescription description : internalAction
					.getFailureOccurrenceDescriptions_InternalAction()) {
				failureDescriptions
						.add(FailureDescription
								.createApplicationFailureDescription(
										description
	...
	/**
	 * Returns the names of the failure types that a given failure handling
	 * entity can handle.
	 * 
	 * @param entity
	 ...
	 */
	private List<String> getFailureTypeNames(final FailureHandlingEntity entity) {
		...
		for (FailureType failureType : entity
			...
			if (failureType instanceof ApplicationFailureType) {
				...
				failureTypes.add(((EnvironmentFailureType) failureType)
						.getProcessingResourceType_EnvironmentFailureType()
						.getEntityName());
			} else {
	...
	private void setResourceState(
		...
		for (int i = 0; i < descriptors.size(); i++) {
			MarkovProcessingResourceState state = (((number >> i) % 2) == 0) ? MarkovProcessingResourceState.OK
					: MarkovProcessingResourceState.NA;
