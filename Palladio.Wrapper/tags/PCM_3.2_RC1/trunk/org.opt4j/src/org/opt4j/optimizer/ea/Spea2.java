 ...
 * </p>
 * <p>
 * The {@code Spea2}-Selector will not work properly if the {@code Objectives}
 * are not fixed, i.e., the {@code Objectives} of an {@code Individual} change
...
public class Spea2 implements Selector {
	...
	protected Map<Individual, IndividualSet> map = new HashMap<Individual, IndividualSet>();
	protected Set<IndividualSet> individualSets = new HashSet<IndividualSet>();

	protected LinkedList<Integer> freeIDs = new LinkedList<Integer>();
	protected double[][] distance;
	protected boolean fitnessDirty = true;

	/**
	 * A wrapper for multiple equal (based on their objectives) Individuals.
	 * 
	 * @author lukasiewycz
	 * 
	 */
	private class IndividualSet extends HashSet<Individual> implements
			Comparable<IndividualSet> {
		...
		public boolean dominates(IndividualSet individualSet) {
			return this.getObjectives()
					.dominates(individualSet.getObjectives());
		}
	...
	public Collection<Individual> getLames(int lambda,
		...
		Set<Individual> lames = new HashSet<Individual>();
		...
		if (lambda > 0) {
			...
			if (countIndividuals(dominated) >= lambda) {
				...
				for (Individual individual : getLamesFromNonDominated(lambda
						- lames.size())) {
	...
	public Collection<Individual> getLamesFromNonDominated(int count) {
		...
		while (set.size() < count) {
			...
			if (candidates.size() <= (count - set.size())) {
				...
				for (IndividualSet individualSet : getNearest(count
						- set.size(), candidates)) {
					Individual individual = individualSet.first();
					remove(individual);
					set.add(individual);
	 ...
	 */
	protected List<IndividualSet> getNearest(int n,
			Collection<IndividualSet> candidates) {

		assert (candidates.size() > n);

		List<IndividualSet> lames = new ArrayList<IndividualSet>();

		Map<IndividualSet, List<IndividualSet>> orderedLists = new HashMap<IndividualSet, List<IndividualSet>>();
		...
		while (lames.size() < n) {
			List<IndividualSet> lcandidates = new ArrayList<IndividualSet>(
					orderedLists.keySet());
			...
			for (int k = 0; k < size - 1; k++) {
				...
				for (IndividualSet candidate : lcandidates) {
					double value = distance(candidate, orderedLists.get(
							candidate).get(k));
					min = Math.min(min, value);
				}

				for (Iterator<IndividualSet> it = lcandidates.iterator(); it
						.hasNext();) {
	...
	protected void update(Collection<Individual> population) {
		Set<Individual> popSet = new HashSet<Individual>(population);

		if (!popSet.equals(map.keySet())) {

			Set<Individual> adds = new HashSet<Individual>(popSet);
			adds.removeAll(map.keySet());

			Set<Individual> removes = new HashSet<Individual>(map.keySet());
	...
	/**
	 * Return the distance of two IndividualSets.
	 * 
	 * @param w0
	 *            first set
	 * @param w1
	...
	/**
	 * Calculate the distance between two IndividualSets.
	 * 
	 * @param w0
	 *            the first set
	 * @param w1
	...
	/**
	 * Returns all dominated IndividualSets (fitness > 0).
	 * 
	...
	/**
	 * Returns all non-dominated IndividualSets (fitness == 0).
	 * 
	...
	/**
	 * Returns the total number of Individuals in a collection of
	 * IndividualSets.
	 * 
	 * @param collection
