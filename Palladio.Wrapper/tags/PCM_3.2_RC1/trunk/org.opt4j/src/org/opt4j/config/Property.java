...
/**
 * The {@code Property} contains information about a single property of a
 * {@link PropertyModule}.
 * 
 * @author lukasiewycz
 * 
...
public class Property {
	...
	protected Collection<Requirement> requirements = new ArrayList<Requirement>();

	protected Map<Class<? extends Annotation>, Annotation> annotations = new HashMap<Class<? extends Annotation>, Annotation>();
	 ...
	 *            the annotations
	 */
	public Property(Module module, String name, Class<?> type, Method getter,
			Method setter, Iterable<Annotation> annotations) {
		super();
		this.module = module;
		this.name = name;
		this.type = type;
	...
	/**
	 * Adds a {@code Requirement}.
	 * 
	 * @param requirement
	 *            the {@code Requirement} to add
	 */
	...
	public void setValue(String value) throws InvocationTargetException {
		...
		try {
			...
			if (Character.TYPE.equals(type) || Character.class.equals(type)) {
				...
				object = PropertyModule.toEnumElement(value, type
				...
				if (value != null && !value.equals("")) {
					...
					if (isAssignable(actual, c)) {
						...
						System.err.println("Can not assign " + c + " to Class<"
								+ actual + ">");
					}
				}
			} else {
