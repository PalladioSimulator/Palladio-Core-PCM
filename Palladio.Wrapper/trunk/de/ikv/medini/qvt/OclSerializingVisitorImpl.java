...
abstract public class OclSerializingVisitorImpl extends SemanticsVisitorImpl implements SemanticsVisitor, QvtVisitor {
	...
	public static final String[] fgOCLKeywords = {
	        "self", "true", "false", "not", "and", "or", "xor", "if", "then", "else", "endif", "min", "max", "abs", "floor", "mod", "concat", "toUpper", "toLower", "subString", "size", "includes", "count", "includesAll", "isEmpty", "notEmpty", "sum", "exists", "forAll", "iterate", "select", "collect", "implies", "in", "let", "allInstances", "oclType", "oclIsUndefined", "oclAsType", "oclIsTypeOf", "oclIsKindOf", "OclVoid", "OclAny", "Set", "Bag", "Sequence", "Collection", "OrderedSet", "TupleType", "Tuple", "undefined", "package", "endpackage", "init", "derive", "body", "context", "inv", "pre", "post", "def" }; //$NON-NLS-1$
	...
	protected void doAccept(SemanticsVisitable node, Object data) {
		if (node != null) {
			/*
			 * Note that Classifier and Namespace sub types may be classes provided by adapters,
			 * e.g. <code>org.oslo.ocl20.bridge4emf.NamespaceImpl</code>, which would not call our
			 * visitor, so handle these cases here explicitely.
			 */
	...
	/**
	 * Returns the representation of the property's name
	 * 
	 * @param property
	 * @return the result
	 */
	protected String getPropertyName(Property property) {
		if (this.isKeyword(property.getName())) {
	...
	/**
	 * Tells whether the given string is a keyword
	 * 
	 * @param string
	 * @return
	 */
	protected boolean isKeyword(String string) {
		return Arrays.asList(OclSerializingVisitorImpl.fgOCLKeywords).contains(string);
	}
	...
	public Object visit(OperationCallExp host, Object data) {
		...
		if ("not".equals(opName)) {
			...
			this.doAccept(host.getSource(), data);
			if (Character.isJavaIdentifierPart(opName.charAt(0)) && !"and".equals(opName) && !"or".equals(opName)
			        && !"xor".equals(opName) && !"implies".equals(opName)) {
	...
	public Object visit(PropertyCallExp host, Object data) {
		this.doAccept(host.getSource(), data);
		this.add(".");
		this.add(this.getPropertyName(host.getReferredProperty()));
		return null;
	}
	...
	public Object visit(Property host, Object data) {
		this.add(this.getPropertyName(host));
		return null;
	}
