...
/**
 * The {@code Opt4JTasksPanel} extends the {@code DefaultTasksPanel} by
...
public class Opt4JTasksPanel extends DefaultTasksPanel {
			...
			if (!(renderer instanceof ProgressRenderer)) {
				...
				if (c instanceof JComponent) {
					...
					if (value != null) {
						...
						int length = jc.getFontMetrics(jc.getFont())
								.charsWidth(chars, 0, chars.length);
						if (this.getColumnModel().getColumn(column).getWidth() < length) {
							jc.setToolTipText(format.formatTooltip(value
									.toString()));
						} else {
	...
	protected class Model extends AbstractTableModel {
		...
		public Object getValueAt(int row, int column) {
			Opt4JTask task = (Opt4JTask) executionEnvironment.getTasks().get(
					row);
			if (column == 0) {
	...
	/**
	 * Constructs a {@code Opt4JTasksPanel}.
	 * 
	 ...
	 */
	@Inject
	public Opt4JTasksPanel(ExecutionEnvironment executionEnvironment,
			Format format, SelectedModules selectedModules) {
	...
	/**
	 * The {@code PopupListener} for the table elements.
	 */
	protected class PopupListener extends MouseAdapter {
		...
		void display(MouseEvent event) {
			...
			if (event.isPopupTrigger()) {
				try {
					...
					Opt4JTask task = (Opt4JTask) executionEnvironment
							.getTasks().get(row);
		 ...
		 */
		private void addConfigLoadMenuItems(JPopupMenu menu,
				final Opt4JTask task) {
			JMenuItem loadConfig = new JMenuItem("Restore Configuration", Icons
					.getIcon(Icons.FOLDER_ADD));
		...
		private void addControlMenuItems(JPopupMenu menu, Opt4JTask task) {
			...
			assert control != null : "Task should be in state EXECUTING but is in state "
					+ task.getState();

			JMenuItem start = new JMenuItem("Start", Icons
					.getIcon(Icons.CONTROL_START));
			JMenuItem pause = new JMenuItem("Pause", Icons
					.getIcon(Icons.CONTROL_PAUSE));
			JMenuItem stop = new JMenuItem("STOP", Icons
					.getIcon(Icons.CONTROL_STOP));
			JMenuItem terminate = new JMenuItem("Terminate", Icons
					.getIcon(Icons.CONTROL_TERM));
