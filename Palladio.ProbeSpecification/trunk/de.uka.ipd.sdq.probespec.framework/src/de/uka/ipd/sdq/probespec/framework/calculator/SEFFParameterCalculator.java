 ...
 * time stamp (first tuple element) and the value of parameter passed to a SEFF
 * (second tuple element). It needs one ProbeSet containing at least a
 * CURRENT_TIME probe and an SEFF_PARAMETER probe.
 * 
 * @author Faber
 * 
 */
public class SEFFParameterCalculator extends Calculator {

	private String probeSetID;
	private static Vector<MeasurementMetric> concreteMeasurementMetrics;

	/**
	 * Constructor for the SEFFParameterCalculator. It takes a reference of the
	 ...
	 */
	public SEFFParameterCalculator(SampleBlackboard blackboard,
			String probeSetID) {
		super(blackboard);
	...
	protected synchronized Vector<MeasurementMetric> getConcreteMeasurementMetrics() {
		if (concreteMeasurementMetrics == null) {
			...
			mm
					.setDescription("This measure represents the value of a parameter passed to a SEFF");
			mm.setMonotonic(false);
			mm.setName("SEFF parameter");
			mm.setStrongMonotonic(false);
			concreteMeasurementMetrics.add(mm);
		}
