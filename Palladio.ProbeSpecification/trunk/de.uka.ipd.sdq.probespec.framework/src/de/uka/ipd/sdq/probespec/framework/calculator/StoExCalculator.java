 ...
 * time stamp (first tuple element) and the value of an evaluated stochastic
 * expression - StoEx (second tuple element). It needs one ProbeSet containing
 * at least a CURRENT_TIME probe and an STOEX probe.
 * 
 * @author Faber
 * 
 */
public class StoExCalculator extends Calculator {

	private String probeSetID;
	private static Vector<MeasurementMetric> concreteMeasurementMetrics;

	/**
	 * Constructor for the StoExCalculator. It takes a reference of the
	 ...
	 */
	public StoExCalculator(SampleBlackboard blackboard, String probeSetID) {
		super(blackboard);
	...
	protected void execute(ProbeSetSample pss) throws CalculatorException {
		if (probeSetID.equals(pss.getProbeSetSampleID().getProbeSetID())) {
			...
			ProbeSample<?, ? extends Quantity> stoex = null;
	...
	protected synchronized Vector<MeasurementMetric> getConcreteMeasurementMetrics() {
		if (concreteMeasurementMetrics == null) {
			...
			mm
					.setDescription("This measure represents the value of the evaluated stochastic expression");
			mm.setMonotonic(false);
			mm.setName("StoEx value");
			mm.setStrongMonotonic(false);
			concreteMeasurementMetrics.add(mm);
		}
