 ...
 * the WaitingTime. It needs two ProbeSets each containing at least a
 * CURRENT_TIME probe.
 * 
 * @author Faber
 * 
 */
public class WaitingTimeCalculator extends Calculator {
	...
	/**
	 * Constructor for the WaitingTimeCalculator. It takes a reference of the
	...
	protected void execute(ProbeSetSample pss) throws CalculatorException {
		...
		if (endProbeSetID.equals(pss.getProbeSetSampleID().getProbeSetID())) {
			...
			if (startSetSample != null) {
				...
				throw new CalculatorException(
						"Could not access the corresponding startProbeSetSample.");
			}
		}
	}
