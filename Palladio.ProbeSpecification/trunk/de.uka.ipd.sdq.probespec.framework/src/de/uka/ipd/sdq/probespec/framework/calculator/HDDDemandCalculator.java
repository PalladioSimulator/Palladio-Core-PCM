 ...
 * time stamp (first tuple element) and the HDD demand of an entity (second
 * tuple element). It needs one ProbeSet containing at least a CURRENT_TIME
 * probe and an HDD_RESOURCE_DEMAND probe.
 * 
 * @author Faber
 * 
 */
public class HDDDemandCalculator extends Calculator {

	private String probeSetID;
	private static Vector<MeasurementMetric> concreteMeasurementMetrics;

	/**
	 * Constructor for the HDDDemandCalculator. It takes a reference of the
	 ...
	 */
	public HDDDemandCalculator(SampleBlackboard blackboard, String probeSetID) {
		super(blackboard);
	...
	protected void execute(ProbeSetSample pss) throws CalculatorException {
		if (probeSetID.equals(pss.getProbeSetSampleID().getProbeSetID())) {
			...
			rules[0] = new ProbeTypeMatchRule(ProbeType.HDD_RESOURCE_DEMAND);
	...
	protected synchronized Vector<MeasurementMetric> getConcreteMeasurementMetrics() {
		if (concreteMeasurementMetrics == null) {
			...
			mm
					.setDescription("This measure represents the HDD demand of the entity");
			mm.setMonotonic(false);
			mm.setName("HDD demand of the entity");
			mm.setStrongMonotonic(false);
			concreteMeasurementMetrics.add(mm);
		}
