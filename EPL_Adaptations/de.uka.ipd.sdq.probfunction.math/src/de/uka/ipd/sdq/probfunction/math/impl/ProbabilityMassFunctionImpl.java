...
public class ProbabilityMassFunctionImpl extends ProbabilityFunctionImpl
	...
	/**
	 * Adjust this PMF and the passed PMF so that both have the same domain (probabilities 
	 * for the same values).
	 * 
...
/*	private IProbabilityMassFunction makeSameDomain (IProbabilityMassFunction otherPMF){
		...
		// test for integer and double?
		// can also be Strings.. need to be the same type. 
		// If different, we need to be able to cast one number into another.  
		boolean isIntegerThis = true;
		boolean isIntegerOther = true;
		
		//store values to add, the add after iterating, then sort.
		List<ISample> valuesToAddToThis = new ArrayList<ISample>(valuesOther.size());
		List<ISample> valuesToAddToOther = new ArrayList<ISample>(valuesThis.size());
		...
		valuesToAddToThis = getAllMissingSamples(valuesThis, valuesOther);
		valuesToAddToOther = getAllMissingSamples(valuesOther, valuesThis);
	...
	/**
	 * Add all values from valuesOther to valuesThis that have not been there before. 
	 * Afterwards, valuesThis should contain all values of valuesOther (and possibly more)
	 * @param valuesThis
	 * @param valuesOther
	 * @return
	 */
	/*private List<ISample> getAllMissingSamples(List<ISample> valuesThis,
			List<ISample> valuesOther) {
		...
		for (int indexThis = 0; indexThis < valuesThis.size(); indexThis++) {
			...
			if (compareResult < 0){
				//sampleThis is smaller -> continue
			} else if (compareResult > 0){
				//sampleThis is larger -> add the other sample to results
				result.add(new SampleImpl(sampleOther.getValue(), 0));
				sampleOther = valuesOther.get(indexOther);
				indexOther++;
				//sampleThis needs to be compared again in the next iteration of the for loop. 
				indexThis--;
			} else {
				//Both have a sample for the same values, nothing to be done
				sampleOther = valuesOther.get(indexOther);
				indexOther++;			
			}
			
			if (indexOther < valuesOther.size()){
				sampleOther = valuesOther.get(indexOther);
			} else {
				break;
			}
		...
		//maybe there are still values not addressed in list valuesOther
		for (int i = indexOther; i < valuesOther.size(); i++) {
			result.add(new SampleImpl(valuesOther.get(i).getValue(),0));
		}
