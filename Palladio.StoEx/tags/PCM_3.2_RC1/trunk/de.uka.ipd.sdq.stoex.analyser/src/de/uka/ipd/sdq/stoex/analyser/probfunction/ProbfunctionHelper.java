...
public class ProbfunctionHelper {
	...
	public static ContinuousPDF createFunction(List<Expression> parameters, String type, ProbfunctionFactory probFuncFactory) {
		...
		try {
			...
			if (type.equals(LOGNORM)){
				...
				lognorm.setMu(getDoubleValue((NumericLiteral)parameters.get(0)));
	...
	private static double getDoubleValue(NumericLiteral numericLiteral) {
		if (numericLiteral instanceof DoubleLiteral){
			return ((DoubleLiteral)numericLiteral).getValue();
		} else if (numericLiteral instanceof IntLiteral){
			return ((IntLiteral)numericLiteral).getValue();
		} else {
			throw new RuntimeException("Unknown numeric literal "+numericLiteral.getClass().getName()+" encountered. Cannot handle it, aborting. Please contact developers.");
		}
	}
