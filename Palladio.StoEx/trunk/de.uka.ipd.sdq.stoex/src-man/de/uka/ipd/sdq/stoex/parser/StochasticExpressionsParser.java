		...
		case LITERAL_IntPMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt942>=1 ) { break _loop942; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
		...
		case LITERAL_EnumPMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt949>=1 ) { break _loop949; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
