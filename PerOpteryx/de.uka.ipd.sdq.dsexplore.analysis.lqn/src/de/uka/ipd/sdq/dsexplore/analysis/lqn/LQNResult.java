...
public abstract class LQNResult extends AbstractPerformanceAnalysisResult implements ILQNResult{
	...
	public LQNResult(PCMInstance pcm, LqnModelType model,UsageScenarioBasedCriterion criterion) throws AnalysisFailedException {
		super(pcm);
		try{
			this.responseTime = retrieveResponseTimeForUsageScenario(pcm, model,criterion);
			this.value = this.responseTime;
	...
	/**
	 * Can be any PCM instance, only the usage model is used. 
	 * @param pcm
	 * @param model
	 * @param criterion
	 * @return
