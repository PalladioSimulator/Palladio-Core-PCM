...
public class SimuComAnalysisResult extends AbstractPerformanceAnalysisResult implements IStatisticAnalysisResult, IPerformanceAnalysisResult {
	...
	// It is highly(!) dependent on what kind of result is needed
	...
	public SimuComAnalysisResult(ExperimentRun run, Experiment experiment, PCMInstance pcmInstance, UsageScenario usageScenario) throws AnalysisFailedException {
		...
		//As stated above THE result is the mean value by default
	...
	private void retrieveServiceResults(PCMInstance pcmInstance,
		...
		HashMap<String, AbstractCallAction> idToExternalCallMap = new HashMap<String, AbstractCallAction>(100);
		...
		for (Repository repository : respositories) {
			...
			for (EObject eObject : allExternalCallsList) {
				if (eObject instanceof ExternalCallAction){
					...
					idToExternalCallMap.put(id, new ExternalCallActionWrapper(externalCall));
				}
		...
		// also consider all EntryLevelSystemCalls
		EntryLevelSystemCall systemCall = UsagemodelFactory.eINSTANCE.createEntryLevelSystemCall();
		List<EObject> allSystemCalls = helper.getElements(pcmInstance.getUsageModel(), systemCall.eClass());
		for (EObject eObject : allSystemCalls) {
			if (eObject instanceof EntryLevelSystemCall){
				EntryLevelSystemCall sysCall = (EntryLevelSystemCall)eObject;
				String id = sysCall.getId();
				idToExternalCallMap.put(id, new EntryLevelSystemCallActionWrapper(sysCall));
			}
		...
		List<SEFFWithSensors> seffWithSensorsList = new ArrayList<SEFFWithSensors>(100);
			...
			try {
				if (sensorName.contains("CallID")){
					...
					if (sensorName.lastIndexOf(" ") < sensorName.length()-2 
							...
							&& sensorName.length() > 2){
						externalCallID = sensorName.substring(sensorName.lastIndexOf(" ")+1, sensorName.length()-1);
					}
				} else if (sensorName.contains("EntryLevelSystemCall id")){
					//current sensor is an EntryLevelSystemCall sensor

					// parse EntryLevelSystemCall id
					if (sensorName.lastIndexOf("id: ") < sensorName.length()-2 
							&& sensorName.lastIndexOf("id: ") > -1
							&& sensorName.length() > 2){
						externalCallID = sensorName.substring(sensorName.lastIndexOf("id: ")+4, sensorName.length()-1).trim();
					}
				}
			} catch (StringIndexOutOfBoundsException e){
				logger.warn("Cannot retrieve call id from sensor. Possibly sensor name labels must have changed. Contact developers if this sensor is needed. Sensor: "+sensorName);
			}
			
			if (externalCallID != ""){	
				// 	find the external call in the model
				AbstractCallAction myCall = idToExternalCallMap.get(externalCallID);

				if (myCall != null){

					AssemblyContextContext callingComponentAssemblyContext = myCall.getCallingComponentAssemblyContext(sensorName, idToAssemblyContextMap);

					if (callingComponentAssemblyContext != null){

						SEFFWithSensors seffWithSensors = getCalledSeffForExternalCallAction(
								pcmInstance, callingComponentAssemblyContext, myCall, idToAssemblyContextMap);

						//reuse pair if it already there
						int index = seffWithSensorsList.indexOf(seffWithSensors);
						if (index >= 0){
							seffWithSensors = seffWithSensorsList.get(index);
						} else {
							seffWithSensorsList.add(seffWithSensors);
						}

						// map all result sensors to the SEFF they call
						seffWithSensors.addSensor(sensor);
						continue;
					} 
				} else {
				...
				logger.warn("Cannot retrieve call id from sensor. If this is a completion component, this is ok. Otherwise, sensor name labels must have changed. Contact developers if this sensors is needed. Sensor: "+sensorName);
			}
		...
		ArrayList<SEFFWithSensors> arrayVersionOfList = new ArrayList<SEFFWithSensors>();
		arrayVersionOfList.addAll(seffWithSensorsList);
		
		
		
		for (SEFFWithSensors seffWithSensors : seffWithSensorsList) {
			List<Sensor> mySensors = seffWithSensors.getSensors();
			...
			if (mySensors.size() > 0){
				...
				myServiceResult.setServiceEffectSpecification_ServiceResult(seffWithSensors.getSEFF());
				myServiceResult.setAllocationContext_AllocationServiceResult(seffWithSensors.getAllocationContext());
				...
				for (Sensor sensor : mySensors) {
					 ...
					 double mean = calculateValue(results, "mean");
					 if (!Double.isNaN(mean)){
						 totalNumberOfMeasurements += measurements.size();
						 weightedAndCumulatedMeanResponseTime += mean * totalNumberOfMeasurements;
					} else {
						logger.warn("Encountered value NaN in sensor "+sensor.getSensorName()+" "+sensor.getSensorID());
					}
				}
	...
	private AllocationContext findAllocationContext(
			AssemblyContextContext myAssemblyContext, PCMInstance pcmInstance) {
		List<AllocationContext> allocationContexts = pcmInstance.getAllocation().getAllocationContexts_Allocation();
		for (AllocationContext allocationContext : allocationContexts) {
			if (de.uka.ipd.sdq.dsexplore.helper.EMFHelper.checkIdentity(allocationContext.getAssemblyContext_AllocationContext(), myAssemblyContext.getAssemblyContext())){
				return allocationContext;
			}
		}
		// this assembly context is not allocated, check parents recursively
		AllocationContext allocContext = findAllocationContext(myAssemblyContext.getParentAssemblyContext(), pcmInstance);
		if (allocContext == null){
			// could not find any context, strange!
			throw new RuntimeException("Could not find Allocation Context for "+myAssemblyContext.getAssemblyContext().getEntityName());
		}
		return allocContext;
			
	}

	private SEFFWithSensors getCalledSeffForExternalCallAction(
			PCMInstance pcmInstance,
			AssemblyContextContext currentContext,
			AbstractCallAction externalCallAction, Map<String, AssemblyContextContext> idToAssemblyContextMap) {
		...
		while (currentContext != null && currentContext.getAssemblyContext() != null) {
		...
		ServiceEffectSpecification seff = externalCallAction.getNextSeff(contextWrapper);
		List<AssemblyContext> providingAssemblyContextHierarchy = externalCallAction.findProvidingAssemblyContexts(contextWrapper, false);
		AssemblyContext calledAssemblyContext = providingAssemblyContextHierarchy.get(providingAssemblyContextHierarchy.size()-1);
		
		AssemblyContextContext calledComponentAssemblyContextContext = idToAssemblyContextMap.get(calledAssemblyContext.getId());
		AllocationContext allocContext = findAllocationContext(calledComponentAssemblyContextContext, pcmInstance);
		SEFFWithSensors seffWithSensors = new SEFFWithSensors(seff, calledComponentAssemblyContextContext, allocContext);
		
		return seffWithSensors;
	}
		...
		if (utilSensor != null /*&& waitSensor != null*/){
			...
			try {
				...
				*/
				...
				resultToFill.setDemandedTime(calculateValue(sam, "mean"));
...
class SEFFWithSensors {
	...
	public SEFFWithSensors(ServiceEffectSpecification e, AssemblyContextContext myAssemblyContext, AllocationContext allocationContext){
		this.seff = e;
	...
	public AssemblyContextContext getAssemblyContextContext() {
	...
	public boolean equals(Object o){
		...
		if (o instanceof SEFFWithSensors){
			SEFFWithSensors other = (SEFFWithSensors)o;
			return de.uka.ipd.sdq.dsexplore.helper.EMFHelper.checkIdentity(other.getAllocationContext(), this.allocationContext)
				&& de.uka.ipd.sdq.dsexplore.helper.EMFHelper.checkIdentity(other.getSEFF().getDescribedService__SEFF(), this.seff.getDescribedService__SEFF())
				&& de.uka.ipd.sdq.dsexplore.helper.EMFHelper.checkIdentity(other.getSEFF().getBasicComponent_ServiceEffectSpecification(), this.seff.getBasicComponent_ServiceEffectSpecification());
		} else {
			return false;
		}
...
/**
 * Represents the {@link AssemblyContext} hierarchy in which a component or call is embedded. An null {@link AssemblyContext} means that the context is the usage model. 
 * @author martens
 *
 */
class AssemblyContextContext {
	
	/** null here means that the context is the usage model */
	...
	/**
	 * May return null if the context is the usage model. 
...
abstract class AbstractCallAction { 
	...
	public abstract ServiceEffectSpecification getNextSeff(ContextWrapper contextWrapper);
	
	public abstract AssemblyContextContext getCallingComponentAssemblyContext(String sensorName,
			Map<String, AssemblyContextContext> idToAssemblyContextMap);

	public abstract List<AssemblyContext> findProvidingAssemblyContexts(ContextWrapper contextWrapper, boolean b);
	
}

class ExternalCallActionWrapper extends AbstractCallAction {
	
	private ExternalCallAction externalCallAction;


	public ExternalCallActionWrapper(ExternalCallAction externalCall) {
		this.externalCallAction = externalCall;
	}
	
	public List<AssemblyContext> findProvidingAssemblyContexts(
			ContextWrapper contextWrapper, boolean b) {
		return contextWrapper.findProvidingAssemblyContexts(externalCallAction, false);
	}



	public ServiceEffectSpecification getNextSeff(
			ContextWrapper contextWrapper) {
		return contextWrapper.getNextSEFF(externalCallAction);
	}

	@Override
	public AssemblyContextContext getCallingComponentAssemblyContext(
			String sensorName,
			Map<String, AssemblyContextContext> idToAssemblyContextMap) {
		...
		AssemblyContextContext callingComponentAssemblyContext = idToAssemblyContextMap.get(assemblyContextID);
		return callingComponentAssemblyContext;
	}
	
}

class EntryLevelSystemCallActionWrapper extends AbstractCallAction {
	private EntryLevelSystemCall userCallAction;
	
	public EntryLevelSystemCallActionWrapper(EntryLevelSystemCall userCallAction) {
		this.userCallAction = userCallAction;
	}

	@Override
	public ServiceEffectSpecification getNextSeff(ContextWrapper contextWrapper) {
		return contextWrapper.getNextSEFF(userCallAction);
	}

	@Override
	public List<AssemblyContext> findProvidingAssemblyContexts(
			ContextWrapper contextWrapper, boolean b) {
		return contextWrapper.getFirstAssemblyContext2(userCallAction);
	}
	...
	public AssemblyContextContext getCallingComponentAssemblyContext(
		...
		return new AssemblyContextContext(null);
	}
}
