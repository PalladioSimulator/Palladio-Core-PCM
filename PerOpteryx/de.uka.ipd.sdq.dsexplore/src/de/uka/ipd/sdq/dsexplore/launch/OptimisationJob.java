	...
	public void execute(IProgressMonitor monitor) throws JobFailedException,
	    ...
	    if (this.dseConfig.isOptimise()){
	    	if (this.dseConfig.getMaxIterations() == 0 && dseConfig.hasPredefinedInstances()){
	    		...
	    		PredefinedInstanceEvaluator gie = new GivenInstanceEvaluator(this.dseConfig,Opt4JStarter.getProblem().getEMFProblem());
	    		gie.start();

	    	} else if (dseConfig.isRandomSearch()){
	    		PredefinedInstanceEvaluator rie = new RandomSearch(this.dseConfig,Opt4JStarter.getProblem().getEMFProblem());
	    		rie.start();
