		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof EnumerationChoice) {
				...
				if (enumerationChoice.getDegreeOfFreedom() instanceof AllocationDegree) {
					...
					if (EMFHelper.checkIdentity(enumerationChoice.getDegreeOfFreedom()
							.getChangeableEntity(),result.getFirst().ac)) {
		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof DiscreteRangeChoice) {
				...
				if (schedChoice.getDegreeOfFreedom() instanceof CapacityDegree) {
					...
					if (EMFHelper.checkIdentity(schedChoice.getDegreeOfFreedom()
