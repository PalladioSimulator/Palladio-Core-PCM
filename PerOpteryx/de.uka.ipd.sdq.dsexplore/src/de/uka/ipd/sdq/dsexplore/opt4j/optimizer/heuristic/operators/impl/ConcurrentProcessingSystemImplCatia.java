		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof ClassChoice) {
				...
				if (ClassChoice.getDegreeOfFreedom() instanceof AllocationDegree) {
					...
					if (ClassChoice.getDegreeOfFreedom()
							.getPrimaryChanged() == result
							.getFirst().ac) {

						// if (ClassChoice.getDegreeOfFreedom().g)

						ClassChoice.setEntity(result
								.getSecond());
	...
	private TacticsResultCandidate createUpdatedSchedulingCandidate(
		...
		for (Choice choice : candidate.getGenotype()) {
			if (choice instanceof SchedulingPolicyChoice) {
				...
				if (ClassChoice.getDegreeOfFreedom() instanceof SchedulingPolicyDegree) {

					SchedulingPolicyDegree schedDegree = (SchedulingPolicyDegree)ClassChoice.getDegreeOfFreedom();
