			...
			if (choice instanceof EnumerationChoice) {
				...
				if (enumerationChoice.getDegreeOfFreedom() instanceof AllocationDegree) {
					if (iterationCounter == allocationContext) {
						...
						DegreeOfFreedom dof = enumerationChoice.getDegreeOfFreedom();
						if (dof instanceof AllocationDegree){
							List<Entity> domain = ((AllocationDegree) dof).getDomainOfEntities();
							for (Entity entity : domain) {
								if (entity.getId().equals(container.getId())){
									enumerationChoice.setEntity(container);
