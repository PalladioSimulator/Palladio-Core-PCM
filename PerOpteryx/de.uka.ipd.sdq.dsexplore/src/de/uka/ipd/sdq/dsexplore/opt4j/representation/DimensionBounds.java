...
class DimensionBounds implements Bounds<Double> {
	...
	public Double getLowerBound(int index) {
		...
		} else if (ClassDegree.class.isInstance(this.problem.getDesigndecision().get(index))){
	...
	private Double getUpperEnumerationBound(int index) {
		...
		if (ClassDegree.class.isInstance(degree)){
			upperEnumerationBound = ((ClassDegree)degree).getDesignOptions().size()-1;
		} else {
	...
	public boolean isInteger(int index){
		return (
				ClassDegree.class.isInstance(this.problem.getDesigndecision().get(index))
