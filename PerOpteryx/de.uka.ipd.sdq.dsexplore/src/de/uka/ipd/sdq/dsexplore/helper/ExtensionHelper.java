...
public class ExtensionHelper {
	 ...
	 * @return all analysis method extensions
	 */
	public static List<IExtension> loadAnalysisExtensions(String qualityAttributeId) {
		...
		for (IExtension extension : exts) {
			if (qualityAttributeId.equals(loadQualityAttributeDimension(extension).getId())){
	 ...
	 * @deprecated use loadQualityAttributeDimension
	 */
	@Deprecated
	...
	/**
	 * Loads the dimension of a single analysis method represented by the passed
	 * extension.
	 * 
	 * @param extension
	 * @return the analysis method's dimension; null if the passed extension is not
	 *         an analysis extension or the attribute is not set.
	 */
	private static Dimension loadQualityAttributeDimension(IExtension extension) {
		IConfigurationElement[] elements = extension.getConfigurationElements();
		for (IConfigurationElement element : elements) {
			if (element.getName().equals("analysis")) {
				//return element.getAttribute("qualityAttribute");
				try {
					IAnalysisQualityAttribute attribute = (IAnalysisQualityAttribute)element.createExecutableExtension("qualityAttributeHandler");
