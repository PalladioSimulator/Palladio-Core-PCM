...
public class DSEEvaluator implements Evaluator<PCMPhenotype>{
	...
	public void retainOnlyEvaluatorsFor(List<List<CriterionAndEvaluator>> orderedObjectives) {
		
		List<CriterionAndEvaluator> flatList = new ArrayList<CriterionAndEvaluator>(orderedObjectives.size()*3);
		for (List<CriterionAndEvaluator> list : orderedObjectives) {
			flatList.addAll(list);
		}
		
		this.objectives.retainAll(flatList);
