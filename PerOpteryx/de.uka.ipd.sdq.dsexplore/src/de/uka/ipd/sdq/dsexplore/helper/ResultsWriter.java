...
public class ResultsWriter {
	...
	private static StringBuilder addResultsToString(Collection<DSEIndividual> individuals,
			List<Exception> exceptionList) {
	...
	private static void writeToFile(String filename, StringBuilder results, int iteration, List<Exception> exceptionList) {
		filename = filename +iteration+"_" +getTimeDateString()+".csv";
		try {
	...
	public static void writeStringToFile(String filename, String content, int iteration, List<Exception> exceptionList){
		writeToFile(filename, new StringBuilder(content), iteration, exceptionList);
	}
	...
	private static StringBuilder prettyPrintResultLineCSV(StringBuilder output, DSEIndividual ind, List<Objective> objectivesWithConfidence) {
		...
		output.append(ind.hashCode()+";");
		
		if (ind instanceof TacticsResultCandidate){
			output.append(((TacticsResultCandidate) ind).getParent().hashCode());
		}
	...
	private static StringBuilder prettyPrintHeadlineCSV(
		...
		if (individuals.size() > 0){
			...
			output.append("Candidate ID;Parent ID if applicable");
	...
	public void writeTacticCandidateInfo(ITactic heuristic, Collection<TacticsResultCandidate> candidatesFromCurrentHeuristic){
		...
		for (TacticsResultCandidate tacticsResultCandidate : candidatesFromCurrentHeuristic) {
			...
			builder.append(heuristic.getClass().getSimpleName()+";"+tacticsResultCandidate.hashCode()+";"+tacticsResultCandidate.getParent().hashCode()+";"+tacticsResultCandidate.getID()+";"+tacticsResultCandidate.getParent().getID()+";"+tacticsResultCandidate.getInfo()+";");
