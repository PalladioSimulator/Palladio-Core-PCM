...
public class MarkovUsageModelVisitor extends UsagemodelSwitch<MarkovChain> {
	 ...
	 */
	public MarkovUsageModelVisitor(
			final MarkovTransformationSource transformationState) {
		this(transformationState, false, true, false);
	}
	 ...
	 * @param optimize
	 *            controls if Markov Chain reduction shall be performed during
	 *            transformation
	 * @param recordTraces
	...
	public MarkovChain caseEntryLevelSystemCall(final EntryLevelSystemCall call) {
		...
		if (contextWrapper == null) {
			contextWrapper = new ContextWrapper(call, transformationState
					.getModel());
		} else {
			contextWrapper = contextWrapper.getContextWrapperFor(call);
		}
		...
		if (seff != null) {
			MarkovSeffVisitor seffVisitor = new MarkovSeffVisitor(
					transformationState, contextWrapper, prefixes,
					simplifiedStateHandling, optimize, recordTraces);
	...
	public MarkovChain caseScenarioBehaviour(
		...
		// Logging & naming:
		String name = scenarioBehaviour.getEntityName() + "["
		...
		while (action != null) {
			actions.add(action);
			actionNames
					.add(action.getEntityName() + "[" + action.getId() + "]");
		...
		MarkovChain aggregateMarkovChain = markovBuilder
				.initSequentialMarkovChain(prefixes, actionNames, states);
