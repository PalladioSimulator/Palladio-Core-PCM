...
public class MarkovBuilder {
	...
	/**
	 * Key to identify a Markov label as holding failure type information.
	 */
	private static final String FAILURETYPEINFO = "FailureTypeInfo";
	...
	public String getFailureTypeLabelValue(final State state) {
		for (Label label : state.getLabels()) {
			if (label.getKey().equals(FAILURETYPEINFO)) {
	...
	private State addStateForFailureDescription(final MarkovChain chain,
		...
		failureLabel.setKey(FAILURETYPEINFO);
	...
	private void appendFailureHandlingChain(final MarkovChain aggregateChain,
		...
		List<State> handlingChainFailureStates = getFailureStates(handlingChainCopy);
		...
		// // Absorb failure states. for already existing failure type states
		// // delegate transitions from inner duplicates:
		// List<State> duplicateFailureStates = new ArrayList<State>();
		// for (State innerFailureState : handlingChainFailureStates) {
		// // check if this is a duplicate
		// State existingFailureState = findMatchingFailureState(
		// aggregateChainFailureStates, innerFailureState);
		// if (existingFailureState != null) { // duplicate found
		// // delegate from duplicate to existing state
