...
public class AggregatedContextSEFFVisitor extends SeffVisitor {
	...
	private CommunicationLinkResourceSpecification findCommunicationLink(
		...
		if (to != null){
			...
			for (LinkingResource linkingResource : linkingResourceList) {
				// must not use equals here
				boolean containsFromContainer = false;
				boolean containsToContainer = false;
				for (ResourceContainer container : linkingResource.getConnectedResourceContainers_LinkingResource()) {
					if (container.getId().equals(fromResourceContainer.getId())){
						containsFromContainer = true;
					}
					if (container.getId().equals(toResourceContainer.getId())){
						containsToContainer = true;
					}
				}
				if (containsFromContainer && containsToContainer){
	...
	private boolean isOrContainsAssemblyContext(AssemblyContext currenAssemblyContext, AssemblyContext targetAssembly) {
		...
		RepositoryComponent component = currenAssemblyContext.getEncapsulatedComponent_AssemblyContext();
		if (component instanceof CompositeComponent){
			CompositeComponent composite = (CompositeComponent)component;
			for (AssemblyContext containedAssemblyContext : composite.getAssemblyContexts_ComposedStructure()) {
	...
	public Object caseInternalAction(InternalAction action) {
		...
		for (ParametricResourceDemand demand : action.getResourceDemand_Action()) {
			...
			Expression solvedExpr = ExpressionHelper.getSolvedExpressionWithoutPMFs(specification, this.contextWrapper);
		...
		if (container instanceof AbstractBranchTransition){
			...
			for (LoopIteration loopIteration : loopIterations) {
				if (loop.equals(loopIteration.getLoopaction_LoopIteration())){
					avgLoopIterationNumber = ExpressionHelper.meanValue(ExpressionHelper.getSolvedExpressionWithoutPMFs(loopIteration.getSpecification_LoopIteration().getSpecification(), this.contextWrapper));
				}
			}
