...
public class Pcm2MarkovStrategy implements SolverStrategy {
	...
	private boolean createNextPermutation(
		...
		MarkovProcessingResourceState tmp; // used for step 4 (see below)
		...
		for (int i = 0; i < size - 1; i++) {
			if (descriptors.get(i).getCurrentState() == MarkovProcessingResourceState.NA
					&& descriptors.get(i + 1).getCurrentState() == MarkovProcessingResourceState.OK) {
				k = i;
			}
		}
		if (k == -1) {
		...
		for (int i = k + 2; i < size; i++) {
			if (descriptors.get(i).getCurrentState() == MarkovProcessingResourceState.OK) {
	...
	private void initializeResourceStates(
		...
		for (int i = 0; i < m; i++) {
			descriptors.get(i)
					.setCurrentState(MarkovProcessingResourceState.NA);
		}
	...
	private void initSensitivityAnalysis() {
		...
		MarkovSensitivity s2 = new ResourceMTTFSensitivity(
				...
				Arrays.asList(new Double[]{1080000.0,108000.0}), // MTTF values
				"C://temp//MTTFSensitivityResult.txt"); // log file
		...
		MarkovSensitivity s4 = new MultiSensitivity(
				...
				false, // isCombinatory
				"C://temp//TripleSensitivityResult.txt"); // log file
		...
		sensitivity = new MultiSensitivity(
				...
				true, // isCombinatory
				"C://temp//BRSSensitivityResult.txt"); // log file
	}
