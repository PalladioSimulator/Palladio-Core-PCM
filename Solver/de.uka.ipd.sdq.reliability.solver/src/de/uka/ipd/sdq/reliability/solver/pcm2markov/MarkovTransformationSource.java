 ...
 * 
 * Holds a PCM instance as basis for the transformation, and keeps track of the
 * physical system states (i.e., states of processing resources) that are
 * considered during the transformation.
 * 
 * @author brosch
 * 
 */
public class MarkovTransformationSource {
	...
	private static Logger logger = Logger
			.getLogger(MarkovTransformationSource.class.getName());
	...
	/**
	 * The list of descriptors for processing resources which have a failure
	 * probability greater than zero.
	 */
	private List<ProcessingResourceDescriptor> unreliableResourceDescriptors;

	/**
	 * Initializes a PCMInformationProvider and creates the corresponding
	 ...
	 */
	public MarkovTransformationSource(final PCMInstance model,
			final boolean sortDescriptors) {
		this.model = model;
	...
	public void printCurrentResourceStates() {
		for (int i = 0; i < unreliableResourceDescriptors.size(); i++) {
			System.out.print(unreliableResourceDescriptors.get(i)
					.getCurrentState() == MarkovProcessingResourceState.OK ? "1"
					: "0");
		}
		System.out.println();
	}
	 ...
	 * 
	 * @param sort
	 *            indicates if the result list shall be sorted according to
	 *            state probabilities
	...
	private List<ProcessingResourceDescriptor> buildResourceDescriptors(
		...
		Collection<ResourceContainer> resourceContainers = model
		...
		for (ResourceContainer container : resourceContainers) {
			...
			for (ProcessingResourceSpecification resource : container
				...
				descriptor.setStateProbability(MarkovProcessingResourceState.OK,
						resourceMTTF / (resourceMTTF + resourceMTTR));
				descriptor.setStateProbability(MarkovProcessingResourceState.NA,
	...
	/**
	 * Builds a list of resource descriptors whose resources can fail, i.e.,
	 * have a greater than 0 probability for the N/A state .
	 * 
	...
	private List<ProcessingResourceDescriptor> buildUnreliableResourceDescriptors() {
		...
		for (int i = 0; i < resourceDescriptors.size(); i++) {
			...
			if (resourceDescriptors.get(i).getStateProbability(
					MarkovProcessingResourceState.NA) != 0d) {
				unreliableResourceDescriptorsList.add(resourceDescriptors
						.get(i));
			}
		}
	...
	private void sort(final List<ProcessingResourceDescriptor> descriptors) {
		/*
		 * Sort the list of descriptors in a way that states, whose failure
		 * probabilities are higher, appear first in the list, successively
		 * succeeded by those, whose failure probabilities are lower. I.e., the
		 * list is sorted in descending order according to failure
		 * probabilities.
		 */
		Collections.sort(descriptors,
				new Comparator<ProcessingResourceDescriptor>() {
					public int compare(final ProcessingResourceDescriptor o1,
							final ProcessingResourceDescriptor o2) {
						if (o1.getStateProbability(MarkovProcessingResourceState.NA) > o2
								.getStateProbability(MarkovProcessingResourceState.NA)) {
							return -1;
						} else if (o1
								.getStateProbability(MarkovProcessingResourceState.NA) < o2
								.getStateProbability(MarkovProcessingResourceState.NA)) {
							return 1;
						} else {
							return 0;
