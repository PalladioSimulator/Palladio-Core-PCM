...
namespace Palladio.Webserver.HTTPRequestParser
{
	...
	public class HTTPRequestParser : IRequestParser
	{
			...
			try 
			{
				while(true) 
				{
					...
					if(buffer.IndexOf("Content-Length:") != -1)
					{
						...
						BinaryReader binReader = new BinaryReader(request.NetworkStream);

						int read = binReader.Read(byteBuffer,start,rest);
						rest = contentLength - read;


						Console.WriteLine("Read Bytes: "+read);
						Console.WriteLine("Missed Bytes: "+rest);
						...
						buffer = Encoding.ASCII.GetString(byteBuffer);

						webserverMonitor.WriteLogEntry("Got request; size (bytes): " + contentLength);
						contentRead = true;
					}
		...
		{							
			...
			if (requestStringBuffer.IndexOf("multipart/form-data")>0)
			{
				// Extract the request-part, that contains the variables-names and values:
				int boundaryStart = requestStringBuffer.IndexOf("boundary=");
				int boundaryEnd = requestStringBuffer.IndexOf("\n",boundaryStart);
				string boundary = requestStringBuffer.Substring(boundaryStart+9, boundaryEnd-boundaryStart-9);

				boundaryStart = requestStringBuffer.IndexOf(boundary, boundaryEnd)+boundary.Length;
				boundaryEnd = requestStringBuffer.IndexOf(boundary,boundaryStart);

				string content = requestStringBuffer.Substring(boundaryStart,boundaryEnd-boundaryStart-4);

				int s = content.IndexOf("filename=");
				int e = content.IndexOf("\"\r\n",s);
				string filename =  content.Substring(s+10,e-s-10);
				s = content.IndexOf("\r\n\r\n");
				string fileContent = content.Substring(s+4);

				httpRequest.SetPOSTVariableValue("fileContent",fileContent);
				httpRequest.SetPOSTVariableValue("fileName",filename);

				s = requestStringBuffer.IndexOf(boundary);
				s = requestStringBuffer.IndexOf(boundary, s + boundary.Length);
				requestStringBuffer = requestStringBuffer.Substring(0,s);
