...
public class ConstructorRole extends Role {
    ...
    public ConstructorRole(String role_name, String identifier, DesignPatternDescription dp_description) {
        super(role_name, identifier, dp_description);
    }
    
    public Collection evaluate(Collection candidate_list) {
        Collection constructor_list = new Vector();
       
        // select only constructors !!!
        
        Iterator itr = candidate_list.iterator();
        
        while(itr.hasNext()) {
            Object candidate = itr.next();
            
            if (candidate instanceof Constructor) {
                constructor_list.add(candidate);
            }
        }
        
        return super.evaluate(constructor_list);
    }
