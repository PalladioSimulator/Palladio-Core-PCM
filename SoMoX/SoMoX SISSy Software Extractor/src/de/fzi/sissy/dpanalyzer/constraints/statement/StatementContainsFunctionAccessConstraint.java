...
public class StatementContainsFunctionAccessConstraint extends
        StatementConstraint implements RoleConstraint, IdentifierConstraint {

    public StatementContainsFunctionAccessConstraint(String check_object_identifier, FunctionAccessRole contained_functionaccess_role) {
        super(check_object_identifier, contained_functionaccess_role);
    }

    public StatementContainsFunctionAccessConstraint(String check_object_identifier, String contained_functionaccess_identifier) {
    ...
    public Collection getCandidateCollection(Object check_object) {
        if (check_object instanceof de.fzi.sissy.metamod.Statement)
            return MetamodRetrievalEngineImplementation.getSingleton().getContainedFunctionAccessesOfStatement((de.fzi.sissy.metamod.Statement)check_object);
        else
            return new Vector();
    }

    public boolean evaluateByIdentifier(Object check_object, Object evaluation_object) {
        if ((check_object instanceof de.fzi.sissy.metamod.Statement)&&(evaluation_object instanceof de.fzi.sissy.metamod.FunctionAccess))
            return MetamodRetrievalEngineImplementation.getSingleton().statementContainsFunctionAccess((de.fzi.sissy.metamod.Statement)check_object, (de.fzi.sissy.metamod.FunctionAccess)evaluation_object);
        else
            return false;
    }
