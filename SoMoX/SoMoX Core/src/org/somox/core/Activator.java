...
import org.somox.controller.impl.SoMoXCoreListenerImpl;
...
public class Activator extends Plugin {
	...
	private int connectCoreControllerExtensions() {
		SoMoXCoreControllerListener controllerListener = new SoMoXCoreListenerImpl(this.somoxCore);
		IExtensionRegistry registry = Platform.getExtensionRegistry();
		...
		for (; i < extensions.length; i++){
			...
			if(controller != null){
				controller.register(controllerListener);
			}
		}
		return i;
	}
	...
	private SoMoXCoreController buildController(IConfigurationElement[] configurations) {
		...
		for(int i = 0; i < configurations.length; i++){
			...
			SoMoXCoreLogger.logInfo("contoller loaded");
		}
	...
	private SoftwareExtractor buildSoftwareExtractor(IConfigurationElement[] configurations) {
		...
		for(int i = 0; i < configurations.length; i++){
			...
			SoMoXCoreLogger.logInfo("extractor loaded");
		}
	...
	private ModelAnalyzer buildModelAnalyzer(IConfigurationElement[] configurations) {
		...
		for(int i = 0; i < configurations.length; i++){
			if(EXTENSION_POINT_ID_MODELANALYZER.equals(configurations[i].getName())){
				...
				SoMoXCoreLogger.logInfo("Tried to build a model analyzer from a non model analyzer plug-in: "+configurations[i].getName());
			}
		}
