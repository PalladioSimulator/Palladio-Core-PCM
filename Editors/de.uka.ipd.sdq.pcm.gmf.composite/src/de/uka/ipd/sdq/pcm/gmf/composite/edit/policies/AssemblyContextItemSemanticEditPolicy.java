...
public class AssemblyContextItemSemanticEditPolicy extends
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (PalladioComponentModelElementTypes.OperationProvidedRole_3007 == req
				.getElementType()) {
			return getGEFWrapper(new OperationProvidedRoleCreateCommand(req));
		}
		if (PalladioComponentModelElementTypes.OperationRequiredRole_3008 == req
				.getElementType()) {
			return getGEFWrapper(new OperationRequiredRoleCreateCommand(req));
		}
		if (PalladioComponentModelElementTypes.SourceRole_3013 == req
				.getElementType()) {
			return getGEFWrapper(new SourceRoleCreateCommand(req));
		}
		if (PalladioComponentModelElementTypes.SinkRole_3014 == req
				.getElementType()) {
			return getGEFWrapper(new SinkRoleCreateCommand(req));
		}
		if (PalladioComponentModelElementTypes.InfrastructureProvidedRole_3015 == req
				.getElementType()) {
			return getGEFWrapper(new InfrastructureProvidedRoleCreateCommand(
					req));
		}
		if (PalladioComponentModelElementTypes.InfrastructureRequiredRole_3016 == req
				.getElementType()) {
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Object p : getHost().getChildren()) {
			if (p instanceof BorderedBorderItemEditPart) {
				...
				if (borderItem.getModel() != null
						&& borderItem.getModel() instanceof View) {
					allEdges.addAll(((View) borderItem.getModel())
							.getSourceEdges());
					allEdges.addAll(((View) borderItem.getModel())
							.getTargetEdges());
				}
		...
		for (Iterator nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			case SinkRoleEditPart.VISUAL_ID:
				for (Iterator it = node.getTargetEdges().iterator(); it
					...
					Edge incomingLink = (Edge) it.next();
					if (PalladioComponentModelVisualIDRegistry
							.getVisualID(incomingLink) == AssemblyEventConnectorEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								incomingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
