...
/**
 * Command to Configure an emit event action with the appropriate event type and
 * source role.
 * 
 * This configuration command takes use of the basic commands provided by the
 * EMF environment like setValueCommand() etc.
 * 
...
public class EmitEventActionConfigureCommand extends ConfigureElementCommand {
	 ...
	 */
	public EmitEventActionConfigureCommand(ConfigureRequest request,
			EventType eventType, SourceRole sourceRole) {
		super(request);
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor,
		...
		CommandResult commandResult = setEventTypeEmitEventAction(monitor, info);
		if (!isOK(commandResult)) {
			return CommandResult
					.newErrorCommandResult("Set EventType for the EmitEventAction failed!");
		}
		commandResult = setSourceRoleEmitEventAction(monitor, info);
		if (!isOK(commandResult)) {
			return CommandResult
					.newErrorCommandResult("Set SourceRole for the EmitEventAction failed!");
		}
	 ...
	 *             indicating any problems during the EmitEventAction
	 *             configuration.
	 */
	private CommandResult setEventTypeEmitEventAction(IProgressMonitor monitor,
			IAdaptable info) throws ExecutionException {

		ICommand cmd = new SetValueCommand(new SetRequest(request
				.getElementToConfigure(), SeffPackage.eINSTANCE
				.getEmitEventAction_EventType__EmitEventAction(), eventType));
	 ...
	 *             configuration.
	 */
	private CommandResult setSourceRoleEmitEventAction(
			IProgressMonitor monitor, IAdaptable info)
			throws ExecutionException {

		ICommand cmd = new SetValueCommand(new SetRequest(request
				.getElementToConfigure(), SeffPackage.eINSTANCE
				.getEmitEventAction_SourceRole__EmitEventAction(), sourceRole));
