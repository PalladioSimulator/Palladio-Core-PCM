...
public class CompleteComponentTypeItemSemanticEditPolicy extends
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (PalladioComponentModelVisualIDRegistry
					.getVisualID(incomingLink) == ImplementationComponentTypeParentCompleteComponentTypesEditPart.VISUAL_ID) {
		...
		for (Iterator it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (PalladioComponentModelVisualIDRegistry
					.getVisualID(outgoingLink) == InfrastructureProvidedRoleEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						outgoingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
			...
			if (PalladioComponentModelVisualIDRegistry
					.getVisualID(outgoingLink) == InfrastructureRequiredRoleEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						outgoingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
			...
			if (PalladioComponentModelVisualIDRegistry
					.getVisualID(outgoingLink) == CompleteComponentTypeParentProvidesComponentTypesEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (PalladioComponentModelElementTypes.InfrastructureProvidedRole_4111 == req
				.getElementType()) {
			return getGEFWrapper(new InfrastructureProvidedRoleCreateCommand(
					req, req.getSource(), req.getTarget()));
		}
		if (PalladioComponentModelElementTypes.InfrastructureRequiredRole_4112 == req
				.getElementType()) {
			return getGEFWrapper(new InfrastructureRequiredRoleCreateCommand(
					req, req.getSource(), req.getTarget()));
		}
	...
	protected Command getReorientRelationshipCommand(
		...
		case InfrastructureProvidedRoleEditPart.VISUAL_ID:
			return getGEFWrapper(new InfrastructureProvidedRoleReorientCommand(
					req));
		case InfrastructureRequiredRoleEditPart.VISUAL_ID:
			return getGEFWrapper(new InfrastructureRequiredRoleReorientCommand(
					req));
