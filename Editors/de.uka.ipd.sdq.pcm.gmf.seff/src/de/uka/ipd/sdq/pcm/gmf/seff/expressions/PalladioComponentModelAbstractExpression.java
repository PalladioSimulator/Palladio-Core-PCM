...
public abstract class PalladioComponentModelAbstractExpression {
	...
	protected void setStatus(int severity, String message, Throwable throwable) {
		...
		if (!this.status.isOK()) {
			PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Expression problem:" + message + "body:" + body(), throwable); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
	...
	public Object evaluate(Object context, Map env) {
		if (context().isInstance(context)) {
			try {
				...
				PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Expression evaluation failure: " + body(), e); //$NON-NLS-1$
			}
		}
		return null;
	}
