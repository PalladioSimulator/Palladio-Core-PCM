...
public class PalladioComponentModelValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	 ...
	 * @generated
	 */
	private static final String MARKER_TYPE = PalladioComponentModelSeffDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$ //$NON-NLS-2$
	/**
	 * @generated
	 */
	...
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof org.eclipse.gmf.runtime.diagram.ui.editparts.GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof SeffDiagramEditor) {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && ResourceDemandingSEFFEditPart.MODEL_ID.equals(PalladioComponentModelVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		public void refresh() {
			...
			try {
				...
				PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getType(IMarker marker) {
			try {
				...
				PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
