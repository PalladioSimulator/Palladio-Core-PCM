...
public class AddCommunicationLinkResourceSpecificationEditHelperAdvice extends
	...
	public ICommand getBeforeConfigureCommand(ConfigureRequest request) {
		...
		ICommand commSpecCommand = new MySetValueCommand(setRequest);
		commSpecCommand.setLabel("commSpecCommand");
		
		// Latency and Throughput
		LinkingResource linkingResource = (LinkingResource)request.getElementsToEdit().get(0);
		...
		TransactionalEditingDomain domain = TransactionUtil.getEditingDomain(communicationLinkResourceSpecification);
		AddLatencyAndThroughputRecordingCommand addLatencyAndThroughputRecordingCommand = new AddLatencyAndThroughputRecordingCommand(domain, communicationLinkResourceSpecification);

		
		return commSpecCommand;
	}
...
////		LinkingResource linkingResource = (LinkingResource)request.getElementsToEdit().get(0);
////		CommunicationLinkResourceSpecification communicationLinkResourceSpecification = linkingResource.getCommunicationLinkResourceSpecifications_LinkingResource();
////		ICommand throughputCommand = new ThroughputEditHelperAdvice().getCommand(communicationLinkResourceSpecification);
////		//ICommand latencyCommand = new LatencyEditHelperAdvice().getCommand(request);
////		
////		ICompositeCommand result = new org.eclipse.gmf.runtime.common.core.command.CompositeCommand("");
////		result.add(throughputCommand);		
////		//result.add(latencyCommand);
//		
//		
//		return result;
//	}
