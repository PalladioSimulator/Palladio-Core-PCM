...
public class ConfigureMinimalSeffCommand  extends ConfigureElementCommand {
	...
	private CommandResult createControlFlow(Start start,
			Stop stop, IProgressMonitor monitor) {
		SetRequest setReq = new SetRequest(start, UsagemodelPackage.eINSTANCE
				.getAbstractUserAction_Successor(), stop);
        SetValueCommand createControlFlowCommand = new SetValueCommand(setReq);
        try {
			createControlFlowCommand.execute(monitor, null);
		} catch (ExecutionException e) {
            Log.error(UsageHelperPlugin.getDefault(),
                    	-1, e
                        .getLocalizedMessage());
	...
	private CommandResult createSEFFAction(IElementType typeId,
			IProgressMonitor monitor) throws ExecutionException {
		CreateElementRequest startRequest = new CreateElementRequest(myRequest
				.getElementToConfigure(), typeId, UsagemodelPackage.eINSTANCE
				.getScenarioBehaviour_Actions_ScenarioBehaviour());
		startRequest.setLabel("Create Action");
		CreateElementCommand createStartCommand = new CreateElementCommand(
				startRequest);
