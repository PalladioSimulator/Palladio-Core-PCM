...
import de.uka.ipd.sdq.pcm.gmf.seff.edit.policies.VariableUsage2ItemSemanticEditPolicy;
...
public class VariableUsage2EditPart extends ShapeNodeEditPart {
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.SEMANTIC_ROLE, new VariableUsage2ItemSemanticEditPolicy());
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof WrappingLabel4EditPart) {
			((WrappingLabel4EditPart) childEditPart).setLabel(getPrimaryShape().getFigureVariableUsageReferenceLabelFigure());
			return true;
		}
		if (childEditPart instanceof VariableUsageVariableCharacterisation2EditPart) {
			IFigure pane = getPrimaryShape().getFigureParametricParameterUsageRectangleCompartment();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((VariableUsageVariableCharacterisation2EditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof VariableUsageVariableCharacterisation2EditPart) {
			IFigure pane = getPrimaryShape().getFigureParametricParameterUsageRectangleCompartment();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.remove(((VariableUsageVariableCharacterisation2EditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected IFigure getContentPaneFor(IGraphicalEditPart editPart) {
		if (editPart instanceof VariableUsageVariableCharacterisation2EditPart) {
	...
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(PalladioComponentModelVisualIDRegistry.getType(WrappingLabel4EditPart.VISUAL_ID));
	}
	...
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == PalladioComponentModelElementTypes.VariableCharacterisation_3035) {
				return getChildBySemanticHint(PalladioComponentModelVisualIDRegistry.getType(VariableUsageVariableCharacterisation2EditPart.VISUAL_ID));
			}
		}
	...
	public class ParametricParameterUsageFigure extends RectangleFigure {
		...
		private void createContents() {
			...
			fFigureVariableUsageReferenceLabelFigure.setText(new PCMStoExPrettyPrintVisitor().prettyPrint(((VariableUsage) resolveSemanticElement())
					.getNamedReference_VariableUsage()));
			fFigureVariableUsageReferenceLabelFigure.setBorder(new MarginBorder(getMapMode().DPtoLP(2), getMapMode().DPtoLP(0), getMapMode().DPtoLP(2),
					getMapMode().DPtoLP(0)));
			...
			this.add(fFigureParametricParameterUsageRectangleCompartment, constraintFFigureParametricParameterUsageRectangleCompartment);
