...
public class PalladioComponentModelNewDiagramFileWizard extends Wizard {
	...
	public PalladioComponentModelNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot, TransactionalEditingDomain editingDomain) {
		...
		myDiagramFilenameSelectionPage = new WizardNewFileCreationPage(Messages.PalladioComponentModelNewDiagramFileWizard_CreationPageName,
				StructuredSelection.EMPTY);
		myDiagramFilenameSelectionPage.setTitle(Messages.PalladioComponentModelNewDiagramFileWizard_CreationPageTitle);
		myDiagramFilenameSelectionPage.setDescription(NLS.bind(Messages.PalladioComponentModelNewDiagramFileWizard_CreationPageDescription,
		...
		myDiagramRootElementSelectionPage = new DiagramRootElementSelectionPage(Messages.PalladioComponentModelNewDiagramFileWizard_RootSelectionPageName,
				diagramRoot);
		myDiagramRootElementSelectionPage.setTitle(Messages.PalladioComponentModelNewDiagramFileWizard_RootSelectionPageTitle);
		myDiagramRootElementSelectionPage.setDescription(Messages.PalladioComponentModelNewDiagramFileWizard_RootSelectionPageDescription);
	...
	public boolean performFinish() {
		...
		try {
			...
			PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Unable to set charset for diagram file", e); //$NON-NLS-1$
		}
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain, "Initializing diagram contents", affectedFiles) { //$NON-NLS-1$
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
				int diagramVID = PalladioComponentModelVisualIDRegistry.getDiagramVisualID(myDiagramRootElementSelectionPage.getSeff());
				...
				Diagram diagram = ViewService.createDiagram(myDiagramRootElementSelectionPage.getSeff(), ResourceDemandingSEFFEditPart.MODEL_ID,
		...
		try {
			...
			PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Unable to create model and diagram", e); //$NON-NLS-1$
		} catch (IOException ex) {
			PalladioComponentModelSeffDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
	...
	private class DiagramRootElementSelectionPage extends WizardPage {
		...
		private void populateComboBox() {
			...
			for (ResourceDemandingSEFF seff : myFoundSeffs) {
				myCombo.add("Container: " + getComponentNameFromSEFF(seff) + " - SEFF " + getServiceNameFromSEFF(seff) + " id: " + seff.getId());
			}
		}
		...
		protected String getServiceNameFromSEFF(ResourceDemandingSEFF seff) {
			return (seff.getDescribedService__SEFF() == null ? "" : seff.getDescribedService__SEFF().getServiceName());
		}
