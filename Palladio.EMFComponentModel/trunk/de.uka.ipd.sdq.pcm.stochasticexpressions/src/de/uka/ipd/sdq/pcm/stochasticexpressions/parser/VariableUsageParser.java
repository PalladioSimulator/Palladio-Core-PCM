		...
		case LITERAL_IntPMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt564>=1 ) { break _loop564; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
		...
		case LITERAL_BoolPMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt578>=1 ) { break _loop578; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
