		...
		case LITERAL_IntPMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt872>=1 ) { break _loop872; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
		...
		case LITERAL_BoolPMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt890>=1 ) { break _loop890; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
