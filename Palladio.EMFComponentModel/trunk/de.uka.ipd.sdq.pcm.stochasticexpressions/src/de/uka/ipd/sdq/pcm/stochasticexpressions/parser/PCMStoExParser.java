		...
		case LITERAL_IntPMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt547>=1 ) { break _loop547; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
		...
		case LITERAL_DoublePMF:
		{
			...
			do {
				if ((LA(1)==LPAREN)) {
					...
					if ( _cnt550>=1 ) { break _loop550; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
