...
public class BasicComponentImpl extends ImplementationComponentTypeImpl implements BasicComponent {
	...
	public boolean ProvideSameInterfacesAsImplementationType(DiagnosticChain diagnostics, Map<Object, Object> context) {
		if (ProvideSameInterfacesAsImplementationTypeInvOCL == null) {
			...
			String body = " if      self.implementationComponentType->notEmpty()      and   self.implementationComponentType.parentCompleteComponentTypes->notEmpty()  then        self.providedRoles_InterfaceProvidingEntity->collect(pr : ProvidedRole | pr.providedInterface__ProvidedRole.id)->asSet()      =           self.implementationComponentType.parentCompleteComponentTypes->collect(pr | pr.providedRoles_InterfaceProvidingEntity.providedInterface__ProvidedRole.id)->asSet()  else   true  endif ";
	...
	public boolean RequireSameInterfacesAsImplementationType(DiagnosticChain diagnostics, Map<Object, Object> context) {
		if (RequireSameInterfacesAsImplementationTypeInvOCL == null) {
			...
			String body = " if      self.implementationComponentType->notEmpty()      and   self.implementationComponentType.parentCompleteComponentTypes->notEmpty()  then        self.requiredRoles_InterfaceRequiringEntity->collect(pr : RequiredRole | pr.requiredInterface__RequiredRole.id)->asSet()      =           self.implementationComponentType.parentCompleteComponentTypes->collect(cct : CompleteComponentType | cct.requiredRoles_InterfaceRequiringEntity.requiredInterface__RequiredRole.id)->asSet()  else   true  endif ";
