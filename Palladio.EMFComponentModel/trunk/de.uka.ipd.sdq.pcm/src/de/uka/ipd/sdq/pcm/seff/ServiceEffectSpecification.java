 ...
 *   <li>{@link de.uka.ipd.sdq.pcm.seff.ServiceEffectSpecification#getBasicComponent_ServiceEffectSpecification <em>Basic Component Service Effect Specification</em>}</li>
 * </ul>
 * </p>
 *
...
public interface ServiceEffectSpecification extends EObject {
	...
	/**
	 * Sets the value of the '{@link de.uka.ipd.sdq.pcm.seff.ServiceEffectSpecification#getBasicComponent_ServiceEffectSpecification <em>Basic Component Service Effect Specification</em>}' container reference.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 ...
	 * <!-- end-user-doc -->
	 * <!-- begin-model-doc -->
	 * if self.describedService__SEFF.oclIsKindOf(OperationSignature) then
	 * 	self.basicComponent_ServiceEffectSpecification.providedRoles_InterfaceProvidingEntity->exists(p | p.oclIsKindOf(OperationProvidedRole) and (p.oclAsType(OperationProvidedRole).providedInterface__OperationProvidedRole = self.describedService__SEFF.oclAsType(OperationSignature).interface__OperationSignature))
	 * else
	 * 	if self.describedService__SEFF.oclIsKindOf(EventType) then
	 * 		self.basicComponent_ServiceEffectSpecification.providedRoles_InterfaceProvidingEntity->exists(p | p.oclIsKindOf(SinkRole) and (p.oclAsType(SinkRole).eventGroup__SinkRole = self.describedService__SEFF.oclAsType(EventType).eventGroup__EventType))
	 * 	else
	 * 		true
	 * 	endif
	 * endif
	 ...
	 * <!-- end-model-doc -->
	 * @model annotation="http://www.eclipse.org/uml2/1.1.0/GenModel body='if self.describedService__SEFF.oclIsKindOf(OperationSignature) then\r\n\tself.basicComponent_ServiceEffectSpecification.providedRoles_InterfaceProvidingEntity->exists(p | p.oclIsKindOf(OperationProvidedRole) and (p.oclAsType(OperationProvidedRole).providedInterface__OperationProvidedRole = self.describedService__SEFF.oclAsType(OperationSignature).interface__OperationSignature))\r\nelse\r\n\tif self.describedService__SEFF.oclIsKindOf(EventType) then\r\n\t\tself.basicComponent_ServiceEffectSpecification.providedRoles_InterfaceProvidingEntity->exists(p | p.oclIsKindOf(SinkRole) and (p.oclAsType(SinkRole).eventGroup__SinkRole = self.describedService__SEFF.oclAsType(EventType).eventGroup__EventType))\r\n\telse\r\n\t\ttrue\r\n\tendif\r\nendif'"
	 * @generated
	 */
