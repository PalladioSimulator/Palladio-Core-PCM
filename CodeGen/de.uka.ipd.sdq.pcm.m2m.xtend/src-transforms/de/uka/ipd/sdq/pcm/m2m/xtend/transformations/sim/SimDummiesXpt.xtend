...
class SimDummiesXpt extends DummiesXpt {
	...
	def dispatch dummyMethodBody(OperationSignature os, System s, OperationRequiredRole r) '''
		...
		«val annotations1 = s.qosAnnotations_System.map[specifiedQoSAnnotations_QoSAnnotations].flatten.filter[(it instanceof SpecifiedReliabilityAnnotation) && (it.role_SpecifiedQoSAnnotation==r) && (it.signature_SpecifiedQoSAnnation == os)]»
		...
		if (ctx.getModel().getConfig().getSimulateFailures()) {   
			...
			«FOR fod : (annotations1.head as SpecifiedReliabilityAnnotation).externalFailureOccurrenceDescriptions__SpecifiedReliabilityAnnotation»«fod.failureOccurrence(r.id, os.id)»«ENDFOR»
		}
		«ENDIF»
		«val annotations2 = s.qosAnnotations_System.map[specifiedQoSAnnotations_QoSAnnotations].flatten.filter[(it instanceof SystemSpecifiedExecutionTime) && (it.role_SpecifiedQoSAnnotation==r) && (it.signature_SpecifiedQoSAnnation == os)]»
		«IF annotations2.size == 1»
		double delay = de.uka.ipd.sdq.simucomframework.variables.converter.NumberConverter.toDouble(ctx.evaluate("«(annotations2.head as SpecifiedExecutionTime).specification_SpecifiedExecutionTime.specification.specificationString()»"));
	...
	def dispatch dummyMethodBody(InfrastructureSignature is, System s, InfrastructureRequiredRole r) '''
		de.uka.ipd.sdq.simucomframework.variables.stackframe.SimulatedStackframe resultStackFrame = 
			new de.uka.ipd.sdq.simucomframework.variables.stackframe.SimulatedStackframe();
		«val annotations1 = s.qosAnnotations_System.map[specifiedQoSAnnotations_QoSAnnotations].flatten.filter[(it instanceof SpecifiedReliabilityAnnotation) && (it.role_SpecifiedQoSAnnotation==r) && (it.signature_SpecifiedQoSAnnation == is)]»
		«IF annotations1.size == 1»
		// Simulate a failure that occurs with a predefined probability:
		if (ctx.getModel().getConfig().getSimulateFailures()) {   
			double accProbability = 0;
			double randValue = Math.random();
			«FOR fod : (annotations1.head as SpecifiedReliabilityAnnotation).externalFailureOccurrenceDescriptions__SpecifiedReliabilityAnnotation»«fod.failureOccurrence(r.id, is.id)»«ENDFOR»
		}
		«ENDIF»
		...
		«val annotations2 = s.qosAnnotations_System.map[specifiedQoSAnnotations_QoSAnnotations].flatten.filter[(it instanceof SystemSpecifiedExecutionTime) && (it.role_SpecifiedQoSAnnotation==r) && (it.signature_SpecifiedQoSAnnation == is)]»
		«IF annotations2.size == 1»
		double delay = de.uka.ipd.sdq.simucomframework.variables.converter.NumberConverter.toDouble(ctx.evaluate("«(annotations2.head as SpecifiedExecutionTime).specification_SpecifiedExecutionTime.specification.specificationString()»"));
		logger.info("SystemExternalCall «annotations2.head.signature_SpecifiedQoSAnnation.entityName» delaying execution for "+delay);
		ctx.getThread().hold(delay);
