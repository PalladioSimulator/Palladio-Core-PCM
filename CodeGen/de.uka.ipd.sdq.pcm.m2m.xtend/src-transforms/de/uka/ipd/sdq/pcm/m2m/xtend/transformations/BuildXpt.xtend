...
class BuildXpt {
	...
	def buildProperties(UsageModel um) {
...
<project name="generated-filesets">
	...
	<fileset id="ejb-classes" dir="${dist-dir}">
		«val systems = um.usageScenario_UsageModel
			.map[scenarioBehaviour_UsageScenario.actions_ScenarioBehaviour].flatten
			.filter(typeof(EntryLevelSystemCall))
...
<application-client version="5" xmlns="http://java.sun.com/xml/ns/javaee" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://java.sun.com/xml/ns/javaee http://java.sun.com/xml/ns/javaee/application-client_5.xsd">
    ...
    «val systemCalls = um.usageScenario_UsageModel.map[scenarioBehaviour_UsageScenario.actions_ScenarioBehaviour].flatten
    	.filter(typeof(EntryLevelSystemCall)).map[providedRole_EntryLevelSystemCall].toSet»
	...
	def dispatch ejbClientRef(OperationProvidedRole role) '''
		<ejb-ref>
			...
			«val delegationConnector = (role.providingEntity_ProvidedRole as System)
		    	.connectors__ComposedStructure
		    	.filter(typeof(ProvidedDelegationConnector))
		    	.filter[outerProvidedRole_ProvidedDelegationConnector == role].head»
			«val componentContext = delegationConnector.assemblyContext_ProvidedDelegationConnector»
			<ejb-link>«componentContext.encapsulatedComponent__AssemblyContext.javaName()»_«role.providedInterface__OperationProvidedRole.javaName»_«componentContext.id»</ejb-link> 
		</ejb-ref>
	'''
	...
	def dispatch ejbClientRef(InfrastructureProvidedRole role) '''
		<ejb-ref>
		    ...
		    «val delegationConnector = (role.providingEntity_ProvidedRole as System)
		    	...
		    	.filter[outerProvidedRole_ProvidedDelegationConnector == role].head»
			«val componentContext = delegationConnector.assemblyContext_ProvidedDelegationConnector»
			<ejb-link>«componentContext.encapsulatedComponent__AssemblyContext.javaName()»_«role.providedInterface__InfrastructureProvidedRole.javaName»_«componentContext.id»</ejb-link> 
		</ejb-ref>
	'''
	
	def applicationXML(UsageModel um) {
