...
class SimSensorsXpt extends SensorsXpt {
	...
	// Take current time
	((org.palladiosimulator.probeframework.probes.TriggeredProbe)this.startStopProbes.get("Â«s»").get(0)).takeMeasurement(ctx.getThread().getRequestContext());
	'''
	
	def endResponseTimeMeasurement(String s) '''
	// Take current time
	((org.palladiosimulator.probeframework.probes.TriggeredProbe)this.startStopProbes.get("Â«s»").get(1)).takeMeasurement(ctx.getThread().getRequestContext());
	'''
